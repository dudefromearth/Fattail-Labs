...
| **Options Lab XSP/SPY scale (active program · DL-700 · XS)** | Plan **v1.3** landed `origin/main`. Board `agents/p-options-lab-xsp-spy-scale/`. Token `agents/go/XS0-W0.md` **GO**. Gates XS0-G…XS5-G **PASS**. Third tree alongside LIM and QFRIC. WIDTH-1 `a27f187` prerequisite. No LIM file, no QFRIC file, no `AnalyzerPositionsList`. Live AT recapture 5/5 on Coach MacBook. MiniTwo is Coach remote (named 2026-09-13; this agent does not SSH). **NX18:** a passing XS gate is not IKI progress. Runner is Template Runner (`web/lib/runner/`). **FI-050** XS-ETF deferred. |
| **Practice Position Lifecycle (active program · DL-702 · PPL)** | Plan **v1.1** SOUND. Spec **v0.1.1 BUILD AUTHORITY**. Board `agents/p-practice-position-lifecycle/`. Token `agents/go/PPL0-W0.md` **GO**. Fourth tree alongside LIM, QFRIC, and XS. Isolation: trade-log domain / routes / capital_positions / `web/lib/tradeLog.ts` / trade-log UI / tests. **No** matcher FIFO rewrite. **No** LIM file, no QFRIC file, no XS file, no `AnalyzerPositionsList`. StudioTwo only. OD-9 / OD-22 / FI-PPL-1 blocked until PPL4. |
| **Help Watch (active program · DL-704 · HW)** | Spec **v0.3 BUILD AUTHORITY**. Board `agents/p-help-watch/`. Token `agents/go/HW-W0.md` **GO**. Seat **Sierra**. First job: retrospective sweep (one Help PR per app area). Isolation: `server/help_reference/*.md` + this board. **No** product trees, no Wiki poller edits, no MiniTwo, no straight-to-main until sweep close. Wiki Follow = existing S1 poll. |
| **VP data end (INFRA instance · DL-720 · VPS / VPSB)** | **Singular drive (DL-727):** 1 download → 2 bins → 3 API. Contract v1.0 frozen (**DL-726**). VP **v0.6.1** `7e3bbedc…`. SA **v0.4 authored** `d68060cc…` (**DL-731**; supersedes v0.3.1; pointer closed). Advisor r2 vs **v0_3_1** → **v0.4.1 via Coach**. Composite fenced. Tonight: VPS1-G → VPS2 ACT 3 (GO only) → VPSB ACT B → backfill tranche 1. Board `agents/p-volume-profile-service/`. **CP-1**. **APPS** `SADEV*` READ-ONLY. **ADVISOR** reviews. |
| **Heatmap Term Mass (GBH instance · DL-747 · DL-763 · GC)** | Fifth tree alongside LIM, QFRIC, XS, PPL. Picker **Term Mass**, id `gex-cal`. Spec **v0.1.1 BUILD AUTHORITY** `FatTail-Labs-Options-Lab-Heatmap-GEX-Calendar-Spec-v0_1_1.md` sha1 `37c4427c2ffc3071dc27a8592c16cf6307acd289`. Plan **v1.1**. Token `agents/go/GC0-W0.md` **GO**. Board `agents/p-options-lab-heatmap-gex-calendar/`. Data: frozen `gex_v1` generations. Flag until GC4-G. **Not** VP stores/surfaces. **Not** the VP heatmap overlay. **Not** LIM / AF / Width Fit files. StudioTwo only. |
