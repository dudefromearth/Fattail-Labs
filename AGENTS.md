...
| **VP data end (INFRA instance · DL-720 · VPS / VPSB)** | This instance is **GROK BUILD — INFRA**: tokens `VPS*` / `VPSB*`, collectors / Engine / mapping / API, StudioOne acts, tonight VPS2 ACT 3 → futures migration. Seated **VP v0.6** + **SA v0.3** (**DL-721**). Board `agents/p-volume-profile-service/`. **CP-1**. Collector wins StudioTwo contention. **APPS** (`SADEV*` · `SA-DEV-W0` **STAMPED GO** **DL-723**) reads ingest **READ-ONLY**. **ADVISOR** owns spec reviews — INFRA does not self-review. Cross-instance conflict → STOP, report to Coach. |
