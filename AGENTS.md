...
| **VP data end (INFRA instance · DL-720 · VPS / VPSB)** | This instance is **GROK BUILD — INFRA**: tokens `VPS*` / `VPSB*`, collectors / Engine / mapping / API, StudioOne acts, tonight VPS2 ACT 3 → futures migration. Seated **VP v0.6** + **SA v0.3** detection (**DL-721**). SA v0.4 file = **DIRECTIVE POINTER** (**DL-728**), not BUILD. **Singular drive (DL-727):** 1 download → 2 bins → 3 API. Contract v1.0 frozen (**DL-726**). Board `agents/p-volume-profile-service/`. **CP-1**. Collector wins StudioTwo contention. **APPS** (`SADEV*` · `SA-DEV-W0` **STAMPED GO** **DL-723**) reads ingest **READ-ONLY**. **ADVISOR** owns spec reviews — INFRA does not self-review. Cross-instance conflict → STOP, report to Coach. |
