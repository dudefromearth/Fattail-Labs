# FatTail Labs — Options Lab Analyzer Algo Alert Spec v2.3.2
...
**Current revision:** **v2.3.2** — **E37–E43, making the model computable and its folds visible.**
...
move_unit  = LABS_ALGO_MOVE_SIGMA × sigma_1min
             × √( LABS_ALGO_MOVE_HORIZON_MIN )                  [points]
             × spot
```
...
**Clause A gets the same treatment, on its own key.** When `p × headroom` falls to within
`LABS_ALGO_REGIME_NEAR_PCT × H` of `at_risk`, the Guide object enters **approaching** (§9.6, E41) and
the surface names it on the canvas **before** the fold. A regime fold that arrives unannounced is the
worst version of this failure, because there is no line movement to see coming.

`REGIME_NEAR_PCT` is **not** `LOOSEN_NARRATE_PCT`. v2.3.1 reused one knob for accumulated guide
movement and for the clause-A approach margin — different quantities, different semantics, and the
next change to either would have silently moved the other. Same reasoning that keeps
...
| HUD | Lower-left, just above $0: **High** · **Profit** · **Given back** · **Budget** · **Guide** (guide print `x_S`). Colons aligned. Shown while **Managing** and — **E3** — **frozen and still visible** in **Fold suggested**. Hidden in Armed, In trade, Idle. Telemetry, not a flatten. |
...
| Regime state | **A state of the Guide object itself, not a fifth widget (E41).** Two states only — **clear** and **approaching** — driven by the clause-A margin `p × headroom − at_risk`. Tokenized, legible **without hover**, **never colour-alone**, and **never a number or a percent** (§13, AT-ALGO-36). `approaching` is entered at `LABS_ALGO_REGIME_NEAR_PCT × H` of equality. Clause A can fire with no line movement at all, so without this the member's first sight of a regime fold is the fold itself (**E32 · E41**). **AT-ALGO-50.** |
...
| **AT-ALGO-50** | **Clause A is visible before it fires.** Replaying fixture 20's tick sequence, the Guide object reads **approaching** on at least one tick **before** Fold suggested fires. Asserted legible without hover, on a touch viewport, and **not by colour alone**. A packet that paints clause A with no regime state is a **blocking defect** **(E41)**. |
...
1. **`MOVE_HORIZON_MIN` stamped** (E37, default 3). No golden may be written before it — the horizon
   sets clause A's personality and `PaR` is quadratic in it.
2. Fixtures **19–27** exist as hand-written numbers — including **21b**, **22a/22b**, **26** (so
   AT-ALGO-44 has a row) and **27** (so AT-ALGO-49 has one) — with **20 / 21 / 21b** built at mid-`H`
...
| `LABS_ALGO_REGIME_NEAR_PCT` | `ALGO_REGIME_NEAR_PCT` | **2.0** (% of `H`) — clause-A margin at which the Guide object enters **approaching**. **Not** `LOOSEN_NARRATE_PCT` (E41 · E42) |
...
| `LOOSEN_NARRATE_PCT` (2.0) / `REGIME_NEAR_PCT` (2.0) | accumulated **guide movement** / clause-A **margin** |
...
|---|---|---|---|---|
| **20** | `H` $1,890 · `U` $1,840 · `headroom` $810 · `Δ` $5/pt · `Γ` −$28/pt² · `m_adv` −4 · `PaR` $204 | `p` 0.22 → budget **$178** | `$50 < $178 ≤ $204`; `U` $1,840 **above** guide $1,712 | **FOLD** `regime` |
| **21** | same | `p` 0.50 → budget **$405** | `$50 < $405` and `$405 > $204` | **HOLD** |
...
Note `m_adv = −4` throughout: the signed adverse increment (**E39**). The 21b wing arithmetic is
`40×(−4) + ½(4)(16) = −128 → PaR $128`.
...
## Appendix C — errata index (E1–E43)
...
| **E41** | **Regime margin is a state of the Guide object** — two states, **clear** / **approaching**, tokenized, no hover, no colour-alone, no number. `REGIME_NEAR_PCT`. **No packet paints clause A without it.** AT-ALGO-50. |
