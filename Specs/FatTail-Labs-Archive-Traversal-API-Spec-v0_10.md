# FatTail Labs — Archive Traversal API Spec v0.10

**Status:** **DRAFT — v0.9 as built (DL-677) plus §3.7.1, the fill-friction model, DESIGN
ONLY, not built.** Coach directed the controls on 2026-09-06 (*"We need controls on the friction
model"*); §3.7.1 is the specification those controls are built against once Coach approves it.
Not yet stamped: Coach (§3.7.1, OD-ATRV-13), India (boundary), Sheldon (greek quantum, fill
estimator, OD-ATRV-12), Foxtrot (host, tier).
**Date:** 2026-09-06 · **Short name:** **ATRV** · **Owner:** Juliet (draft) → Alpha / Foxtrot
**Supersedes:** v0.9 (2026-09-06). v0.10 adds the friction model's shape and controls, and
the spread measurement they rest on (`docs/evidence/quant-spread-probe-XSP-2026-09-04.*`).
v0.9 was the first revision written **from measurement
...
| Fill placement | `mid ± f × half-spread`, `f ~ U[0.5, 1.0]`; spread read from the store at that instant | spread is real; the placement law is assumed — **and on a one-tick market it is fiction (§3.7.1)** |
...
#### 3.7.1 The friction model — controls, shape, and what the spread probe says (DESIGN, not built)

Coach, 2026-09-06: *"We need controls on the friction model."* And before that: *"Based on
spread is probably most realistic"*; *"sometimes it can take 10–30 seconds to fill"*; *"or you
can just apply a certain probability based on the VIX."* This section is the shape those
sentences take. **Its mechanics are declared; its probabilities are fitted (Sheldon, OD-ATRV-6).
Nothing here replaces the unfitted constant with a made-up curve — it replaces the made-up
*placement* with measured structure, and gives the constant an honest meaning until the fit
lands.**

##### What the spread probe measured — XSP 2026-09-04, session only

`scripts/quant-spread-probe.py` (read-only, 0.5 s per book-day) over the built store, per side
× **signed OTM distance** × 30-minute ET slot. Sorted quantiles; no means.

| Side · OTM distance | quotes | bid **null** | half-spread p10 / p50 / p90 ($) | ask p99 when bid null |
|---|---|---|---|---|
| Put ITM | 111,168 | 0.0% | 0.025 / **0.265** / 0.310 | — |
| Put 0–2 | 12,898 | 2.2% | 0.005 / 0.010 / 0.010 | 0.02 |
| Put 2–5 | 19,347 | **14.3%** | 0.005 / **0.005** / 0.010 | 0.02 |
| Put 5–10 | 32,245 | **34.7%** | 0.005 / **0.005** / 0.010 | 0.02 |
| Put 10–20 | 40,505 | **69.2%** | 0.005 / 0.005 / 0.010 | 0.02 |
| Put 20+ | 6,192 | 95.8% | 0.005 / 0.010 / 0.010 | — |
| Call 2–5 / 5–10 / 10–20 | 19,347 / 32,245 / 40,540 | 28.0% / 52.4% / 88.4% | 0.005 / 0.005 / 0.010 | 0.02 |

Three findings, each of which changes the model's shape:

1. **Where Coach trades, the quoted spread is one tick.** OTM puts 2–10 points from spot are
   quoted $0.01–0.02 wide all session (p90 half-spread $0.010). In dollars the spread is not
   the friction on a $0.30–0.50 fly. A model that places a fill "inside the half-spread" on a
   one-tick market is placing it at a price that does not exist: **the grid is the tick, and
   the tick is the only inside there is.** The v0.9 placement law is retired by this table.
2. **The friction is one-sidedness.** A third of 5–10 OTM put quotes and two thirds of 10–20
   OTM carry **no bid at all**. A leg you are *selling* into a null bid does not fill at any
   price. A leg you are buying does (the ask is there). This is a law, not a knob (below).
3. **The vendor mid on a null-bid quote is `ask / 2`** — the bid-as-zero convention. On this
   day it is **immaterial**: where the bid is null the ask is ≤ $0.02 at p99, so the mid is
   $0.005–0.010 and the mark error is under a dollar per contract. It is recorded here because
   a wide-ask null-bid quote (max seen: $4.80, one contract, once) would make it material, and
   because the mark series trusts it without saying so. OD-ATRV-12 decides whether the mark
   keeps the vendor convention or names it. **Not changed in this version** (change control).

The ITM half-spread ($0.27) is recorded and irrelevant to OTM flies; it matters the day a fly
is run into.

##### The object: a complex order at a net limit, resting in the path

A butterfly is sent as **one order at one net debit**. It fills whole or not at all. The v0.9
placeholder fills three legs independently, which is pessimistic in the wrong shape (it can
fill two wings and no body — a position Coach has never held). The model's unit is therefore
the **complex order**: legs, quantities, a **net limit**, a **resting window**, and a
**re-seat policy**. Per snapshot `k` inside the window the store yields, from the legs' quotes
at that instant:

| Quantity | Definition | Null-bid rule |
|---|---|---|
| `complex_natural` | Σ buy-legs × ask − Σ sell-legs × bid | a sell leg with null bid → **undefined this snapshot**; the order cannot fill |
| `complex_mid` | Σ qty × mid, vendor mid (OD-ATRV-12) | as the vendor marks it, `ask/2`, labelled |
| `edge_ticks` | `(limit − complex_mid) / tick`, **positive = paying above mid** | — |
| `marketable` | `limit ≥ complex_natural` (buying) | — |

**Fill law, per snapshot in the window:** if `marketable`, the order fills at its limit with
probability `1 − p_miss_marketable` (latency; fitted, unfitted default below). Otherwise it
fills with hazard `h_k = P_fit(edge_ticks, one_sided, regime)`, a monotone curve in
`edge_ticks` that **Sheldon fits from Coach's fill history joined to the store's state at each
order** (OD-ATRV-13). **A fill is always at the limit — never better.** Price improvement is
not assumed; that is the pessimistic side, deliberately (§3.7, "The default is pessimistic").
When the window expires unfilled, the re-seat policy improves the limit by `improve_ticks` and
opens a new window, up to `max_reseats` times; after that the order is **cancelled, counted
`no_fill.entry`, and the path does not trade** (AT-ATRV-17 unchanged).

**The unfitted state keeps an honest meaning.** Until a fit is stamped, the flat constant
`LABS_QUANT_FILL_P_UNFITTED` (0.85) is the **probability the order fills within its window**,
not per snapshot — spread across the window's `K` snapshots as the constant hazard
`h = 1 − (1 − p)^(1/K)`. (Applying 0.85 *per snapshot* over fifteen snapshots would make a
fill near-certain, the opposite of pessimistic.) Every response in this state still says
`fill_model: unfitted_pessimistic`, `fit_id: null`.

**Exit is the same machinery, selling.** `exit_kind = target` becomes what Coach actually does:
a **closing order resting from the fill instant** at `(1 + pct/100) × debit`, evaluated every
snapshot against `complex_mid` (hazard) and the selling-side natural (marketable). It is no
longer "the first mid-mark touch" — a touch is where the order *could* fill, and the fill law
decides whether it did. `exit_kind = time` sends a marketable closing order at `t_exit` with the
same window and re-seat policy; if it is still unfilled at the end of the last window, the
position **crosses to natural**, and any long leg with a null bid is **abandoned at $0** and
named in `assumptions.abandoned_legs[]` — never marked at a fictional mid.

The `legged` mode — v0.9's independent legs — is **kept only as a contrast**, labelled, never
the default.

##### The controls — every one on a declared grid

QLAB §4.2 holds: no off-grid control, and a study sweeping controls counts cells against the
**64-cell ceiling**. Each control is returned verbatim in `assumptions.controls{}`; an
off-grid value is refused `CONTROL_OFF_GRID`, never rounded.

| Control | Grid | Default | Kind |
|---|---|---|---|
| `order_type` | `complex` · `legged` | `complex` | declared |
| `limit` | `{kind: abs, debit}` on the $0.01 grid — Coach's **10%-of-width rule** is `abs` ($0.50 on a 5-wide, $0.30 on a 3-wide); or `{kind: offset, ticks}` ∈ {−1, 0, +1, +2, natural} from `complex_mid` | `offset +1` | declared |
| `window_s` | 10 · 20 · 30 · 60 seconds, converted to snapshots at the day's cadence and reported as both | 30 | declared |
| `reseat` | `improve_ticks` ∈ {0, 1} × `max_reseats` ∈ {0, 1, 2, 3} | improve 1, max 2 | declared |
| `regime_factor` | {0.5, 0.75, 1.0} multiplier on the hazard — Coach's VIX dial, **declared until a VIX column exists in the store** (OD-ATRV-14) | 1.0 | declared → fitted |
| `p_miss_marketable`, `P_fit` | **not controls** — fitted parameters carried by `fit_id`; unfitted state uses the window constant above | — | fitted |
| `null_bid` | **not a control — a law.** A sell leg with null bid cannot fill; at forced exit it is abandoned at $0 | — | law |
| fees | `LABS_QUANT_FEE_PER_CONTRACT`, fail loud | config | config |

Six controls; the largest legal single sweep is one axis at full width (`limit` offsets × 5)
crossed with one other (`window_s` × 4 = 20 cells), well under the ceiling. A member never sees
a control off its grid, and the page shows the grid, not a free field.

##### The fit — OD-ATRV-13

`P_fit` is fitted from **Coach's fill history**: one row per order sent, with `sent_at` (ET),
`book`, `legs` (the `628P:+1,…` string), `open|close`, `limit`, `filled` (bool), `filled_at`,
`reseats`, `cancelled_at`. Each row is joined to the store at `sent_at` to recover
`complex_mid`, `complex_natural`, `edge_ticks`, and one-sidedness — the covariates the law
uses. The fit is a monotone logistic in `edge_ticks` with a one-sidedness term; the fitted
object is `fit_id`, `n_orders`, date range, parameters, and is **byte-reproducible** from the
history file plus the store (AT-ATRV-43). Twenty orders make a first curve that is honestly
labelled `n_orders: 20`; the label is what keeps a small fit from being read as a law.
Broker exports are the source of record; a remembered dozen is admissible for a first curve
and says so in `fit_id`.
...
| **OD-ATRV-6** | **Fill model** — shape and controls now specified in §3.7.1 (complex order at a net limit, resting window, re-seat, null-bid law); the **hazard curve `P_fit` and `p_miss_marketable` are Sheldon's to fit** from OD-ATRV-13's history | **Sheldon** | Until stamped, `unfitted_pessimistic` with the window-constant meaning of §3.7.1; no made-up curve |
| **OD-ATRV-12** | **Mark basis when the bid is null** — keep the vendor `ask/2` convention (measured immaterial on 2026-09-04: ask ≤ $0.02 at p99 where bid is null) or name it in the mark response | **Sheldon** | Keep the vendor mid, **label it** (`mark_basis: vendor_mid_null_bid_as_zero`), and count null-bid legs per mark. Never a liquidation mark inside the mark series — that would double-count the exit tax |
| **OD-ATRV-13** | **Fill history for the fit** — source (broker export vs remembered orders), schema in §3.7.1, and where it lives (not in the repo: it is Coach's trading record) | **Coach** | Broker export as the source of record; a remembered dozen admissible, labelled in `fit_id` |
| **OD-ATRV-14** | **Regime dial** — `regime_factor` as a declared control until a VIX (or realised-vol) column is in the store, then fitted per band (era-2) | **Sheldon · Foxtrot** | Declared control, grid {0.5, 0.75, 1.0}, default 1.0 |
...
| **AT-ATRV-39** | Every friction control is **on its declared grid** and echoed verbatim in `assumptions.controls{}`; an off-grid value is refused **`CONTROL_OFF_GRID`**, never rounded or clamped (§3.7.1, QLAB §4.2). |
| **AT-ATRV-40** | In `complex` mode an order **fills whole at its limit or not at all**; no response shows a path holding a subset of the legs, and no fill is ever recorded better than the limit (§3.7.1). |
| **AT-ATRV-41** | A **sell leg whose bid is null never fills** at any price; at a forced time exit it is abandoned at $0 and named in `assumptions.abandoned_legs[]`. A fixture with a null-bid body at entry produces zero entry fills at that instant (§3.7.1). |
| **AT-ATRV-42** | In the unfitted state the per-snapshot hazard integrates to the declared window probability: for a window of `K` snapshots, `1 − (1 − h)^K = LABS_QUANT_FILL_P_UNFITTED` within 1e-9, for every `K` on the grid at the day's cadence. The response carries `fill_model: unfitted_pessimistic`, `fit_id: null` (§3.7.1). |
| **AT-ATRV-43** | A `fitted_v1` response carries `fit_id`, `n_orders`, and the history's date range; re-running the fit from the same history file and store yields **byte-identical parameters**. A fit whose history rows cannot be joined to a store instant is refused, not imputed (§3.7.1, OD-ATRV-13). |
| **AT-ATRV-44** | `exit_kind = target` is a **resting closing order** from the fill instant, subject to the fill law; a target touched on the mid-mark and not filled is reported as such (`exit.touched_at`, `exit.filled_at`), never as an exit at the touch (§3.7.1; supersedes the touch semantics of AT-ATRV-36 once §3.7.1 is built). |
...
| **v0.10** | 2026-09-06 | **§3.7.1 — the fill-friction model: controls, shape, and the measurement it rests on. Design only; not built.** Coach: *"We need controls on the friction model."* The spread probe (`scripts/quant-spread-probe.py`, evidence `quant-spread-probe-XSP-2026-09-04.*`) found that **where Coach trades the quoted spread is one tick** — so v0.9's "place inside the half-spread" is placing at a price that does not exist, and is retired — and that **the friction is one-sidedness**: 35% of 5–10 OTM put quotes and 69% of 10–20 carry no bid. The model's unit becomes the **complex order at a net limit, resting in the path** for a declared window with a re-seat policy; fills are at the limit, never better; a sell leg into a null bid never fills; the unfitted constant gains an honest meaning as the **probability of filling within the window**, spread as a constant hazard (0.85 per snapshot would have been near-certain). `target` exit becomes a **resting closing order** subject to the same law, which is what Coach does. **Six controls, each on a declared grid** (order type, limit as Coach's 10%-of-width absolute or an offset in ticks from the complex mid, window 10–60 s, re-seat, regime factor, fees), echoed verbatim, off-grid refused; the probabilities are **fitted, not controls** (Sheldon). The fill-history schema for the fit is specified (OD-ATRV-13). The vendor's null-bid mid (`ask/2`) is measured **immaterial on this day** (ask ≤ $0.02 at p99 where bid is null) and left unchanged pending OD-ATRV-12 — an earlier chat line calling it "fiction" is corrected here. OD-ATRV-12…14. AT-ATRV-39…44. |
