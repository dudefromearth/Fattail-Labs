# FatTail Labs — Autofilter Spec v0.2

**PARKED (DL-586).** Not the active Autofilter program. Journal, records, and six extra
columns wait on a later Coach GO. Active program: Trade Log Autofilter Spec
**v0.1.1** (`Specs/FatTail-Labs-Trade-Log-Autofilter-Spec-v0_1_1.md`).

**Provisional filename and version. Coach names the real ones.**

| | |
|---|---|
| Status | **PARKED** — DRAFT advisor; not BUILD AUTHORITY; not the next GO |
| Supersedes | v0.1 (2026-08-25) |
| Date | 2026-08-25 |
| Type | Product / UX — shared filter component across Practice surfaces |

**Scope statement (this parked document).** Surfaces named here: **the Trade
Log, records, and the journal**, plus **Campaign** as the extract source and for badge
routing. Touches outside program: **NONE**. Trade Log shipped on the narrow spec;
records and journal are **not** authorized by this parking.

**DL-539 is satisfied by Coach naming these surfaces.** That rule governs *drift* — an
agent reaching into code Coach did not name. Coach named these; the naming is the
authorization, and no further sign-off is required to work inside it.

**Three OKs would apply** if this component were proposed for a surface Coach has not
named. It must not reach Reports, Capital, Positions, or anything else without
Coach's direction.
...
### 2.2 One cut per surface

Ship the button removal **in the same change that mounts Autofilter** on that surface.
Not progressively, not "buttons stay until Autofilter is perfect."

A migration window where both exist is the defect state §2.1 forbids — it does not
become acceptable by being temporary.
...
**One filtered stream out.** Filter state and distinct-value collection live **inside**
the shared component. A surface must not pass pre-filtered rows *and* apply a second
client-side filter — that is the dual-mechanism failure moved inward where it is harder
to see.
...
**If it stays, it must be identical to the column filter — not merely equivalent.**
Same state object, same clear path, same contribution to `shown/total`. A chip driving
its own private filter is precisely the second mechanism §2 exists to kill, and it
would be worse than the button being removed because it looks like a shortcut rather
than a mechanism.

**OPEN — Coach:** remove the chip, or keep it under the identity rule above? No default
is written here.
...
**OPEN — Coach.** Three shapes, no default written:

| | Behaviour | Cost |
|---|---|---|
| **Select-time** | The conflicting value greys out and explains | Prevents the mistake. Cannot catch conflicts only visible after composition |
| **Apply-time** | Selection allowed, fails loud on apply | Catches everything. Lets the member make the mistake first |
| **By class** *(Grok)* | Select-time where the conflict is knowable from current distinct values; apply-time where it only appears after composition | Most precise. Two behaviours to build and to explain |

All three are fail-loud. They differ in when, and in how much is built.

**Persistence.** The Trade Log stores UI preference in localStorage under
`ft.tradeLog.lastUsed.v1`.

**If filters do persist, they get their own named key and their own visible control.**
Do not silently merge filter state into `ft.tradeLog.lastUsed.v1` — a filter the member
cannot see and did not knowingly set is the "where did my data go" failure with no
affordance to recover from it.

**OPEN — Coach:** does a filter survive a page reload, or start clean each visit? Sticky
filters are the main source of members concluding data is missing; starting clean costs
a re-selection. No default is written here.
...
| A16 | If the `Open:N` chip stays | Same state object as Status=Open — one clear path, one `shown/total` |
| A17 | Any surface, any commit | Never both mechanisms at once |
| A18 | Component receives rows | One filtered stream out; no surface-side second filter |
...
| # | Item | § |
|---|---|---|
| 1 | Which surface is "records" — **blocks Records seeds only**; Trade Log and journal proceed without it | 5 |
| 2 | `Open:N` chip — remove, or keep under the identity rule? | 4 |
| 3 | Conflicts — select-time, apply-time, or by class? | 8 |
| 4 | Do filters persist across reloads? | 8 |
| 5 | Default view per surface, and whether the choice is remembered | 6.1 |

**Struck from Grok's review:** a separate Coach sign-off to work on the named surfaces.
Coach named them, and DL-539 governs drift — an agent reaching into code Coach did not
name — not directed work. Requiring a second authorization for the first would be
ceremony on Coach's own instruction.
