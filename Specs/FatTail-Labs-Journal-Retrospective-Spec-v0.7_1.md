# FatTail Labs — Journal Retrospective Spec v0.7
...
### 6.2 Layout — the map and one step

**A 3×3 grid of nine tiles, persistent, with the focused step's content below it.**

The grid is a **diagram of the ceremony**. A member looks once and sees that a retrospective has nine
parts and what state each is in. That is the comprehension a scrolling list of cards never delivers,
and the procedure is the thing being taught (§2).

It replaces the step rail rather than adding a level. Grid above, one step's content below, always
both — so opening a step never isolates it from its neighbours (§6.1).

| Element | Behavior |
|---|---|
| Grid | Persistent. Tiles numbered 1–9, laid out 1-2-3 / 4-5-6 / 7-8-9 |
| Tile | Number, title, and **state** |
| Below the grid | The focused step, expanded |
| Order | Any tile, any time. Numbering carries the sequence |

**Tile states:**

| State | Shows |
|---|---|
| Has content | A summary — "4 tags, 2 deviations" |
| Nothing here | Quiet, present, never omitted |
| **Needs you** | Distinct treatment — the member has to write something |

**"Needs you" is what makes the grid earn its place.** Of nine steps, only **5** (what I name as the
cause) and **8** (the one thing) require the member to author anything; the other seven are assembled
for them. In a scrolling layout that is invisible, and a member reads nine cards without knowing
where they are actually needed. On a grid it is two lit tiles, and the ceremony reads correctly:
look at seven things, write two.

A tile summary compresses rather than hides — a tile showing only a title has concealed information;
one showing "nothing here" or "4 tags, 2 deviations" has summarized it.

**The grid is the only multi-column element at ceremony level. Content stays one column below it, and
multi-column inside a step is fine** — step 2's period readings across
a row are correct. Steps side by side are not: two columns make reading order ambiguous, and the
order is the thing being taught.

**Not a wizard** (§6.1): every step is reachable in any order, nothing is withheld pending an answer,
and there is no Next button. Progressive focus is a default state, not a gate.

**Nine expanded sections mostly reading "nothing here" is the failure this replaces.** Each empty
step costs a screen and conveys less than a line would.
...
withheld pending an answer to a previous one; no Next-button pagination. One step expanded by default below a
persistent 3×3 grid; tiles render number, title, and state; steps 5 and 8 render a distinct "needs
you" state and no other step does; every step reachable from any tile in any order; step *content*
never renders side by side.
...
| 2 | Indicator window — **recommended lock:** one instrument, two contexts, never one frame. Ceremony period-scoped, Journey rolling, no side-by-side (§7) | India + Tango |
...
| 9a | **Maiden scope on import** — `practice_epoch` resolves to earliest trade, so an importing member's first ceremony spans their entire history. Maiden scope starts at import; prior history is baseline context, not review scope | India + Hotel |
| 9b | **Rate labels** — "activity days / wk" currently renders a raw count. Every per-period label must be normalized or relabelled (§12) | Alpha + India |
| 9 | **Routine-day definition** (§12.2) — message-timestamp based, amending Journal Session v0.6 and Journey v1.0 §4.1 as one decision | India |
| 10 | **Keep-rate presentation and pairing** (§11.1) — member-facing fact only; product evaluation reads it only alongside a specificity trend | Coach + Tango + Hotel |
| 11 | **Notification channel policy** (§14) — in-app only, or email with a defined payload | Mike |
