# FatTail Labs — Journal Session Spec v0.7

**Status:** **BUILD AUTHORITY** — Coach GO 2026-08-13 · **DL-325** (B-Agent override) ·
**DL-326** (this spec) · board `agents/p-journal-session-v07/` · plan
`docs/Journal-Session-v0.7-Full-Agent-Bench-Plan.md` v1.1. Implements ruled **B-Agent**.
...
| **Member Data Privacy v0.1 · Member Practice Portability v1.4** | Family B; export/purge. This spec's store delta requires **v1.5** (J7-7, after charter). |
...
- **Book has unmatched opens →** the Coach receives; acknowledges briefly or not at all;
  answers only non-analytically. **Zero unprompted questions, zero analysis (asked
  analysis is a hard reject), zero emotional processing.** A member question such as
  "what do you think of this trade?" does not license an analytic turn. Violations are
  hard-rejected at the same render layer as every other §5 gate.
...
- **Closed field set:** SoR is the **code** union of `TAG_FIELD_SPECS` in
  `server/journal_session_structured.py`. Documented snapshot (identical on GO day):
  `instrument` · `thesis_direction` · `trigger_level` · `size_risk` · `invalidation` ·
  `watching` · `plan_diff` · `deviations` · `what_worked` · `open_thread` ·
  `differed_from_plan` · `note`. No psychological, state, or emotion fields. **Unknown
  keys are a hard reject** (code, not review). Extract-and-confirm does **not** restore
  v0.2 required-for-complete / seal gates — absent remains lawful.
...
purge yes, export no — §7). **Practice Portability version bump required (v1.5 — J7-7, after charter stores exist).**
...
| 6 | Portability **v1.5** bump contents | Lima · India | J7-7 only — not charter GO |
...
| 2026-08-13 | **v0.7 DRAFT rev 2** — advisor review folded (H1–H6, S1–S5) |
| 2026-08-13 | **v0.7 BUILD AUTHORITY** — DL-325 / DL-326. P1 closed keys listed; P2 Portability v1.4 / v1.5 later; P3 asked-analysis hard reject |

---

*BUILD AUTHORITY — DL-325 · DL-326. Charter implementation: J7-0…J7-4.*
