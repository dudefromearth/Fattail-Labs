# FatTail Labs — Member Campaign Structured Practice Spec v1.2
...
**Supersedes:** [`FatTail-Labs-Member-Campaign-Structured-Practice-Spec-v1.1.md`](./FatTail-Labs-Member-Campaign-Structured-Practice-Spec-v1.1.md)  
**Date:** 2026-08-08  
**Type:** Practice Campaign model (structured practice · instances · charter bounds · bound roles · **Campaign Journey**)  
**v1.2 delta (Coach-ratified):** Adds the **Campaign Journey** — the charter **radar chart** + **time scrub / slider bound T0 (campaign start) → present** (Delta Handoff **D5**; four design laws normative, §6a; acceptance #20–#23). Reuses Journey **J2 pattern** (slider↔chart), never Journey data. **D6 (convexity gauge / vol-correlation module) is deliberately queued, not dropped:** blocked on the **vol data-source OD** (external VIX feed per Sacred #1); lands as v1.3 or companion spec once the OD is approved.  
**v1.1 delta (Coach-ratified):** Restores the **Two Roles** of charter bounds — **`boundary`** (corridor / variance) and **`goal`** (mark / progress). Source: [`docs/Delta-Handoff-Goal-Role-Restoration-and-Bench-Findings.md`](../docs/Delta-Handoff-Goal-Role-Restoration-and-Bench-Findings.md) (D1) plus bench findings A–C (D2–D4).  
...
## 6a. Campaign Journey — charter radar + time scrub (normative, v1.2 / D5)

**Framing (Coach lock):** the Journey app is the **trader's** journey — the person, lifelong, unending. This is the **Campaign Journey** — one season, one contract, **finite by design**: born at signature, ends at completion or honest early end. Day = atom · campaign = finishable journey · trader = unending journey — finished journeys are what make the unending one gradeable. The Archive is the bookshelf of completed journeys; Renew is the sequel; the cycle chain is a series.

**Surface:** Charter campaign detail gains a **radar chart** + **time slider bound T0 (signature/first fill) → present**, bidirectional (drag slider ↔ click chart; reuses the Journey J2 slider–chart component pattern). Scrubbing to time T replays the season's biography.

### Four design laws (normative)

1. **Axes = the declared bounds.** A five-bound charter renders a five-axis radar; the radar is *this campaign's* fingerprint, not a fixed template. Both roles render (boundary axes and goal axes, role-aware). The **ledger has no radar** — furniture has no shape.
2. **Axes plot band-alignment, never raw magnitude (load-bearing).** Raw win-rate would render 85% against a 40–60 band as a glorious spike — celebrating drift toward the high-win harvest curve. Axis extension = **alignment with the declared range**: full in-band, decaying off-band **on either side**; goal-role axes = **progress toward the mark**. Outer ring = "operating as declared / reaching what was declared" — same outer-ring semantics as the Journey compass. Big shape means **faithful**, never big numbers. **Hotel gates the off-band decay function** (doctrine, not styling).
3. **Scrub to T evaluates fills [T0→T] against the bounds in force at T** — same as-of-T reconstruction law as §5.4 (amendment rows + signed snapshot). Amendments visibly bend the shape at their dates; **no retroactive rewriting** of the earlier season. Goal axes at T read progress against the goal ranges in force at T.
4. **n-floor renders as focus.** Below-validity axes render **"gathering"** (treatment = Echo: dimmed / center-pinned / undrawn). Process-clause axes wake immediately (witnessable per fill); signature axes wake at their n-floors — the scrub shows the fingerprint **coming into focus** as statistical validity arrives. The visualization teaches sample-size honesty with zero copy.

### Guards

- **No Journey feed** — shared pattern, never shared data plane; campaign-scoped data stays on campaign surfaces.
- **Derived at render, never stored** — no shape-series table (same law as panel readings and cycle numbers, §5.6).
- Clinical register only: "gathering / in range / tracking toward" — never grade or celebration chrome.
- Not Reports chrome (DL-257); not P&L — axes are alignment/progress values, never equity or returns.

### Opens (noted, non-blocking)

- **Shape-strength companion line** under the radar (single alignment-over-time curve; nearly free once per-T shapes compute; where "the week the season loosened" becomes visible) — Echo/Coach.
- **Cycle compare** (this season's shape vs predecessor at equivalent elapsed time) — V-next, not v1 freight.
- Member-facing label: **Campaign Journey** is the concept name; UI wording = Tango's vocabulary map.
...
20. **Radar axes = declared bounds only** (both roles, role-aware render); a charter with no bounds shows an empty-state invitation, not a fake shape; the **ledger has no radar**.  
21. **Band-alignment, not magnitude:** axis extension decays off-band on **both** sides (regression: an out-of-band-HIGH win-rate reading **reduces** extension — the drift toward high-win harvest must not render as a spike); goal axes render progress toward the mark.  
22. **Scrub temporal honesty:** slider bound **T0 → present**, bidirectional with the chart; at T the shape derives from fills [T0→T] against **bounds in force at T**; amendment dates visibly segment the series; earlier shape unchanged by later amendments.  
23. **Focus rendering:** below n-floor axes render "gathering," never fabricated values; process-clause axes wake immediately; radar derived at render (no stored shape series).  
...
- Raw-magnitude radar axes (alignment/progress only — law §6a.2)  
- Stored radar shape-series tables (derive at render)  
- Radar/panel data as Journey compass input (shared pattern, never shared plane)
...
| Hotel | Style × horizon frames **with goal defaults beside boundary defaults**; band semantics; n-floors; R:R basis; win definition; structure-family taxonomy; Sharpe keep/kill; **expectancy cross-check as frame-authoring sanity** (§7b); which process attributes are goal-eligible; **§6a off-band alignment-decay function** (radar semantics are doctrine) |
| Tango | Mandatory-stamping copy; variance neutrality; panel clinical register; **goal progress register** (never grade); ledger naming; **End campaign / Ended early vs Abandoned surface doctrine** (§6); import path labels (ledger vs pick vs new — no “none/unstamped”); **Campaign Journey vocabulary (label, axis states, scrub copy)** |
| Echo | Trade-sheet density; **dual panel UI** (corridors + progress); Desired Outcomes block; suffix preview; restamp UX; import sheet path chrome; **§6a radar + slider (J2 component pattern); gathering-axis treatment; shape-strength line keep/kill** |
| Mike | Family B; import names; import stamp provenance; **role** on pack bounds |
| Kilo | §10 characterization tests incl. **#15–#16**, **#17 goal never variance**, **#18 critical rejects goal**, **#19 pack role**, expectancy cross-check optional; **#20–#23 radar: celebrate-the-drift regression (out-of-band-high reduces extension), as-of-T amendment segmentation, gathering states, no stored series** |
...
| Date | Note |
|------|------|
| 2026-08-08 | **v1.2** — **Campaign Journey** added (§6a): charter radar + time scrub T0→present per Delta Handoff **D5**; four design laws normative; acceptance #20–#23; gates extended (Hotel decay function, Echo radar/slider, Tango vocabulary, Kilo regressions). **D6 (convexity gauge) queued, not dropped** — blocked on vol data-source OD; lands v1.3 or companion spec. Supersedes v1.1. |
