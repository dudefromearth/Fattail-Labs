# FatTail Labs — Options Chain Picker Spec v1.0.2

**Status:** **RATIFIED** (v1.0.1 base Coach GO 2026-08-10 DL-279…280) · **v1.0.2** fold 2026-08-10 (OC6a · routes · Market Bus parent · OC15 stages)  
**Date:** 2026-08-10  
**Current revision:** **v1.0.2**  
**Canonical filename:** `Specs/FatTail-Labs-Options-Chain-Picker-Spec-v1.0.2.md`  
...
**As-built paths (v1 land / v1.0.2 route reconciliation):**

| Layer | Path |
|-------|------|
| UI (primary) | `web/app/app/options-lab/page.tsx` · **route `/app/options-lab`** (Apps card as-built string) |
| UI (legacy) | `web/app/app/market/chain-ladder/` → **redirect** to `/app/options-lab` |
...
**OD-nav (product catalog name):** Path parent remains **Market / analyzer** area; card string **“Options Lab”** is **as-built only** until Coach + Echo settle OD-nav (picker bench). Spec law refers to **chain ladder surface**, not a ratified catalog “Lab” tier name.
...
| [Massive Market Bus & Shared Client Spec v1.0](./FatTail-Labs-Massive-Market-Bus-Shared-Client-Spec-v1.0.md) | **Scale / transport parent (DRAFT until Coach GO)** — Redis multi-worker OC15 (MB-P1) **successor** to in-process H1-2; one WS shared client |
...
**Nav placement (DL-232 lesson · v1.0.2 as-built):**

| Choice | Meaning |
|--------|---------|
| **As-built primary route** | **`/app/options-lab`** — first member surface of the **Market / options analyzer** parent area (not a Practice suite pill, not a Strategy Lab tab). |
| **Legacy** | `/app/market/chain-ladder` redirects to `/app/options-lab`. |
| **Not** | Suite chrome tab that fights Design sub-nav (DL-232 Symbols fight). |

Subsequent analyzer surfaces (e.g. smile, term structure) **inherit this parent area**. Echo/Coach may rename “Market” → “Analyzer” or ratify “Options Lab” as catalog name via **OD-nav** without changing OC laws.
...
| **OC6a — Exact listed strikes (display)** | Strike cells show the **listed** contract strike **cent-exact** (e.g. AAPL **302.50**). Formatting that rounds half-strikes to whole dollars (302.50 → 303) is **forbidden**. Integer strikes may omit trailing zeros. Law lives on this surface Spec; Market Bus e2e AT-MB5 verifies transport does not strip precision. |
...
| **OC15 — Shared upstream fetch (scale)** | **One Massive chain snapshot fetch per `(feed_symbol, expiration)` (and fetch generation) per server TTL window** — not one upstream call per member poll. Side / wings / band / diff hash are applied **from the shared cached snapshot**. N members polling the same contract share one generation (Arch/18 S-3 · Process Runtime shared-generation doctrine). Client poll interval remains ~2s; **upstream** rate is TTL-bounded. **Implementation stages:** (1) **H1-2 minimal** — in-process generation + single-flight (single-worker honest); (2) **MB-P1 production** — Redis multi-worker store per [Market Bus Spec](./FatTail-Labs-Massive-Market-Bus-Shared-Client-Spec-v1.0.md) §1.2 — **successor**, not a second OC15 theater. Delta must not pass OC15 twice on unrelated evidence. |
...
| **v1.0.2** | 2026-08-10 | **OC6a** exact listed strikes; Market Bus parent + OC15 H1-2/MB-P1 stages; as-built **`/app/options-lab`** + legacy redirect; canonical filename `…-v1.0.2.md`; OD-nav name still open |
