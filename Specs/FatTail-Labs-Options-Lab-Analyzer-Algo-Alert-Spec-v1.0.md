...
**Status:** **SUPERSEDED** as product law by [AZ-ALGO v2.0](./FatTail-Labs-Options-Lab-Analyzer-Algo-Alert-Spec-v2.0.md) (**DL-660**). Kept as the **as-built characterization** of W1–W4 (Demo-only trail). Original header follows.

**Status (historical):** DRAFT — Coach 2026-08-21 (Analyzer Algo alert). **v1.0.16** HUD **Profit** (current gain) between High and Trail. **v1.0.15** product is **Trader Feed** (**TF** · **DL-517**). **v1.0.14** Reason floater is a **Trader Feed venue** (market + position + trader aware; same base market info — **DL-516**). **v1.0.13** Reason floater is FN **timestamped post tape**; house base **is** the host **instructions prompt** (**DL-515**). **v1.0.12** Reason floater **supplements** via Labs **Feature Narrative** ([FN](./FatTail-Labs-Feature-Narrative-Spec-v0.1.md) · **DL-514**) — Algo is a **host**; trail/Reason/ALGO-N1 unchanged. **v1.0.11** floating narrative is feature-specific and context-aware (trader + examining position(s)). **v1.0.10** house Reason **base prompt** is in-place admin on Alerts Manager (`/app/alerts`). **v1.0.9** one **Reason** for the whole trail; markdown prompt; T Ortho–similar narrative floater (**ALGO-R1**). Supercedes two-stop hold/fold (**DL-484**). **v1.0.8** Start/End Trail % boxed with Decay. **v1.0.7** HUD only while Live + Armed. **v1.0.6** High / Trail / Stop HUD. **v1.0.5** trail % = **give-up of profit** (`S = (1−g)×H`). **v1.0.4** no narrative on the Algo panel (**ALGO-N1**). **v1.0.3** add fly after Time Machine day (**ALGO-TM1**). **v1.0.2** floor default **25** (DL-482) · knobs are law · Recorded `mode`. **v1.0.1** review fold (ALGO-B1 far-side regime · ALGO-A1 monotone `f` · ALGO-A2 pulse hysteresis). Not BUILD AUTHORITY until Coach Phase 5.  
...
> So, the Narrative is feature-specific and contectually aware including aware of the trader and their current position(s) they are examining
    ...
    **These features** = Analyzer **Algo** Reason floater **and** Surface **T Ortho** squawk. Same **job**: a **floating narrative for trader review**. Same **awareness law**: feature-specific and **contextually aware** of **this trader** and the **position(s) they are examining**.
...
> So, this is a new feature I am proposing. It will take the place of the narrative box in T Ortho, and it will supplement the Algo Alert feature, and it may be used in other geatures throughout FatTail Labs.
    ...
    **Seating:** Algo **Reason floater is a host** of [Trader Feed](./FatTail-Labs-Trader-Feed-Spec-v0.1.md) (`host_id: algo-reason`). TF **supplements** this feature — it does **not** replace trail math, knobs, Reason, house base, HUD, or **ALGO-N1**. T Ortho remains OL-TO (no algo, no trail). DL-513 “two hosts, two products” is **reshaped for chrome** (one primitive); **eval isolation** is kept. This spec still owns the Algo **voice**, mount gate (**ALGO-R1**), and trail.
...
> Each place the feature is employed, it will have an instructions prompt to follow. The narrative will be a continuous scroll, so the user can view older posts. The posts will be timestamped. This feature is very similar to the Jounaling feature. But it is generalized
    ...
    **Seating:** Algo house base **is** this employment’s **instructions prompt**. Reason markdown remains optional **focus**. The FN body is a **continuous scroll of timestamped posts** (append, not overwrite). Journal-shaped, generalized — not the Journal product. **ALGO-N1** still: no narrative on the Builder panel.
...
> One of my members referred to this feature as the trader feed. A merket and position and trader aware contrinuous narrative. It is customized per venue. But largely based on the same base market info.
    ...
    **Seating:** Algo Reason is one **venue**. Chrome title **Trader Feed**. Customization = house base + optional focus + Algo voice. Base market info is the shared OPF / Arch 28 / Heatmap-held plane — not a second Massive. T Ortho is a **different venue** on the **same** prints.
...
| **Trader Feed (TF)** | **Chrome** for the Reason floater. This spec owns the **host pack** (mount = Reason; voice = §9.2; examining = bound fly). TF **supplements** Algo — it does not replace the trail. [TF spec](./FatTail-Labs-Trader-Feed-Spec-v0.1.md) · **DL-514** · **DL-517**. |
| **T Ortho egg** | **Visual reference only** (floatable glass, drag, play-by-play). Analyzer uses **dark-pinned HI tokens**, not a copy of T Ortho hex / position-list / tape prefs. Do **not** share T Ortho’s `localStorage` key. Do **not** import `TimeOrthoEggPanel`. |
...
**What it affects:** **not the alert.** A **Trader Feed** instance on the Analyzer canvas (`host_id: algo-reason`). Similar in **job** to Surface T Ortho trader review. **Not** an import of `TimeOrthoEggPanel`. Mount law is §9.

**Trader review (Coach §0.15 / §0.16):** this floater exists for **trader review**. It **supplements** Algo via FN (**DL-514**). It is **Algo-specific** (trail / 0DTE OTM butterfly / Reason). It is **contextually aware** of **this member** and the **position(s) they are examining** (focused / Shown book on this Analyzer, bound `position_id`, Algo knobs). It does not narrate a generic market or another member’s book. T Ortho’s squawk is the **other** feature’s **host pack** — do not merge stores, prompts, or eval.
...
3. **Standing context** (Algo knobs, 0DTE OTM butterfly, Heatmap on the plane, **this trader**, **position(s) they are examining**)
...
**Chrome** is Labs **Trader Feed** (`algo-reason`): glass panel, **drag**, **timestamped post tape**, on-screen clamp ([TF](./FatTail-Labs-Trader-Feed-Spec-v0.1.md) §5–§5.2). **Not** a copy of `TimeOrthoEggPanel`. Body is markdown **posts** (append). House base **is** the instructions prompt; member Reason field is optional focus.

| Law | |
|-----|--|
| Dialect | Dark-pinned work-surface **tokens** (same as Builder). |
| Drag | Header grab; remember position (`ft_labs_trader_feed_pos_algo-reason` — do not share the egg key or T Ortho’s TF key). |
...
| Does not | Change alert state, `f`, `H`, `S`, Recorded, or the position. Fail-open: named “AI quiet” / local **posts**, never a silent empty tape while Reason is on. |
| testid | `analyzer-algo-narrative` (alias) plus `data-trader-feed="algo-reason"` |
| Review | **Trader review** of **this** Algo on **this** bound fly (and Shown book they are examining). Feature-specific. FN **supplements**; T Ortho is a **different host pack**, not this voice. |
...
| **On, prompt blank, not yet Armed** | Floater mounts; **basic** waiting **post**; no trail-life model tick. |
| **On, prompt blank, Armed** | **Basic narration posts** from the **house base** (instructions prompt, §5.5) plus standing context (Algo knobs, 0DTE OTM butterfly, Heatmap raw, §9.2 voices). Empty house base is **admin fail-loud** (prompt required); member tape fail-open = local **posts** + named “AI quiet”. |
| **On, prompt present, Armed** | Same basic narration **posts**, **focused** by the member prompt (market condition or trader concern). Must not divert from primary purpose (or from the house base). Each tick **appends**. Egg `/session-note` overwrite is **out**. |

Cadence: Keep-Warm Working / Away. Not 1s. Threaten/pulse is visual; the next allowed tick may **append** “spot is threatening the trail.” The model does **not** have to fire every Keep-Warm tick — bounded, not a chat. Older posts remain; timestamps visible (TF §5.1).
...
On exit the tape **appends a Recorded post** in the same voice (process facts): high-water, trail at exit, `f`, spot, time ET, debit, **`exit_side` (near \| far)**, whether entry had armed. **No** “you booked a winner.” Near vs far are different lessons (“faded off the high” vs “blew through the tent”) — name the side, do not verdict the P&L. Then the alert **stops** (§6 Recorded). Older posts stay on the tape.
...
**Tango (member-facing copy — labeled):** Labs chrome, titles, notifications, and the narrative window speak **process** (waiting, armed, high-water, trail %, threaten, recorded, decay). They **do not** promise P&L, “maximize profit,” or “bank it.” Coach named the HUD row **Profit** — it is **current gain telemetry** (`U`), not a Labs promise. Coach’s Reason assumption includes **“potential profit”** as the *subject of inference* — that is the job of the model, not a Labs headline or notification. A member-authored entry % is their telemetry (AZ-ALB §7). Invariant #8 / ALM #5 still govern what **Labs** says.
...
| Floating narrative for trader review; feature-specific; aware of this trader and examining position(s) | **IN-SCOPE** · §0.15 · TF host `algo-reason` (**DL-514** · **DL-517**); eval still isolated from T Ortho |
| Trader Feed **supplements** Algo (does not replace trail / Reason / ALGO-N1) | **IN-SCOPE** · §0.16 · §2 · **DL-514** · **DL-517** |
| Instructions prompt + timestamped post tape (Journal-shaped, generalized) | **IN-SCOPE** · §0.17 · house base **is** the prompt · **DL-515** |
...
| **AT-ALGO-R3** | Reason **checked** (prompt empty or not) → `analyzer-algo-narrative` mounts with **basic narration** as FN **posts**. Unchecked → not mounted. Clearing the prompt does not unmount. |
| **AT-ALGO-R4** | Reason on does **not** change arm, `S`, Recorded, or the position. Built-in trail always. |
| **AT-ALGO-R5** | Model active only while the trail is in effect (**Armed**). Fail-open named **timestamped post**, never a silent empty tape while Reason is on. Ticks **append** (**AT-FN-11…13**). |
...
| **AT-ALGO-R8** | Floater is **Algo-specific** and **context-aware**: this member and the examining / bound position(s). Chrome is Trader Feed `algo-reason` (**AT-FN-4**). Not the T Ortho **voice**. Not a generic market tape. |
| **AT-ALGO-17** | **Live + Armed** Algo paints lower-left of the plot, just above $0: **High** (H) · **Profit** (current U, same units as High) · **Trail** (g %) · **Stop** (ticker `x_S`). Colons aligned. Hidden unless Live and Armed. Host `data-algo-highest` / `data-algo-profit` / `data-algo-trail` / `data-algo-stop`. |
...
| Narrative panel | Trader Feed host `algo-reason`; T Ortho / egg **not** imported. Mounts under ALGO-R1. |
...
| Ver | Date | Notes |
|-----|------|--------|
| **v1.0.16** | 2026-08-21 | HUD: **Profit** (current gain) between High and Trail. **DL-518**. |
| **v1.0.15** | 2026-08-21 | Product renamed **Trader Feed** (**TF**). **DL-517**. |
| **v1.0.14** | 2026-08-21 | Algo Reason is a **Trader Feed venue** (market + position + trader aware; same base market info). **DL-516**. |
| **v1.0.13** | 2026-08-21 | Coach: FN tape is **timestamped posts** (append, continuous scroll). House base **is** the Algo **instructions prompt**; Reason field remains optional focus. **DL-515**. |
| **v1.0.12** | 2026-08-21 | Coach: Labs **Feature Narrative** **supplements** Algo Alert. Reason floater is FN host `algo-reason`. Trail / Reason / ALGO-N1 / ALGO-R1 unchanged. **DL-514**. |
| **v1.0.11** | 2026-08-21 | Coach: Algo and T Ortho each feed a **floating narrative for trader review**; each is **feature-specific** and **context-aware** (this trader + examining position(s)). Two hosts — do not merge. **DL-513**. |
