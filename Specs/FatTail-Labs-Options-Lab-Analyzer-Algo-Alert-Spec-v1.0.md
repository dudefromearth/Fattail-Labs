...
| **T Ortho egg** | **Visual reference** for the narrative window (floatable glass, drag, play-by-play). Analyzer uses **dark-pinned HI tokens**, not a copy of T Ortho hex / position-list / tape prefs. |
...
| `position_id` | Card id |
| `trigger.family` | `algo` |
| `trigger.algo` | `{ variant: "otm_fly_trail", entry_pct, trail_start_pct, trail_floor_pct, decay_end, trail_stop_reason?, trail_end_reason?, overlay, high_water_color, trail_color }` (`decay_end`: `"eod"` or ISO datetime). Missing reason fields → built-in engine at that stop. |
...
| Dismiss | Follows alert Idle (hide) / Recorded may **remain** with the last tape until Reset or navigating away. Esc does not kill the alert. |
...
**v1 SoR for copy:** **local deterministic** sentences from OPF + GEX + VP + trail math. Must work **without** an LLM. Optional model enrichment (T Ortho `/api/me/options-lab/session-note` family) is **fail-open** and **FLAGGED** for a later pass — not required to Save or Arm.

Cadence: Keep-Warm Working / Away. Not 1s. Threaten/pulse is visual; the window may say “spot is threatening the trail” on the next allowed tick.
...
**Tango (member-facing copy — labeled):** Labs chrome, titles, notifications, and the narrative window speak **process** (waiting, armed, high-water, trail %, threaten, recorded, decay). They **do not** promise P&L, “maximize profit,” or “bank it.” A member-authored entry % is their telemetry (AZ-ALB §7). Invariant #8 / ALM #5 still govern what **Labs** says.

**Hotel:** gamma / debit / probability / GEX / VP lines are **measurements on the OPF plane** (and engaged overlays). Do not invent a greek, a wall, or a VP node. Wrong structure in this window is **severity: high** (capital-adjacent judgment).
...
| T Ortho–similar narrative window | **IN-SCOPE** (chrome reference; HI tokens) |
...
| LLM session-note like T Ortho | **FLAGGED** — local copy is v1; model fail-open later |
...
- Heatmap / Surface / VP **page** drawing these lines (Analyzer 2D only).  
...
| Narrative panel | Analyzer-hosted; not a T Ortho import |
