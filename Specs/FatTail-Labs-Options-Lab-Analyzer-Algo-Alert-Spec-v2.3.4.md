# FatTail Labs — Options Lab Analyzer Algo Alert Spec v2.3.4
...
**Current revision:** **v2.3.4** — **E49–E51.** Errata only, and narrower than v2.3.3: **no geometry
change, no new product decisions, no formula change to the two clauses, and no reopening of E44–E48.**
E49 marks two superseded statements the E44 sweep left standing — the fourth partial sweep this
document has produced, and the first one caught *before* a builder read the stale line. E50 gives §14
the citation it has never carried: criterion 3 is a **taxed shape transform** (ATRV), and this spec
had no sentence preventing it from being answered on untaxed mid-marks. E51 gives the AZALGO-REGIME
isolation rule a **flag** instead of a paragraph. v2.3.3's own summary follows.

**Previous revision:** **v2.3.3** — **E44–E48, closing P0.** Errata only: **no geometry change, no new
...
| # | Change | Why |
|---|---|---|
| **E49** | **Two superseded statements marked.** The **E37** row in §0's history and the **E37** row in the §17 errata index both still print `move_unit … × √HORIZON_MIN × spot` with no pointer to E44. Both now carry **`SUPERSEDED BY E44`**. Live §9.4.1 is confirmed correct and unchanged | A builder greps `move_unit =` and hits the E37 row **eight hundred lines before** the live law. The historical text is right to preserve — an errata log records what was written — but an unmarked superseded formula is indistinguishable from a current one. This document's established failure mode is fixing a line and leaving its twin; this is the fourth instance and the first caught before it reached a build |
| **E50** | **§14 cites ATRV.** Criterion 3 is answered **after tax** as a shape transform (**AT-ATRV-26**), clauses A and B reported separately (**OD-ALGO-10**), and any era-1 evaluation is labelled **`idealised`** and does not promote (**AT-ATRV-29**) | This spec cited ATRV **zero times**. §14.3 could be — and by the cheapest reading would be — answered as two fold counts on mid-marks. A three-strike fly round trip crosses **eight contract-spreads**, 13–67% of a $300 debit (ATRV §3.7): a guide that improves the top band before friction can destroy it after. Promoting the proposed line on untaxed counts is the promotion-gate leak, and one sentence closes it |
| **E51** | **`LABS_ALGO_REGIME_FOLD_ENABLED`**, default **false**, fails loud when absent. While false, clause A may be **computed and logged** but **no Fold suggested is emitted from clause A alone**. It flips true only when AZALGO-REGIME passes (§16) | §16 already says a packet painting clause A outside AZALGO-REGIME's gate has shipped a fold the member cannot see coming — as **prose**. P2 computes clause A and P3/P4 tick a Demo. Between them there is nothing but that paragraph stopping a Fold-suggested-from-A reaching P4, and a paragraph is not a gate. Law with no switch is the same defect class as law with no test (**E48**) |
...
3. **Primary criterion:** whether the proposed line would have folded trades bound for the **top return
   ...
   **This is answered after tax, as a shape transform, or it is not answered (E50).** The question is
   not "how many folds" — it is what the guide does to the **distribution**: untrailed versus trailed
   over the same trades, each tail reported separately, with entry and exit friction charged
   (ATRV §3.7, **AT-ATRV-26**). A three-strike fly round trip crosses **eight contract-spreads** — on a
   $300 debit that is 13–67% depending on the spread, which is first-order, not a haircut. A line that
   improves the top band on mid-marks and loses it to friction has failed this criterion while looking
   like it passed, and that is precisely how a backtest becomes a sales document.

   **Era-1 days cannot answer it.** The tax's probability component is fitted from depth the era-1
   write path discarded (SSR-MEXP §3.1, ATRV §3.6). An era-1 evaluation is labelled **`idealised`**,
   is legitimate for shape exploration, and **does not promote the proposed line**
   (**AT-ATRV-29**). Package `PaR` on era-1 remains a lookup and is available today — the two are
   different claims and E50 exists so they are not conflated.
4. Compare against the **legacy linear taper** on the same trades.
...
| **AT-ALGO-60** | **Criterion 3 is taxed (E50).** A §14.3 evaluation reports untrailed-versus-trailed **distributions** with each tail separate and entry/exit friction charged per leg (ATRV §3.7, AT-ATRV-26). A run reporting two fold counts, or a single net figure, or any figure computed on mid-marks, **fails**. |
| **AT-ALGO-61** | **Era-1 does not promote (E50).** A §14 run over era-1 days carries `fidelity: era1_no_depth` and the label **`idealised`**, and the promotion gate refuses it. A promotion recorded against an `idealised` run **fails** (AT-ATRV-29). |
| **AT-ALGO-62** | **Clause-A isolation is a switch, not a paragraph (E51).** With `LABS_ALGO_REGIME_FOLD_ENABLED=false`, a fixture that satisfies clause A and not clause B produces **no** Fold suggested — while still writing `fold_clause` to the log. **Source grep:** no clause-A emit path that does not read the flag. The key absent → boot aborts. |
...
| `LABS_ALGO_REGIME_FOLD_ENABLED` | `ALGO_REGIME_FOLD_ENABLED` | **false** until AZALGO-REGIME passes (**E51**). While false, clause A is computed and logged and **emits no Fold suggested on its own**. Absent → module load aborts naming it |
...
| **E37** | **`SUPERSEDED BY E44` (formula only).** **Adverse horizon is an explicit key.** `MOVE_HORIZON_MIN` (3) separate from `MOVE_WINDOW_MIN` (20); `move_unit` scales by `√(HORIZON_MIN / 1 min)` — E37 wrote the naked count, see E44. Live law is §9.4.1. Horizon and `P_BASE` are **one calibration**, fitted together at §14.5. Calibration table in §9.4.1. AT-ALGO-52. |
...
| **E49** | **Two superseded E37 statements marked** — §0 history and this index both printed the naked-minute formula with no pointer to E44. Live §9.4.1 confirmed correct. Fourth partial sweep; first caught before a build read it. |
| **E50** | **§14 cites ATRV.** Criterion 3 answered **after tax** as a shape transform (AT-ATRV-26); era-1 runs are `idealised` and do not promote (AT-ATRV-29); clauses reported separately (OD-ALGO-10). This spec previously cited ATRV zero times. AT-ALGO-60, 61. |
| **E51** | **`LABS_ALGO_REGIME_FOLD_ENABLED`** (default **false**, fail loud) isolates clause-A folds until AZALGO-REGIME passes. §16's prose becomes a switch. AT-ALGO-62. |
...
| **OD-ALGO-12** | **The `approaching` visual law.** E41 and E45 settled the *state* and its *threshold*; the surface is still a phrase list — "tokenized, legible without hover, never colour-alone, never a number." No glyph, no position on the Guide object, no reduced-motion behaviour, no HUD fallback | **Echo · Tango** | **Blocking AZALGO-REGIME.** One page, before P5. It is a design decision, not a unit pick, and this spec will not invent it |
...
| Ver | Date | Notes |
|-----|------|--------|
| **v2.3.4** | 2026-09-06 | **Errata only. No geometry change, no product decisions, no formula change, no reopening of E44–E48.** **E49** marks two superseded statements the E44 sweep left standing — §0's E37 history row and the §17 errata index both still printed `√HORIZON_MIN` as a naked minute count with no pointer to E44, eight hundred lines above the live §9.4.1 (confirmed correct and unchanged). The historical text is right to preserve; an *unmarked* superseded formula is not. Fourth partial sweep in this document and the **first caught before a builder read the stale line** — which is the whole reason §17 exists. **E50** gives §14 a citation it has never carried: criterion 3 is a **taxed shape transform** (ATRV §3.7, AT-ATRV-26), untrailed versus trailed with each tail separate, not two fold counts on mid-marks. A three-strike fly round trip crosses **eight contract-spreads** — 13–67% of a $300 debit — so a line that wins the top band before friction can lose it after, which is how a backtest becomes a sales document. And era-1 days **cannot** answer it: the tax's probability component is fitted from depth the era-1 write path discarded, so era-1 runs are labelled `idealised` and do not promote (AT-ATRV-29), while package `PaR` on era-1 stays a lookup available today. This spec previously cited ATRV **zero times**. **E51** turns §16's isolation paragraph into a switch — `LABS_ALGO_REGIME_FOLD_ENABLED`, default **false**, fail loud — because between P2 computing clause A and P4 ticking a Demo there was nothing but prose preventing a Fold-suggested-from-A shipping; law with no switch is E48's defect class with a different name. AT-ALGO-60, 61, 62. **OD-ALGO-12 opened:** the `approaching` visual law is still a phrase list and blocks AZALGO-REGIME — Echo and Tango owe one page, and this spec declines to invent it. |
