...
**Status:** DRAFT — Coach 2026-08-20. **v0.1.5** add position after the day, then Algo. **v0.1.4** Spot field + Autofit X = session **open**. **v0.1.3** no start-time picker. **v0.1.2** speeds 10× / 20× / 50×. **v0.1.1** 390 candles/closes · close-to-close **or** TPO path. Not BUILD AUTHORITY until Coach Phase 5.  
...
**ATM-S2.** Stop returns the playhead to the first print, priced at that session’s **open** (**ATM-O1**).
...
| **Stop** | Pause + return playhead to the **session open** (first print, priced at bar `o` — **ATM-O1**). |
...
**ATM-H5.** Hide the window when no day is selected (Time Machine idle). Stop does **not** hide it.
