# FatTail Labs — Options Lab — Create / Edit Position Dialog — Spec v0.13
...
**Status:** **APPROVED — BUILD AUTHORITY.** Landed `Specs/` 2026-09-12 on Coach's stamp. Consolidated DLG2.
...
| 0.11 | **Coach, 2026-09-12:** *"the white triangle in an edit field to show it is a drop down."* The card's corner-nested menu marker is the dialog's, at dialog scale. New DLG-VOCAB-5. It replaces the platform chevron — a recorded, deliberate deviation from DLG-HIG-3. |
| 0.12 | **Coach, 2026-09-12:** field widths must match the **content type**, not be uniform — QTY holds two digits, not ten; DEBIT three digits and two decimals; the date field a little wider. And **strikes show two decimals on symbols with fractional strike grids** (.25 / .50 / .75). New DLG-LAYOUT-14 and DLG-FN-10. |
| 0.13 | **Coach, 2026-09-12:** *"The expiration field should be the widest, then next the strike field."* The ordering is now a rule in its own right, not an outcome of the sample values. |
...
| **DLG-VOCAB-5** | **The card's menu marker is the dialog's menu marker.** *(Coach, 2026-09-12.)* The same `CardMenuField` the card uses, at `surface="dialog"`. **Form:** a small solid triangle **nested into the bottom-right corner** of the field — right angle at the corner, hypotenuse running upper-right to lower-left, flush to the field's inner edge, legs roughly one third of the field's height. Always visible, never a hover reveal. It is **not a button and not a hit target** (`pointer-events: none`) — the whole field opens the menu. **Colour:** a semantic marker token that contrasts with the field it sits on — light on a dark field, dark on a light one. *"White"* describes the ToS reference, not a literal value; a white triangle on a light field is a defect. **Placement:** on **every field that opens a menu** — symbol, strategy, strike, type, expiration — and **nowhere else**. A field without a menu carries no marker; that is the entire point of it. |
...
| **DLG-LAYOUT-14** | **Each field is as wide as its content type, and no wider.** *(Coach, 2026-09-12: "the width of your edit fields doesn't match the content type… the qty field is unlikely to contain more than 2 digits, yet the field is wide enough for 10 digits.")* Width is sized to the **widest value that type can legitimately hold** — never to a uniform default, and never so tight that a legitimate value truncates or ellipsises. A number is never clipped. Baseline sizing: |
| | **QTY** — signed, two digits: room for `−99`. **STRIKE** — five digits plus two decimals: room for `7635.25`. **EXPIRATION** — the widest formatted date, and **a little wider than it needs**, per Coach. **DEBIT** — three digits plus two decimals: room for `999.99`. **POS** — three digits. **TYPE** is a control, not a text field. Columns stay aligned on the grid (DLG-HIG-4) even though the fields differ in width. |
| | **Width ordering is a rule, not an outcome.** *(Coach, 2026-09-12.)* **EXPIRATION is the widest field in the legs table; STRIKE is the second widest.** DEBIT, POS and QTY follow, each at its own content width. This ordering holds **even if the sample values above change** — shortening the date format does not promote STRIKE past EXPIRATION. |
...
| **DLG-FN-10** | **Strike precision follows the symbol's listed strike grid.** *(Coach, 2026-09-12.)* A symbol whose OPF-held chain lists fractional strikes — .25, .50, .75 — displays strikes to **two decimal places**, so `7635.25` and `7635.50` are distinguishable at a glance. A symbol on whole or five-point strikes displays no decimals. Precision is **read from the chain, never invented** (DL-309): the dialog does not add decimals a symbol does not list, and does not round away a decimal it does. The same precision governs the field, the stepper's increment and the ToS script. |
...
| **AT-DLG-32** | **Width ordering holds, measured.** EXPIRATION is the widest field in the legs table and STRIKE the second widest, at the default and the larger type settings. |
| **AT-DLG-30** | **Field widths match content type.** QTY fits `−99` and no more; STRIKE fits `7635.25`; DEBIT fits `999.99`; POS fits three digits; EXPIRATION is comfortably wider than its longest date. No field is a uniform default width, and no legitimate value truncates or ellipsises. |
| **AT-DLG-31** | **Strike precision follows the symbol.** On a symbol with .25 / .50 / .75 strikes the field, the stepper increment and the ToS script all show two decimals; on a whole-strike symbol they show none. Precision is read from the chain, never invented. |
| **AT-DLG-29** | **Every field that opens a menu carries the corner-nested triangle**, and no field without a menu carries one. It is bottom-right, flush, always visible, not a hit target, and legible against its field in **both** themes — never white on a light field. |
