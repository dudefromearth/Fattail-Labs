# FatTail Labs — Options Lab — Create / Edit Position Dialog — Spec v0.5
...
**Status:** **CANDIDATE — narrow amendment to v0.4. Ready to land on Coach's stamp.**
...
| **DLG-HIG-2** | Spacing runs on a **named 8-point grid — 4 · 8 · 12 · 16 · 20 · 24**. Every gap, margin and inset in the dialog is a value from that scale; an off-grid number is a defect. **20** separates labelled groups; **12** separates rows within a group; **8** separates a label from its control. Related things are grouped, unrelated things are separated. |
...
| **DLG-HIG-10** | **The dialog has real margins, and the panel is wide enough to hold them.** Content is inset **20** from all four edges of the panel — side margins are not optional and content never runs to the edge. The panel widens from **770** to **820** so the new margins *add* room rather than taking it: content width goes from 746 to **780**. The panel remains free-floating and draggable (DLG-LAYOUT-8). *(Coach, 2026-09-11.)* |
