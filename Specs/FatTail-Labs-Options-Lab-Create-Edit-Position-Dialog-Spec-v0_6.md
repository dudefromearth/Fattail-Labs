# FatTail Labs — Options Lab — Create / Edit Position Dialog — Spec v0.6
...
Program of record remains the v1.0 bench plan, executed against this Spec from DLG2.
...
| **DLG-HIG-8** | **Semantic colour only.** Every colour is a semantic role — primary label, secondary label, separator, control background, accent, and the named code-surface token — resolved from the member's theme. A literal hex or a Tailwind palette class such as `bg-emerald-600` or `#22c55e` is a defect. Meaning is never carried by hue alone; Buy and Sell are distinguished by selection state and label, not colour. |
| **DLG-HIG-9** | **Surfaces and separators.** Grouped content sits on a panel surface distinct from the dialog background. Groups are divided by hairline separators or spacing, not by boxes inside boxes. The panel has one elevation and one corner radius, both from tokens. |
