# FatTail Labs — Options Lab — Create / Edit Position Dialog — Spec v0.7
...
**Status:** **APPROVED — BUILD AUTHORITY.** Landed `Specs/` 2026-09-11 on Coach's stamp. DLG2 reopened.
...
| **DLG-HIG-5** | **One commit, one dismiss, in platform order.** Create shows Analyze and Cancel; Edit shows Update and Cancel. The commit is the **default button** — visually dominant, placed last in reading order, bound to Return. Cancel sits beside it and is bound to Escape. **No Done link, no third action.** |
...
| **DLG-HIG-9** | **Component surfaces and inner padding, per the prototype.** The dialog is not a flat sheet. The **legs table sits on its own filled surface**, visibly distinct from the dialog background, with a border, a corner radius and **its own inner padding** — it does not bleed into the page. Menu fields and the segmented controls sit on filled control surfaces. Groups are divided by hairline separators or spacing, never by boxes inside boxes. One elevation, one radius, all from tokens. **A dialog with no component shading fails this law.** |
| **DLG-HIG-10** | **Real margins, and a panel wide enough to hold them.** Content is inset **20** from all four edges; content never runs to the edge. The panel widens **770 → 820** so the margins add room rather than take it — content width **746 → 780**. Free-floating and draggable per DLG-LAYOUT-8. *(Coach, 2026-09-11.)* |
...
| **DLG-LAYOUT-1** | **Title** — "Create Position" or "Edit Position", centred, with a separator beneath it. |
...
| **DLG-LAYOUT-3** | **Direction row** — directly beneath the strategy field, one row carrying, left to right: the **payoff icon**, the **Buy / Sell segmented control**, and the **derived name** ("Buy Butterfly") as plain text. *(Coach, 2026-09-11.)* It is not a labelled row; it carries no field label. |
| **DLG-LAYOUT-4** | **Right (Call / Put)** rides in the direction row's band, as a segmented control, shown only for strategies in the existing `TEMPLATE_HAS_SIDE` set and hidden for the two-right structures *(§8 decision 5)*. It does **not** get its own labelled section row. |
...
| **DLG-LAYOUT-6** | **Leg row contents.** QTY: signed value with a stepper. STRIKE: value with a menu. TYPE: the right, as a control. EXPIRATION: the date with a menu. **DEBIT and POS appear on the leg 1 row only** — DEBIT carries the package basis, its stepper and the **padlock**; POS carries the package count, its stepper and the **quick-pick**. Both columns are empty on every other row. |
...
#### 5.3.1 In the code, not in the prototype

These controls exist in `PositionBuilder.tsx` and are **not drawn in the reference image**. They are
named here rather than silently placed. **Until Coach rules on each, DLG2 neither invents a section
for them nor deletes them** — they hold their current behaviour and are raised at the gate.

| Control | Status |
|---------|--------|
| **Centre** | Not in the prototype. The prototype sets strikes per leg directly. **Coach to rule:** keep it, and where — or drop it. |
| **Width** | Not in the prototype, same question, same ruling. |
| **Expiration (structure-level)** | Not in the prototype as a separate row; the prototype carries expiration per leg in the legs table. **Coach to rule.** |
...
| **AT-DLG-16** | Measured, not judged: panel width **820**, content inset **20** on all four sides, content width **780**. Every gap, margin and inset in the component is a value from the 8-point scale — a grep for off-grid spacing classes returns nothing. |
