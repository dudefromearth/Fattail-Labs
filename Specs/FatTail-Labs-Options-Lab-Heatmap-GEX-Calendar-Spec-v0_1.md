...
| **GEX Calendar** | This template. Member label pending **OD-GC4** (placeholder `GEX calendar`). |
...
```ts
{
  ...
  label: "GEX calendar",                  // OD-GC4 — Echo may rename; not a vendor name
  description:
...
Until then the registry may include the template behind the flag, but the member
switcher must not show it (LIM37 / E14 precedent: a switcher entry that renders
...
| Help | `server/help_reference/options-lab-heatmap-gex-cal.md` — **after** the view works, not before |
...
| **AT-GC6** | When the flag is on **and** ODs stamped: switcher includes **GEX calendar**. Frozen `gex` and `lim` still render as before (screenshot or byte check on **their** compute, not this chrome). |
...
| **OD-GC4** | **Member label** | `GEX calendar` vs Echo’s name? | Ship placeholder **GEX calendar**. Echo owns the picker string. Must not contain a vendor name. |
...
> **DL-xxx — Heatmap GEX Calendar template.** New Heatmap template `gex-cal`
> (`layout: "matrix-profile"`): strike × listed-expiration matrix of frozen `gex_v1`
> net GEX, companion signed profile, NET footer, cyan/magenta diverging, gold peak.
> Observation-only. Not true dealer GEX (HM12). Not ME `calendar`. Not a restyle of
> frozen `gex` or LIM. No vendor name. Replay stays Time Machine. Empty pack fails
> loud (no fake columns). Hash of Spec v0.1 recorded at GO. **OD-GC1…GC4** as stamped.
...
Add Heatmap template `gex-cal`: strike × expiration matrix of `gex_v1` net GEX
dollars, cyan/magenta diverging, gold peak, NET footer, companion signed profile of
the same strikes. Same Options Lab Heatmap switcher. Do not name the vendor. Do not
touch frozen GEX or LIM compute. Do not invent a second replay. If the pack is not
there, fail loud (AT-GC8) instead of drawing fake columns. Tests first.
