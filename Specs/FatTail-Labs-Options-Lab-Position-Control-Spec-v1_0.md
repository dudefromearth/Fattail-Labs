# FatTail Labs — Options Lab Position Control Spec v1.0
...
default is permitted.** Two items are deliberately delegated rather than decided here, and both name
their owner: the **tick band table** (PC-CHAIN-7 — written at build time, never invented) and the
**density-profile exemption** for Analyzer chrome (§5.2 — Echo rules before the layout work).
...
| **PC-VOCAB-5** | **Package fields and leg fields never share a column.** Row 1 carries **BASIS**, **POS** and the lock. Leg rows carry **MARK**. |

```
STRATEGY   SIDE  QTY  SYMBOL  EXPIRATION  STRIKE  TYPE   │ row 1: BASIS · POS · lock │
Butterfly  BUY    +3   SPX    9 SEP 26    7655    CALL
           SELL   −6   SPX    9 SEP 26    7675    CALL
           BUY    +3   SPX    9 SEP 26    7695    CALL
```
...
| Legs | Dates | Rights | Ratio | Side pattern | Additional | → |
|---|---|---|---|---|---|---|
| 1 | 1 | one | 1 | either | — | **Single** |
| 2 | 1 | one | 1-1 | opposite | different strikes | **Vertical** |
| 2 | 1 | both | 1-1 | same | same strike | **Straddle** |
| 2 | 1 | both | 1-1 | same | different strikes | **Strangle** |
| 2 | 2 | one | 1-1 | opposite | same strike | **Calendar** |
| 2 | 2 | one | 1-1 | opposite | different strikes | **Diagonal** |
| 3 | 1 | one | 1-2-1 | **body opposite the wings** | wings equal | **Butterfly** |
| 3 | 1 | one | 1-2-1 | **body opposite the wings** | wings unequal | **BWB** |
| 4 | 1 | one | 1-1-1-1 | **outers opposite the inners** | — | **Condor** |
| 4 | 1 | both | 1-1-1-1 | **wings opposite the body** | body strikes shared | **Iron Fly** |
| 4 | 1 | both | 1-1-1-1 | **wings opposite the body** | body strikes separated | **Iron Condor** |
| *anything else* | | | | | | **CUSTOM** |

**Side pattern is a required predicate, not a nicety.** Without it a 1-2-1 with all three legs long
would classify as a Butterfly, and a four-leg 1-1-1-1 with every leg long would classify as an Iron
Fly. Neither is that structure — a butterfly is a body opposite its wings — and naming them so would
break Litmus 3 by describing the position wrongly. Any ratio-matching shape whose side pattern does
not match the row is **CUSTOM**.
...
**Delegated, with an owner: Echo rules on whether Analyzer chrome is exempt from the density
profile, before the layout work begins.** Neither answer changes a law — PC-HIG-1 (no overlapping hit
rectangles) and the floors themselves hold either way. What Echo's ruling changes is whether the
shortfall closes with padding or requires a focus-revealed control overlay.
...
Broker OMS · order placement · per-leg Greeks · multi-card aggregate curves · Surface 3D changes ·
MSC imports · server-side multi-device book · historic-chain replay of closed trades · a second
position store · a global state store · a selectable "Custom" mode · undo of a Trade Log promotion.

---

*Position Control v1.0. Contract. No implementation before Coach's GO.*
