# FatTail Labs — Options Lab Time Machine Spec v0.6.1

**Status:** DRAFT v0.6.1 — **fold-leftover pass, no new rulings.** v0.6 folded Amendment A1 but left five places still describing the retired 1-minute source, and one acceptance test ahead of an open. Corrected here. Prior status: v0.6 — **Amendment A1 folded.** Wings are a property of the day, not the generation — the envelope does not carry the band. The chain is the past-day walk; the 1-minute OPF pull retires and the mini window downsamples from the chain. Calendar states, the seam, and the coarse pass carry measured numbers. **No member-facing change. Six blocking opens untouched.** Still the **combined** spec. One surface, one scrubber, one date control. **Not BUILD AUTHORITY** until Coach Phase 5.
**Type:** Product + technical spec — **Time Machine**
**Supersedes:** v0.6, v0.5.1 and its Amendment A1 (folded at v0.6), `FatTail-Labs-Options-Lab-Time-Machine-Spec-v0_4.md` (replay chain: v0.4, v0.3.1, v0.3, v0.2, v0.1.1, v0.1) **and** `FatTail-Labs-Options-Lab-Analyzer-Time-Machine-Spec-v0_1.md` at **v0.1.8** (the Day fractal). Both are folded here; neither survives as a separate document.
**Routes:** `/app/options-lab/analyzer` · `/app/options-lab/heatmap` · `/app/options-lab/surface`
**Filename:** `FatTail-Labs-Options-Lab-Time-Machine-Spec-v0_6_1.md`
...
| Blue/green glow vs silent promotion — **unresolved** | **§12.6** | §0-A.7 vs §0.37 |
...
| **ATM-A1** | **CONTRADICTS TMI-67 — see §12.5.** Coach §0-A.14: the book may be empty when the day loads; the member **adds the position afterwards**, then **creates the Algo alert**, "just like real life." Position Builder, Algo eligibility, and Demo ticks use the playhead. Creating Demo while Time Machine is on does not turn What-if on. **Not resolved here.** |
| **TMI-34 · ATM §7** | Also §12.5. The replay chain kept Algo Create Alert out of v1; the Day document points Demo Algo at the replay clock (ATM-K6, §0-A.11). |
...
Raise the scrubber
    ...
    → all writes disabled with named reasons (TMI-67, subject to §12.5)
...
|---|---|
| **Time Machine, any date** | **§12.6 — unresolved.** §0-A.7 and §0.15 give blue for a past day and green for today. §0.37 says promotion must not feel different. Two colours is a felt difference. |
...
| Blue inner glow Time Machine; red What-if | **IN for red; blue is §12.6** |
...
| Position added afterwards, then the alert created | **§12.5** — contradicts TMI-67 |
...
| No writes in replay | **IN** · TMI-67 · *vs ATM-A1, §12.5* |
...
| Silent promotion, no felt difference | **IN** · TMI-64 · *frame tension §12.6* |
...
| **AT-ATM-9** | Glow criterion is held pending §12.6. Red for What-if alone stands. |
| **AT-ATM-19** | Held pending §12.5 (alert creation under a playhead). |
...
| **AT-TM-C2** | Selecting a past date produces no banner, no confirm, no transition. A recorded screen comparison shows no difference other than the loaded range and, where §12.6 lands, the glow. |
...
| Existing glow tokens | **Viewport or panel? Read and record.** §12.6. |
...
The first four blocked Phase 2 before this merge. **§12.5 and §12.6 are new and are Coach-versus-Coach.**
...
15. **Does Basic/Enhanced survive, and does it govern today as well as past days?** §0-A.10 was said before the collapse — and before every date had a chain behind it. **Its premise is gone** (ATM-E4): GEX-off was an honesty rule; keeping it is a simpler-desk choice. Decide it as that, or retire it. Worth noticing why the law exists: GEX and Probability were forced off in Basic because a 1-minute underlier path carries no chain, so the overlay would be lying. Today's derivation **is** a chain, and so is any past day the corpus covers. If that is the real reason, Basic and Enhanced are a function of **what data is behind the date**, not a member mode — and the control in §12.18 may not be a control at all. Raised by Grok; the ruling is yours.
...
4. Phase 3 **Echo + Tango:** — note that the first Analyzer packet is a **layout move**, not a new widget: Strikes/in goes left of Autofit and the transport right of it (§6.1), against as-built that has them the other way.
   Then: the glow ruling and the three tells on one seat; fidelity-indicator copy; disabled-control reasons under TMI-67; Basic/Enhanced naming; no profit chrome.
