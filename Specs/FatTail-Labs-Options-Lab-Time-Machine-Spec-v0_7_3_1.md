# FatTail Labs — Options Lab Time Machine Spec v0.7.3

**Status:** **STAMP CANDIDATE v0.7.3 — no open contradicts a ruling anywhere in this file, preface included.** v0.7.2 corrected the body but left the note at the top still telling a builder the six were unresolved, plus four stale pointers in the change table, §14, the bench notes, and the parent amendments. All corrected. **No new rulings, no law added or retired.**
**Type:** Product + technical spec — **Time Machine**
**Supersedes:** v0.7.2, v0.7.1,
**Routes:** `/app/options-lab/analyzer` · `/app/options-lab/heatmap` · `/app/options-lab/surface`
**Filename:** `FatTail-Labs-Options-Lab-Time-Machine-Spec-v0_7_3.md`
...
3. Phase 2 **India:** implement the browser cache as ruled — TMI-4 and TMI-79, one day held, discard before accepting the next, **no `server/` film module** (do not reopen 12.1); the corpus read path and the optional TPO; the mini-window downsample as a derivation of the same generations, not a second series; one playhead owner; no second socket. **Read and record the measured capture cadence** from the archive stats pass (§12.4) — a fact to carry, not a decision to make.
