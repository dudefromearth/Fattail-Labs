# FatTail Labs — Options Lab Time Machine Spec v0.7.4

**Status:** **BUILD AUTHORITY v0.7.4 — one wording fix, no ruling changed.** TMI-79 is **two slots**: today always capturing in the browser; at most one archive day. Capture-always-on and one-archive-day-open never fought. The v0.7.3 phrase "one day is held at a time" was the advisor's wording and would have paused today's film while a past day was open — that is not Coach. **No new rulings, no law added or retired** at GO. Coach GO 2026-08-27. **DL-598.** **2026-08-29:** **TMI-97 DRAFT** (not BUILD) seated on §6.2 — **usability law directed to the scrubber window** (landmarks). Sibling DRAFT holds the four tests. Clock stays. **DL-646.**
**Type:** Product + technical spec — **Time Machine**
**Supersedes:** v0.7.3, v0.7.2, v0.7.1,
**Routes:** `/app/options-lab/analyzer` · `/app/options-lab/heatmap` · `/app/options-lab/surface`
**Filename:** `FatTail-Labs-Options-Lab-Time-Machine-Spec-v0_7_4.md`
...
**Items §0.24–47 came from a voice session on a walk** and are quoted as the transcript recorded them, including its errors, marked `[sic]` with the reading in brackets. Three turned on a word the transcript may have mangled — §0.28 "SSIS", §0.29 "rights", §0.35/38 "pie machine"/"iMachine". **Coach confirmed all three readings on 2026-08-27 (§0.49):** the decay curve is his and not a member setting, **writes** are forbidden, and both machine words are Time Machine. No law rests on an unconfirmed word.

**The two source documents contradicted each other in four places, and every one is ruled.** They were §12.5 through §12.10 — the Algo alert against no-writes, the glow against silent promotion, the source label against density invisibility, and the entry position by derivation. **§12 is the record of those answers, not a queue.** Nothing in this file is waiting on Coach. Two of those four were the real collisions — the Algo alert and the glow. Entry position by derivation was a designed difference rather than a contradiction, and **§12.10 remains a small label question, not a fight**. The smaller items still listed in §12 as open never blocked and do not gate Phase 5.
...
| Algo alert creation vs no-writes — *was* the unresolved contradiction at v0.5; **ruled at §0.54 as rehearsal objects** (TMI-80) | **§12.5 — closed** | §0-A.14 · §0.29 |
...
| **TMI-79** (§0.48 · §0.52) | **Two slots, and they are different things.**

**Today** is always capturing (TMI-65). It is never paused and never discarded because a past day was opened; it is discarded only when OPF's trading date changes (TMI-73). Loading an archive day does not put a hole in today's session.

**Archive** holds **at most one past day**. Selecting a different past date discards the one held before accepting the new one. Returning to live, or Reset, drops the archive slot — **today stays**. Returning to a past day just left re-downloads it.

The memory ceiling is therefore *today's decayed session, plus at most one archive day*, not one day in total. *(v0.7.3 said "one day is held at a time" without saying which kind, which read as forbidding continuous capture while a past day was open. That was the advisor's wording, not a Coach ruling; §0.48 and §0.52 never conflicted.)* |
...
| **TMI-97** DRAFT (not BUILD) | **Usability of the scrubber window.** Landmarks so the member can locate themselves **in the day**, not only in the tick array. Clock stays. Full law in §6.2 and the sibling DRAFT. Not calendar / AV-*. |
...
| **TMI-68** | **The ladder.** Native interval **2 seconds**; the oldest end thins to **at most about 10 seconds** (§0.26), roughly **halving** the footprint (§0.27). Boundaries are config, fail-loud. *§12.4 answered: DL-400 [3,5] reversed (DL-606). 2 s is Massive's floor; size the ladder on the slow end (~5 s p95).* |
...
**TMI-97 DRAFT (not BUILD).** **A usability law directed to this window** — not an availability law, not the calendar. While the playhead is up, the window shows **where in the day we are** (ATM-H2) with landmarks: **start of day, midday, end of day**, plus a **small pre-market area** and a **small extended area**, distinguished from regular hours. **The clock stays.** Landmarks are position; the HUD clock is precision; do not replace one with the other. Bounds come from **OPF**, never the browser clock. The regions are what **this hold** actually contains — if the tap started at 11:00, do not draw empty pre-market. Orientation, not chrome; not a P&L colour (same rule as the watermark, TMI-25). An early close is that session’s end of day, not assumed 16:00. The window agrees with the hold line (TMI-89). Full text and four tests (SL-1…SL-4): `Specs/DRAFT-FatTail-Labs-Options-Lab-Time-Machine-Scrubber-Landmarks-Spec-v0_1.md`. No product code until Coach stamps a plan.
...
| Landmarks on that window (start / midday / end; pre-market / extended areas) | **DRAFT** · TMI-97 · usability of this window, not availability · clock stays |
...
| **AT-TM-C8** | No `server/` replay cache module exists; selecting a second **past** day discards the first before accepting it; **today's capture continues throughout and is not discarded**; returning to live drops the archive slot and leaves today intact; memory does not grow with days *visited*. |
| **AT-TM-C11** | Resident browser bytes are **measured and recorded**, not assumed: a full simulated session (native then decayed) alone, and again with one full-fidelity past day loaded on top. §0.48's 10–20 MB is transfer, not resident. The recorded figure is what the decay ladder is re-tuned against. |
...
| Redis generation cache (Heatmap Templates §8) | **This is the Heatmap generation cache and it stays exactly that.** It is not Time Machine film and it is not a replay store. §12.1 is ruled — the replay cache is browser-side and no server film exists — so there is no relationship to reconcile and **nothing here is extended, wrapped, or rebuilt**. India reads and records what it is; India does not build a second store. |
...
| # | Asked | Ruled |
|---|---|---|
| **12.1** | Where does today's cache live — server Redis (§0.18) or browser (§0.45)? | **Browser-side** (§0.48). Two slots: today always capturing, plus at most one archive day (TMI-79). **No `server/` film module is built.** TMI-4, TMI-79; TMI-5, TMI-56, TMI-58–63 retire. |
...
| **12.4** | 2 s native (§0.26) against DL-400's 4 s with a fail-loud band of [3,5]? | **Answered by measurement. DL-400 [3, 5] is reversed (DL-606).** AT-SOAR-45 Friday 2026-08-28 SPX: median 2.24 s, p95 4.53 s, 0 gaps. **2 s is Massive's floor**; the spread is vendor latency, not drift. Fail-loud band **[2, 5]** (DL-428 as-built, now proven). Ladder is sized on the **slow end** (p95 ≈ 5 s), not on a 3 s floor that excludes the fastest snaps. Collection outranks reads: full-pool load of 2026-08-25 SPX left median/p95 slightly faster, still 0 gaps. |
...
## 13. Parent amendments (same packet as the stamp)

Parents gain these one-line pointers **in the same packet as the build**, not later:
...
- **Trade Log §4.4** → replay is not a write source and `entry_source` gains no fourth value. **Ruled (§0.54):** rehearsal objects never reach the log at all, so there is nothing for it to record and nothing conditional about it.
- **AZ-ALGO** → Demo on the replay clock; **creation under a playhead is allowed as a rehearsal object** (§0.54, TMI-80) — badged, never stored, never notifying, disposed on Reset with an announcement.
- **Arch 28** → replay read path named; one socket, no client Massive.
- **One Source v0.4 (2026-08-29)** → derivation is StudioOne for every date including today. `TODAY_LIVE` as a fetch refusal is retired. SO-AR §3 rewritten.
...
3. Phase 2 **India:** implement the browser cache as ruled — TMI-4 and TMI-79, **two slots**: today capturing continuously, plus at most one archive day discarded before the next is accepted and dropped on return to live; **no `server/` film module** (do not reopen 12.1); the corpus read path and the optional TPO; the mini-window downsample as a derivation of the same generations, not a second series; one playhead owner; no second socket. **Read and record the measured capture cadence** from the archive stats pass (§12.4) — a fact to carry, not a decision to make.
...
**India:** one surface, one playhead owner per tab, hosts bind and never fork. **The cache is browser-side and that is ruled (§0.48)** — there is exactly one source of record for a sample and no server film to reconcile against. **The two-truths risk is closed** (§12.14): the chain is the walk and the mini window is a downsample of the same generations, so a past day has one series and one spot.

**Mike:** no member identity in whatever holds the cache; terms confirmed before stored snapshots are served back.

**Echo:** the tells are settled and the work is seating them. **What-if keeps the red glow; Time Machine has none** (§0.57). Time Machine speaks in marks — the REPLAY watermark on the canvas and the counter-clockwise badge on rehearsal cards, one language at two scales. The live job: size the watermark so it cannot read as a signal on a tent and never obscures a strike, a leg, a price, or the sim-spot indicator; keep it out of every P&L colour; give it a coherent seat beside What-if's red, which may show at the same time; and make it static under reduced motion, never a pulse. Three hosts, three treatments, one meaning.
...
| **v0.7.4** | 2026-08-27 | **BUILD AUTHORITY (Coach GO · DL-598).** One wording fix, no ruling changed. **TMI-79** is two slots: today always capturing; at most one archive day. §12.1, §14 Phase 2, AT-TM-C8 restated. **AT-TM-C11** measures resident bytes; 10–20 MB is transfer. Status header paste wreck from v0.7.3 tidied in-repo (A1 — no version bump). **2026-08-29 (no version bump, not BUILD):** **TMI-97 DRAFT** seated on §6.2 — usability law directed to the scrubber window (landmarks). Sibling DRAFT holds the four tests. Clock stays. |
| **v0.7.3** | 2026-08-27 | **Preface and pointers, no new rulings.** v0.7.2 fixed the body but left **the first thing a builder reads** still saying the garbled words await confirmation and that four Coach-versus-Coach contradictions are unresolved — both closed since §0.49 and §0.54. The preface now states the confirmations and says plainly that §12 is a record. The **v0.5 change table's Algo row** no longer reads as a live fight. **§14 Phase 2 India** implements the browser cache rather than reopening 12.1, and carries the measured cadence as a fact to read. **Bench notes** for India and Echo no longer wait on decided opens; Echo's now describes the watermark's seat, sizing, colour prohibition, and reduced-motion behaviour. **Parent amendments** corrected: the Heatmap Cache line points at §12.9 rather than the glow ruling, Trade Log is unconditional, and AZ-ALGO names rehearsal creation. |
