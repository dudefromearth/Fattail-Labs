...
**Version:** v0.3 — **STAMPED 2026-09-14** (D-B1-1 resolved by Coach). Authority for B1 build planning. **Build start remains gated on PPL3 + PPL4 PASS (§0).**
**Date:** 2026-09-14
**Supersedes:** `FatTail-Labs-Practice-Position-Lifecycle-B1-Spec-v0_2.md` (baseline; leave on disk, do not plan or build against). v0_1 is an earlier baseline.
**Target location:** `Specs/FatTail-Labs-Practice-Position-Lifecycle-B1-Spec-v0_3.md` in the FatTail Labs repo on **StudioTwo**
...
| # | Change |
|---|---|
| 1 | **D-B1-1 STAMPED by Coach (2026-09-14): immediate current-structure switch; badge informational, not a gate.** Moved from recommended to stamped (§5) |
| 2 | AT-B1-7 revision flag removed — the test is written to the stamped call |
| 3 | No open decisions remain anywhere in this document |
| 4 | Status raised to planning authority; §0 clarified — planning may proceed now, build start stays gated |

(The v0.1→v0.2 change table lives in `…-v0_2.md`.)
...
Out of scope (non-goals): order execution/automation, lot-picker close selection (OD-24 stands), trash-bin/undo delete, cross-account lineages (D-B1-2), **lineage forking** (D-B1-6), **lineage-level delete** (delete remains per-position, successor-first, each under OD-19's kit dialog), any change to the FIFO matcher.
...
**UC-T1 — Roll the position.** User ends the current strikes and opens new ones as one intentional act. **Transform consumes the entire remaining position (D-B1-6).** To roll only part of a runner: Scale Out the part that ends, then Roll the remainder — composed from existing primitives, no forked lineages.
*Implementation:* **Roll** wizard in the drawer: composes closing fills + opening fills + one transformation event in a single gesture. Manual path (per the manual-and-wizard rule): the same fills entered separately, then explicitly linked via **Link as roll**. Both paths emit the identical event record — this is the invariant that keeps the feature from forking.
...
| Roll wizard | UC-T1 | Drawer lifecycle group |
| Link as roll (manual) | UC-T1 | Drawer |
...
The drawer's lifecycle group (the framed, labeled cluster per the 2026-09-14 UI decision) is the single home for Scale Out, Close, Roll, Unlink, and Delete — state-aware, consistent look and feel.
...
|---|---|---|
| **Roll** | the act (wizard, manual link) | "transform", "transformation event" |
...
- **AT-B1-4** Manual close + open, then Link as roll: event semantics identical to AT-B1-3.
...
- **AT-B1-7** Leg out one wing of a fly: current-structure reads vertical immediately; "now a vertical" badge shown; Analyzer renders the vertical (D-B1-1).
...
**PPL5** transformation + revocation events, derived lineage, AT-B1-3/4/8/10/11 · **PPL6** drawer surfaces (Scale Out, N-close stack, Roll wizard, Unlink, lineage view, §4a copy) · **PPL7** re-recognition + current-structure + Analyzer consumption · **PPL8** import roll recognition (proposal-first) + journal/retrospective grouping.
