# FatTail Labs — Session Option Volume Profile Spec v0.3

**Status:** **DRAFT — BLOCKED.** Not build authority, and the `svp_v1` freeze is **stopped**.

> **P-SV1 is REOPENED (2026-09-01).** P-SV8 measured the archive against the vendor at the
> **matching** window and found errors on **both sides of zero** — −2092 (−38.30 %) to +4665
> (+22.84 %), median 0, with all three ATM calls clean (−12 / −0.04 %) and every non-zero
> 10-strike-out name off. A one-sided window effect cannot produce that shape.
>
> Two explanations have been proposed and both are dead: **convention** (killed — the vendor agrees
> with itself on all 15) and **clock** (killed — two-sided error at one window). §2.3's decomposition
> stands as a *description of the 16:15 gap*, not as an account of the per-strike residual.
>
> **Per-strike volume is what this product is.** Until P-SV9 characterises the error, Hotel cannot
> sign, the tolerance cannot freeze, and no artifact should be written. Escalated: the shipped
> Heatmap ladder renders a per-strike volume column from the same generation path.
**Date:** 2026-09-01
**Current revision:** **v0.3.1** (filename stays `…-v0_3.md`; the revision field is authority)
**Supersedes:** [v0.2](./FatTail-Labs-Session-Option-Volume-Profile-Spec-v0_2.md) (content rev v0.2.2) — probe evidence returned and folded
**Canonical filename:** `Specs/FatTail-Labs-Session-Option-Volume-Profile-Spec-v0_3.md`
**Type:** Product + data-plane Spec — **session-cumulative option contract volume by strike**
**Short name:** **SVP** / **Session Volume Profile**

**Content hash (v0.3.1):** recompute at Coach GO.

**Why v0.3 exists:** P-SV1, P-SV4 and P-SV5 returned. The measurement claim **survived**; four
structural assumptions in v0.2 did **not**. v0.2.2's header said the next artifact was the evidence
packet, not another draft — this revision is that packet folded, and nothing more.

**Evidence:** `docs/evidence/session-volume-profile/P-SV1.md`, `P-SV4.md`, `P-SV5-day-roll.md`
(run on StudioOne, read-only, 2026-08-31).

---

## 0.3 As-built honesty — what the archive actually is

| Item | Measured | Effect on this Spec |
|------|----------|---------------------|
| **Volume field** | `generation.rows[].volume` (integer) — **not** `day.volume`. The archive stores **Labs generations**, not raw vendor snapshots | SV1 provenance reworded; HM22 names the real path |
| **Cumulative?** | **Yes.** 08-28 `O:SPXW260828C07735000`: 4877 → 24748 → 56794 → 77002 across the session | **§2 stands.** This was the gate and it passed |
| **Day roll** | Real, and **after** the open: first drop just after **09:31 ET** on five weekdays, never at 09:30:00. 59 of 62 contracts drop in one 11-second pair; on 08-20 the roll spread over **~7 s** in two groups. Post-roll values are small positives (1–451), **never 0** | **SV62** — new law. v0.2's SV3 clamp would have silently eaten this |
| **Reconcile** | 15 fixtures: all deltas ≥ 0, 0 … +4667 (0.00–22.85 %), no invert. Vendor self-consistent (aggs = open-close, 15/15) and stable on re-query. **08-19 ATM call: archive 29005 @15:59:55 · vendor 29017 · archive 30451 @16:04:59** | **SV63** — the gap is **post-16:00 volume**. At the matching window the residual is **−12 (−0.04 %)**. A decomposition, not a tolerance |
| **Session window** | Volume still accruing after 16:00 (77395 → 77427) | **SV64** — SPX options do not close at equity RTH. v0.2's "RTH equities/ETFs" would truncate real volume |
| **Archive scope** | **Band, not full chain**, and the band **varies**: `wings` is **15 or 25**, changing *inside one morning* (08-28 roll pair is 15 → 25). 51 strikes 7610–7860 vs spot 7736 | **SV66** — scope is per-snapshot, and strikes enter and leave mid-session |
| **Expirations** | **0DTE only.** Every SPX snap is same-day expiry for its session date | **SV65** — the historical door serves 0DTE sessions only |
| **Horizon** | `day=2026-08-14` … `2026-08-31`, **twelve trading days** (08-15/16 are Sat/Sun, not gaps) | Coach's "about two weeks" confirmed as archive depth |
| **Cadence** | Median **~2.4 s**; max named RTH gap **54.7 s** | 1-minute bucket clock (OD-SV3) is a lawful downsample |
| **Degraded day** | **08-21**: 459 RTH gaps totalling **7862 s** — ~2.2 h of a 6.5 h session | One of twelve days is not `complete`. **OD-SV36** |
| **Greeks** | `spot`, `open_interest`, `gamma` all present, **0/102 null** at 10:00 ET. **No per-row timestamp** | SV33 / SV42 / SV61 fully supported by the archive as it stands |
| **Truncation** | `massive_page_limit=250`, snaps 60–102 rows, **no `next_url`** | No truncation on the band path. A future full-scope poller **will** page, and the client has no pagination — a build risk, not a data defect |
| **Mount** | Env has `raw-primary:/Volumes/FatTail2TB` on StudioOne. **`market_storage_mount` table does not exist** in Labs MySQL | OD-SV5 answered; SV10's assumption corrected |
| **Gold volume** | `du` **stalled 20–25 s** | The Collector Hardening warning is live, not theoretical |
...
**SV62 — the day roll is a session boundary, not a revision (P-SV5).** Until roughly **09:31 ET**
every contract carries its **previous session's** cumulative total. The roll then drops all
contracts to today's small accumulating count — 59 of 62 in one 11-second pair on 08-28, never to
zero, and on 08-20 spread across **~7 seconds** in two groups.

| Rule | Law |
|------|-----|
| **The clamp must not run across the roll.** SV3 would compute `max(0, 1886 − 4433) = 0` and report that nothing traded through the open. Detecting the roll **precedes** applying SV3 (**AT-SV57**) |
| **Pre-roll reads are prior-session data.** They are discarded, not accumulated, and never contribute to `V_end` or to any bucket |
| **The roll is a window, not an instant.** Detection tolerates a spread of at least 15 seconds and is evaluated **per contract**, because contracts in the same session roll seconds apart |
| **The first post-roll value is unattributed.** It is already a positive count (1–451 observed) — volume that traded between the open and the roll and can never be placed in a bucket. It goes to `unattributed` under its own reason code, distinct from a collection gap (**AT-SV58**) |
| **Never inferred from the clock alone.** ~09:31 is what five days showed, not a contract. Detection is by the monotonic break; the clock is a sanity bound, and a roll outside the expected window is recorded, not ignored |
| **Not testable against the prior day on this archive** | Every SPX snap is 0DTE for its own session date, so no contract exists on day N and N+1 (SV65). That the pre-roll value *is* the prior session's total is the best-supported reading, not a proven one — **recorded as an inference** |

| Rule | Law |
|------|-----|
| Non-monotonic read **after** the roll (vendor revision, late print, correction) | **SV3** — clamp to 0, increment `revision_count`. **Never a negative bar** |
...
### 2.3 The reconciliation did not match — and that is the finding (P-SV1 §7)

`O:SPXW260828C07735000`, session 2026-08-28:

| Source | Value | vs vendor daily |
|--------|-------|-----------------|
| Vendor daily aggregate (aggs and open-close) | **76997** | — |
| Archive 15:55 ET | 77002 | **+5** |
| Archive last RTH 15:59:50 | 77395 | **+398** |
| Archive last snap 19:59:59 | **77427** | **+430** |

**SV63 — the difference is a clock, not a convention (P-SV6).**

**P-SV6 (15 reconciles, 3 clean sessions) explained it.** The vendor's daily window ends at
**16:00**; the option keeps trading past the equity close; the archive counts that. The delta *is*
post-16:00 volume.

| Evidence | |
|----------|--|
| 08-19 ATM call: archive **29005** @15:59:55 · vendor daily **29017** · archive **30451** @16:04:59, then flat | The vendor figure sits **between** two archive clocks — the window boundary is visible in the data |
| At the **matching** window: 29005 vs 29017 = **−12 (−0.04 %)**, archive slightly **below** | Five unsampled seconds before the bell. This is the real reconciliation, and it is tight |
| All 15 deltas ≥ 0, **no invert**; range 0 … +4667 (0.00 % … 22.85 %) | Post-close volume cannot be negative |
| Scatter is **not** constant %, and **not** worst on thin wings (0, +2, 0) | Closing-rotation flow varies by contract; the 22.85 % outlier is a put 10 strikes out — where closing and rolling flow lands |
| 08-25 far-wing put leaves the band at 16:05: last value 54, vendor 54, **delta 0** | A contract with no post-close trading matches exactly |
| Vendor **agrees with itself**: aggs volume = open-close volume on all 15 | We are not comparing against a figure that is multi-valued |
| Vendor daily is **stable**: 08-28 re-queried, still 76997 | No T+1 revision to model |

**So `svp_v1` does not freeze an unexplained tolerance.** It freezes a **decomposition**:

```
archive(session close)  −  vendor daily  =  volume traded after 16:00
archive(16:00)          −  vendor daily  ≈  0     (unsampled seconds only)
```

| Rule | |
|------|--|
| **The artifact reports the full session** — through the instrument's close, including post-16:00 volume. That is the honest total for an option that is still trading |
| **It also carries `volume_at_1600`**, so the panel can answer "why doesn't this match the number I saw elsewhere" without a member having to ask. Two windows, both labelled, neither adjusted toward the other |
| **`vendor_daily_delta`, when a vendor figure is available, is labelled as post-16:00 volume** — a decomposition, not an error bar (**AT-SV61**) |
| **The tolerance that freezes into `svp_v1`** is the **16:00-to-16:00** residual, not the whole-session gap. One fixture gives **−12 / −0.04 %**; **P-SV8** widens it to the same 15 before the freeze |
| **Never smoothed.** SVP does not adjust its own value toward the vendor's, in either direction |

### 2.4 Session window — SPX options do not close at 16:00

**SV64 — the session is the instrument's, not the equity market's.** v0.2 §3 inherited VP's "RTH
equities/ETFs". The evidence shows volume still accruing after the equity close (77395 → 77427),
which is exactly what an SPX option trading to **16:15 ET** should do. Truncating at 16:00 would
discard real volume at the most-watched moment of the day and call the remainder a session total.

| Rule | |
|------|--|
| The session window is a property of the **instrument**, from config, per product (**OD-SV35**) |
| The window used is recorded in coverage and stated on the surface |
| Post-window snapshots are retained but excluded from `V_end`, and their delta is reported as `after_session_volume` rather than dropped silently |

### 2.5 Coverage object (carried on every artifact and every API response)
...
coverage = {
  ...
  wings_seen: [15, 25],                 // SV66 — band varies WITHIN a session
  strike_first_seen: { [strike]: t },   // SV66 — when each strike entered the band
  roll_detected_at, roll_spread_seconds,// SV62
  unattributed_pre_roll,                // SV62 — distinct from collection gaps
  session_window,                       // SV64 — the instrument's, not equity RTH
  after_session_volume,                 // SV64
  vendor_daily_delta,                   // SV63 — when a vendor figure is available
...
**SV65 — the archive is 0DTE only, and the historical door says so.** Every SPX snapshot in the
twelve-day archive is same-day expiry for its own session date. Back-select therefore serves
**0DTE sessions**, not arbitrary expirations. Dated expirations become available only from the day
a wider capture runs — the history clock for them starts then, and the panel must not imply
otherwise. *(This is also why SV62's "pre-roll equals yesterday's total" stays an inference: no
contract exists on two consecutive days to prove it.)*

**SV66 — scope is per-snapshot, and strikes come and go mid-session.** `wings` is **15 or 25** and
changes *inside a single morning* (the 08-28 roll pair is 15 → 25). So:

| Rule | |
|------|--|
| `strike_scope` is recorded **per snapshot**, and the set of bands seen in a session is carried in coverage (`wings_seen`) — never one constant per day |
| Every strike carries **`first_seen_at`**. A strike that entered the band at 11:00 has no observations before it |
| **Session totals survive this**; sliced modes do not. Because the field is cumulative, a late-entering strike still carries its full session volume on arrival. But `window` and time-sliced modes are **invalid for that strike before `first_seen_at`**, and render invalid rather than zero (**AT-SV59**) |
| The profile must not present a late-entering strike's first observation as a burst of trading at that moment. It is a first sighting, not an event |
...
**SV67 — consume the archive that exists; do not build a second one.** v0.2 §4 specified a new
tree under `LABS_MARKET_DATA_MOUNTS`. The evidence says an archive already exists and is already
being read by a governed surface:

| As-built | |
|----------|--|
| Store | `/Volumes/FatTail2TB/fattail-market-data/ssr/live_capture/day=YYYY-MM-DD/` (DL-597), written by `ssr_live_capture` on **StudioOne** under launchd, Mon–Fri |
| Reader | **StudioOne Archive Read API** — `/api/coverage`, `/api/index`, `/api/fetch`, `/api/stats`, re-served by Labs at `/api/me/options-lab/archive/*` (Spec v0.8, board `p-studioone-archive-read`) |
| Mount registration | env `raw-primary:/Volumes/FatTail2TB` on StudioOne. **`market_storage_mount` does not exist** in Labs MySQL — VP17's catalog was never built |

**Therefore:** the SVP artifact is **derived from that archive through that read API**, and the
SVP auxiliary plane (HM21) is that API. Building a parallel tree and a parallel reader for data
that already has both is the failure the doctrine names by name. v0.2's §4 layout is **withdrawn**;
what follows describes the **derived** artifact only.

**Operational constraint, measured:** `du` on the gold volume **stalled 20–25 s**. Any SVP reader
inherits the Collector Hardening rule — never on the live write or dash HTTP path.

**Boundary for India:** SVP overlaps `p-studioone-archive-read`. SVP owns the **session artifact
and its measurement law**; that board owns the **archive read surface**. Where they disagree about
reading the archive, the Archive Read API Spec wins.

Inherits VP v0.4 storage law for the derived artifact only.
...
(§2.5) and would have arrived as tooltips — which is how honest metadata becomes decoration. They
...
| **SV18 — coverage always** | Every 200 carries the §2.5 coverage object. A response without it is malformed |
...
(G15). **The AT set was closed at AT-SV56** and is reopened once, here, for AT-SV57–62 — these test laws that did not exist until the evidence returned, not extra coverage of old ones. It closes again at AT-SV62 — further coverage belongs in seeds and gate reports, not in
more spec rows.
...
| **AT-SV57** | Roll fixture (08-28 09:31:07 → 09:31:18): the boundary is detected **before** SV3 clamps; pre-roll reads are discarded, not accumulated; no bucket reports zero across the roll (SV62) |
| **AT-SV58** | First post-roll value lands in `unattributed` under a **pre-roll** reason code distinct from a collection gap, and `Σ Δ + unattributed = V_end` still holds (SV62, SV5) |
| **AT-SV59** | A strike entering the band mid-session renders session totals normally, but `window`/sliced modes are **invalid** before its `first_seen_at` — never zero, and never a burst at first sighting (SV66) |
| **AT-SV60** | A session whose window is the instrument's (to 16:15 for SPXW) includes post-16:00 volume in `V_end`; a post-window delta is reported as `after_session_volume`, not dropped (SV64) |
| **AT-SV61** | Payload carries `volume_at_1600` and `vendor_daily_delta`; the delta is labelled **post-16:00 volume**, not an error bar; SVP never adjusts its own value toward the vendor's (SV63) |
| **AT-SV63** | Reconciliation fixture: `archive(16:00) − vendor daily` is within the frozen residual, **and** `archive(close) − archive(16:00)` equals the reported post-16:00 volume — the decomposition closes exactly (SV63) |
| **AT-SV62** | 08-21 fixture (459 gaps, 7862 s) → session marked **not complete**, window modes disabled with the reason on the strip (SV57, OD-SV6) |
...
| **P-SV4** | **CLOSED 2026-08-31** — see §0.3. Twelve trading days, band(15/25), 0DTE only, mount env-only. Original text: what is actually on the archive mount today — path, mount role, symbols, expirations, earliest and latest complete session, whether generations are band-clipped or full | Converts "about two weeks" from assertion into recorded evidence, and settles SV8 backfill scope |

| **P-SV6** | **CLOSED 2026-08-31.** 15 reconciles, 3 clean sessions (08-19, 08-20, 08-25; 08-21 excluded). No sign inversion; vendor self-consistent; vendor stable on re-query. **Result: the gap is post-16:00 volume** (§2.3) |
| **P-SV8** | **NEW, the only item blocking the freeze.** Two parts: (a) for the same 15 fixtures, `archive at last read ≤16:00:00` vs vendor daily — the **apples-to-apples** residual, expected small and slightly negative; (b) for 3–5 contracts across 2 days, archive values at 16:00 / 16:05 / 16:10 / 16:15 / 16:20 — does everything go flat after ~16:05, or did that one contract simply stop trading? Answers OD-SV35 empirically |
| **P-SV7** | **NEW** — confirm the roll on a **dated** (non-0DTE) expiration once a wider capture exists, so SV62's "pre-roll is yesterday" stops being an inference |

Store under `docs/evidence/session-volume-profile/`. Returned: `P-SV1.md`, `P-SV4.md`,
`P-SV5-day-roll.md`.
...
| **OD-SV34** | **RESOLVED by P-SV6** — the divergence is a window, not a convention. Remaining question: does the panel show `volume_at_1600` beside the session total, or only on request? | **Show it on the coverage strip when a vendor figure exists.** It pre-empts the one question this surface will otherwise generate every day |
| **OD-SV35** | Session window per instrument (SV64) | **Do not hardcode 16:15.** The 08-19 fixture goes flat after **16:04:59**, and one contract on one day cannot tell us whether trading stopped or the field stopped updating. Config a per-product window, and record the **last observed change** per day beside it (**P-SV8**) |
| **OD-SV36** | Degraded days (08-21: 7862 s of gaps) | **Serve it, marked not complete**, window modes off. Hiding a bad day teaches less than showing why it is bad |
...
| **SVP2** | **Extend** scope forward — the tap already runs at ~2.4 s; this widens the band and adds dated expirations. **The client has no pagination and a full chain will page** (P-SV4) — that is this phase's real work | AT-SV5, P-SV2 evidence |
...
| **v0.3.1** | 2026-09-01 | **P-SV6 folded — SV63 rewritten.** The vendor divergence is a **window**, not a convention: the vendor's daily aggregate ends at 16:00, the option trades past it, the archive counts it. Proven by the 08-19 fixture where the vendor figure falls between two archive clocks, and by the matching-window residual of **−12 (−0.04 %)**. `svp_v1` now freezes a **decomposition** rather than an unexplained tolerance; artifact carries `volume_at_1600`. OD-SV34 resolved; OD-SV35 no longer hardcodes 16:15; AT-SV63 added; **P-SV8** opened as the last item before the freeze |
| **v0.3** | 2026-09-01 | **Probe evidence folded (P-SV1 / P-SV4 / P-SV5).** §0.3 as-built honesty added. Measurement claim **survived**. New: **SV62** day roll is a session boundary and the clamp must not cross it; **SV63** archive exceeds vendor daily by +398 (+0.52 %) one-way — frozen as tolerance, disclosed not smoothed; **SV64** session window is the instrument's, not equity RTH; **SV65** archive is 0DTE only; **SV66** band varies intraday, per-strike `first_seen_at`; **SV67** consume the existing StudioOne archive and its read API, v0.2 §4 layout **withdrawn**. Coverage extended. AT-SV57–62 (set reopened once, then closed). OD-SV34–36. P-SV6/P-SV7 opened |
| **v0.2.2** | 2026-09-01 | **Errata pass on advisor sign-off — no new law.** §5 primary path names the **poller's** artifact explicitly; §2.5 coverage gains `live_tail_scope`, `last_poller_at`, `wing_clock_seconds` so SV54b has a payload; SV57's taught question scoped to 4a (volume + its terms) with the flow-vs-gamma synthesis moved to §A.1; "rail visible" removed from the 4a Comfort description; SVP4a exit line scopes trail to Comfort / edge tick; document-control banner deduplicated; §A.2 states the Compact row-ownership rule for when 4b opens. **Spec revision stops here — next artifact is the SVP0 evidence packet** |
