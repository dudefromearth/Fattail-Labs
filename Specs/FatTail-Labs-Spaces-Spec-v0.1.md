...
**Version:** v0.1 (provisional — see Open Question 1)
...
**A claim carries its own liveness.** If the holder goes quiet, the claim lapses and the work becomes available again. Nobody has to detect the failure. (See Open Question 2 — the earlier phrasing described this liveness as a lease, and §7 has since replaced leases with atrophy. The reconciliation needs Coach's word.)
...
1. **File name and version.** This document carries a provisional name and version. Coach names both.
2. **Claim liveness vs. atrophy.** §6 says a claim carries its own liveness; the earlier session described that liveness as a lease, and §7 has since replaced leases with atrophy. Is a claim's liveness attention-tracked in the same way as everything else, or is a claim the one place a bounded lease survives?
3. **Transaction scope.** Does an atomic claim span multiple spaces, or only things within one space?
...
- No implementation design, storage model, transport, or API surface. None was directed.
