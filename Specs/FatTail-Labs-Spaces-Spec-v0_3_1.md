...
**Version:** v0.3.1 (filename and version still Coach's to name — see §16.1)
...
**Positions written in §18, awaiting Coach's word — these are what a contract needs:** attention (18.2), transaction scope (18.3), the handle (18.4), authority over a space (18.6), and how the pair is law (18.7). Each has a position; none is a blank. **A Factory card cannot atrophy honestly until 18.2 is stamped**, so that one is the near-term gate.
...
4. **Whether attend is its own operation** or a claim is the only attention that counts. This follows from how 18.2 is answered and changes the op set an instance implements. *Position: its own operation — a thing can be attended without being claimed, and if only claiming counts as attention then reading a space is invisible to it, which makes atrophy blind to exactly the entity that is paying attention without taking.*
