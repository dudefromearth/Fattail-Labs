...
| **Find and Badge** | Campaigns main page `/app/practice/campaign#find-badge` — book-wide AutoFilter / campaign badge. **Campaign-side law:** [Member Campaign Spec §9a](./FatTail-Labs-Member-Campaign-Spec-v1_3.md). Found set counts **positions** (single, vertical, butterfly, …). Untyped market rows are **Single**. A close-out of the same structure is not another position. Notes are not positions. Table pages **50**. Window-ineligible assigns are **rejected** |
