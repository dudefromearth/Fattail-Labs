...
**Advisor note — created ≠ published.** Session language was "create or edit flag." As
written above the agent polls **publication**, not creation. This is deliberate: the
platform's draft→publish invariant holds that content is born draft and invisible until a
human promotes it. An agent polling *creation* would compose pages from unpromoted drafts
and publish them to a wide-open wiki, defeating that gate from the outside. If Coach intends
the agent to see drafts, that is a ruling that reopens the invariant — **OD-11**, not an
advisor default.
...
| **S5** | YouTube published | `youtube` | Push | Poll — feed item is the signal | Same skill family as S4 |
...
| **OD-9** | Backfill fetch — does the floating bot pull source metadata itself (§4.2) | Open |
| **OD-10** | Does Descript expose a publication signal, or only a list | Open |
| **OD-11** | **Created vs published.** The agent polls publication (§3.5). Does it ever see drafts? Seeing drafts reopens the draft→publish invariant. | **New, open** |
| **OD-12** | **Sitemap.** `fattail.ai`, `labs.fattail.ai`, or both — and linkage target only, or an eighth `source_kind`? | **New, open** |
| **OD-13** | **Poller watchdog.** Alarm when the watermark stops advancing, to satisfy L7 on the poll path (§6.2). | **New, open** |
| **L10** | Hash-over-signal — advisor-proposed, awaiting stamp | Pending |

**Interaction worth ruling in order:** OD-4 and B-2 are coupled. If all composed pages are
human-gated unless Coach names an exception, the admin over-publish path acquires a human in
front of it and **B-2 shrinks considerably.** Ruling OD-4 first is likely the cheaper move.

**On B-3:** the proposed parent citation originated with the reviewing party. Confirmation
by the same party is not verification. A file header closes it; a restatement does not.
...
`Wiki-Source-Contract-Spec-v0.1.3` — content hash to be stamped by Lima on landing, per
...
**End of Wiki Source Contract Spec v0.1.3**
