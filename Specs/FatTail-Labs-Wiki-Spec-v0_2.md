# FatTail Labs — Wiki Spec v0.2 (Unified)
...
> **SUPERSEDED (DL-555).** Advisor draft. Spec of record is
> [`FatTail-Labs-Wiki-Spec-v0_2_1.md`](./FatTail-Labs-Wiki-Spec-v0_2_1.md)
> (v0.2.1 APPROVED; hunks 2–4 are the bump). Banner-only; body frozen.
...
- **`registration`** (IKI templates): payload is the Template Help Package. Agent
  drafts the template's page — what it is, and **how it fits into the rest of
  FatTail Labs**, from package fields + linkage evidence. Insufficient package
  fields → `rejected`/`failed`, never invented fit; extending the package is a
  source-side obligation. Page is a **consequence** of registration (OD-4 as ruled),
  never a precondition. Version bumps = new contract → revision draft.
...
registration kind live end-to-end (waits on Coach directing the Help Package spec
and naming file + version) · everything in I.5.
...
1. **Floating agent delivery** (III.3): discovery → launcher + window + `/app` hub
   provider + in-window adoption proposal. Includes the two owed WA-4 record checks
...
3. **Registration live end-to-end** ("WA-5" lineage): waits on Coach directing the
   Help Package spec in-tree and naming file + version.
