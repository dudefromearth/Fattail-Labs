# FatTail Labs — Wiki Spec v1.0

**Scope statement (doctrine, 2026-08-22)**
- **Active program:** IKI Lab — Wiki.
- **Touches:** this spec (new; becomes the wiki's spec of record, superseding Member Wiki v0.1's *intent* — its data model is carried forward, not replaced); agent charter `agents/bench/oscar.md` (new, companion); parent amendments **named** in §9 (Help Arch, Content Board, Template Runner), not made.
- **Touches outside program:** **NONE.** The admin interface, including any mount outside IKI/Wiki routes, is the **Wiki Admin Interface Spec v0.1** (companion) and carries its own scope statement.

**Status:** DRAFT for Juliet — advisor draft, no authority. First delivery to the bench.
**Date:** 2026-08-22
**Canonical filename:** `Specs/FatTail-Labs-Wiki-Spec-v1_0.md`
**Companions:** `Specs/FatTail-Labs-Wiki-Admin-Interface-Spec-v0_1.md` · `agents/bench/oscar.md`
**Carries forward:** Wiki Proactive Compilation Spec v0.1–v0.4 and Grok review rounds 1–2 — every law, OD, and AT below that originated there is marked **(WP)**. That lineage is retired by this spec; nothing in it is lost.

**Parents:**

| Doc | Role |
|---|---|
| Member Wiki Spec v0.1 | Storage model (§3.0 `lab-wiki` repo + MySQL derived index), tables §3.1–3.4, pipeline ①–⑤, laws W1–W11 — **carried forward as §4 here** with the amendments in §9 |
...
| Party | Job | Never |
|---|---|---|
| **Oscar** (agent, charter `agents/bench/oscar.md`) | Watch production. Register what shipped. Propose target and audience. Compile on disposition. File and link. | Invent product. Publish. Widen audience. Touch page data or Family B. |
...
| # | Law | Source |
|---|---|---|
| **WK1 — Corpus is what ships** | A wiki candidate exists for every §3 kind present on a **production deploy** (WK2a) or **pointed at by an administrator** (WK2b). | Coach · (WP1) |
| **WK2 — Two inputs** | (a) Deploy event `{sha, ts, env=production}` → registrar diff over §3 kinds. (b) Admin-point → candidate directly; never SHA-diffed, never takes the directive shortcut. | (WP1, WP6 r2#4) |
| **WK3 — Directive path unchanged** | Content carrying an explicit directive (§5) flows as Member Wiki ①–⑤. This spec adds no step. Directed reject has no inbox recovery; re-enter via admin-point or a later undirected version. | Coach · (WP2, §4b) |
...
| **WK14 — Member Wiki laws W1–W11 stand** | Carried forward unchanged. W11 (no Family B in compilation) is restated as a hard boundary on Oscar (charter §3). | Member Wiki v0.1 |
...
| `feature` | Help Arch **H1** `surface_key` registry (as-built prerequisite, §9) | `surface_key` [+ declared `state_key` when discriminating] — same key for both origins; `app`/`route` are columns | from route (IKI public → `public`; member → `member`) | **1** |
...
## 4. Pipeline (Member Wiki §4, amended)

```
  ⓪  DEPLOY EVENT {sha, ts, env=production}  (OD-WK1)
        ▼
  ①' REGISTRAR (Oscar) — diff deployed SHA vs last-registered over §3 kinds; first SHA = snapshot (WK7)
        ▼
      directive? (§5) ── yes ──► ③ COMPILER (Oscar) ──► ④ BOARD ──► published
        │ no
        ▼
  ①'' CANDIDATE LIST + PROPOSAL (§6)   ◄──── ⓪' ADMIN POINT (Admin Interface Spec) — writes here directly
        ▼
  ⓐ  DISPOSITION (administrator, via Admin Interface Spec): compile [wiki|help|both] · dismiss
        │ compile
        ▼
  ③  COMPILER (Oscar) — mints 0–2 content_items (stubs, OD-WK2) ──► ④ BOARD ──► published
        ▼
  ⑤  RELATED ENGINE (unchanged)
```

②, ④, ⑤ unchanged. ⓐ writes disposition only; ③ mints. No arrow from `published` back to ⓪ (WK10).
...
```
Append-only. `corpus_items.kind` gains `template \| feature` **at compile** (§9).
...
Rule of thumb (fixed, in the charter): member surface → help, wiki links · template → help (package is body), wiki links · spec/DL → staff wiki · admin-pointed → help default, wiki optional. **Never an unbuilt thing (WK11).**

## 7. Compile (OD-WK2 recommendation: stubs)
Compile writes structured fields from source — no generated prose. For `template`, the fields **are** the help package, already gated at authoring (TH / Tango / Hotel). Board approval is "this goes live on this surface," not a second prose wash. Hotel's compile guidelines (W6) unchanged **iff** this holds.

## 8. Write matrix (Mike)

| Writer | May write | May not |
|---|---|---|
| Oscar — registrar | candidate rows on deploy diff | disposition, corpus, help, page data |
| Oscar — compiler | wiki draft + help draft **after** `disposition=compile`; `corpus_items` row for the compiled kind | publish; candidate identity; Family B |
...
| Member Wiki §3.1 | `corpus_items.kind` += `template \| feature` at compile | W9 |
...
## 10. Acceptance (registrar / compile; interface ATs live in the Admin Interface Spec)
...
| AT-WK5 | First SHA → zero candidates. |
...
| AT-WK9 | Template `public`-licensed → candidate and help article `audience=public`, flag intact. |
...
## 11. Boundaries
No agent-direct publish (W5). No second approval state. No audience widening. No page data or Family B in any capture or compile context (W11). No compiler prose (iff OD-WK2). No staff or public sink until ruled. No product proposal (WK11). Admin interface and mounts: companion spec.
...
| OD-WK2 | Compile = stubs (recommended) or prose | **Coach** |
...
| OD-WK8 | Snooze disposition — only with explicit wake; not v1.0 | Coach |

## 14. Review chain
Juliet → §9 parents landed → India → Hotel (W6 guidelines unchanged iff OD-WK2) → Mike (W11, §8) → Tango (any string Oscar emits) → Foxtrot (OD-WK1) → Coach → Lima → Juliet seeds → Delta. Design chain reviews the **Admin Interface Spec**, not this one.
