...
**Status:** DRAFT v1.1 for Juliet — advisor draft, no authority. Grok round 3 folded; changes marked **[v1.1]**. **First seed is §15 W0, not wave 1.**
**Date:** 2026-08-22
**Canonical filename:** `Specs/FatTail-Labs-Wiki-Spec-v1_1.md`
...
| **Oscar** (agent principal, charter `agents/bench/oscar.md`) | Propose target/audience on each candidate. Compile stubs on disposition. File and link on publish. Compile the directed path (③) for both corpora. | Invent product. Publish. Widen audience. Touch page data or Family B. Poll YouTube. Transcribe. Invoke ⑤. |
...
| **WK15 — File at publish** **[v1.1 new]** | Compile mints drafts + `content_items` only. `corpus_items`, page→help ref, and `compiled_from` are written **on the board's publish event** (OD-WK9), never before. A rejected compile leaves no corpus row. ⑤ stays a tick; Oscar does not invoke it. | Grok r3 #4, #10 |
...
```
 ...
 ② transcriber — captions / Whisper               first SHA = snapshot (WK7); upsert identities
        ...
        │                                            │ compile
        ▼                                            ▼
 ③ COMPILER — OSCAR (both paths; stubs, OD-WK2) — mints 0–2 content_items ◄───────────────┘
        ▼
 ④ BOARD — awaiting_approval → published | rejected      (publish event → OD-WK9)
        ▼ on publish
 OSCAR FILE & LINK (WK15) — corpus_items row · page→help ref · compiled_from
        ▼
...
| Oscar — compiler | wiki draft + help draft + `content_items` **after** `disposition=compile` | publish; candidate identity; `corpus_items`; Family B |
| Oscar — filer (on publish event) | `corpus_items` row; page→help ref; `compiled_from` | anything before publish; ⑤ |
...
| AT-WK13 **[v1.1]** | Publish event → `corpus_items` row + page→help ref written; board reject → no corpus row exists. |
...
## 11. Boundaries
No agent-direct publish (W5). No second approval state. **[v1.1]** No replacement of Member Wiki ①②⑤. No model call in the watcher. No `corpus_items` before publish. No `public` inferred from a route. No audience widening. No page data or Family B in any capture or compile context (W11). No compiler prose (iff OD-WK2). No staff or public sink until ruled. No product proposal (WK11). Admin interface and mounts: companion spec.
...
## 15. Seed slices **[v1.1 new — Grok r3 #6]** — the §3 wave table is product intent; this is the packet order
...
| **W1** | Launcher on IKI + Wiki routes · admin-point · compile **wiki-only** stubs · board card · file-on-publish for wiki pages | OD-WK2 = stubs · OD-WK9 publish event | Help target (chooser shows help disabled) |
...
| **W4** | `kind=feature` from watcher | H1 `surface_key` registry as-built, **or** a declared IKI/Wiki-local `surface_key` list inside this program (never an invented Application Framework `apps` table) | — |
