# Volume Profile Service — Spec v0.6

**Status:** DRAFT — contract close. Build in flight under stamped tokens
(VPS1-W0, VPS2-W0, VPSB-W0); this version folds resolved decisions and
field-proven amendments into law. No new build scope until its review.
**Date:** 2026-09-17
**Supersedes:** Volume-Profile-Service-Spec-v0_5.md
...
Parents: Analyzer AZ-VP-9 (amended per Q10 = (b): the member volume
profile surface renders SA objects; this service feeds computing
consumers only — the amendment's surface law lives in SA spec §8),
AZ-VP-3, AZ-VP-6; GEX Quad complement; OPF named-state doctrine.
...
Sessions + developing ship first; composite publish follows without a
new contract.
...
**Publication space (Q7 = (a), law):** source-space bins + mapping
block; consumers apply the published offset and ratio. A mapping
failure degrades only the mapping block, never the histogram. VP-L11's
round-and-merge governs the service's internal target-space
accumulations (the §6 roll-coherent stores and any /range served in
target space), where unequal grids merge nearest-half-up, source rows
landing on one target row sum, empty target rows stay empty.
...
- /health: ingest liveness per collector, last print per source, gap
  ...
  FatTail-Intelligence API auth section. Access to this API is
  entitlement for computing consumers; per VP-L18 no member surface
  fetches it for rendering.
...
- **VP-L18** Raw bins live on the data end (StudioOne) and are
  ...
  render Structural Analysis objects (SA spec §8; AZ-VP-9 amendment).
...
+2.6.
**F4 — unequal-grid merge (ES→SPX).** ES 6558.25/6558.50/6558.75/
6559.00 × 100/150/120/80 → SPX (nearest half-up): `6558: 100 ·
6559: 350`.
**F5 — gap.** 6-min feed hole mid-session → status GAPPED, interval in
payload, totals exclude it; prior session COMPLETE fetchable.
**F6 — renderer-boundary harness:** no consumer outside the computing
class can obtain bins (VP-L18 enforcement check).
...
**F9 (new) — MES→XSP ratio-from-metadata.** Ratio 0.1 exact; MES rows
7682.25/7682.50/7682.75/7683.00 × 100/150/120/80 → ×0.1 →
768.225/768.250/768.275/768.300 → XSP grid 0.10 (nearest half-up):
`768.2: 100 · 768.3: 350`. Offset fitting and freeze behave as F3 on
the scaled values.
...
everywhere; mapping-only refresh changes nothing; live publish matches
`vp_rebuild` byte-for-byte.
...
- v0.6 — this document: resolved decisions folded into law; futures
  ingest law field-proven; MES staging + ratio-from-metadata; vendor-
  floor retention; mark provenance; VP-L18. Next: Grok review of the
  complete file (triple pre-flight), VPB-Q1 evidence, Q8.
