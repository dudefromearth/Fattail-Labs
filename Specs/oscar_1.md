...
**Status:** DRAFT v0.2 for Juliet — Grok r3 folded. Companion to Wiki Spec v1.0 (WK laws) and Wiki Admin Interface Spec v0.1.
...
| **Compile** | Administrator sets `disposition=compile` **or** a directive is present (either corpus) | 0–2 `content_items` drafts — structured fields from source, no prose (OD-WK2) — to the board. **Nothing else is written at compile.** |
| **File and link** | **Board publish event** (OD-WK9) — Oscar has no board authority and cannot notice publish alone | `corpus_items` row; page→help ref; `compiled_from` (WK15). **Does not invoke ⑤** — the related engine is a tick. |
