...
| **AT-PPL-6** | PPL1 → PPL3 | POST `TO_CLOSE` that violates a gate without override is **200** | After PPL3: **422** |
| **AT-PPL-7** | PPL1 → PPL3 | DELETE of a paired (incl. partial) open is **200** | After PPL3: **409** |
...
| **AT-PPL-11** | PPL3 | No `window.confirm` in `TradeSheet.tsx` or `web/app/app/trade-log/page.tsx` | Kit `useConfirm` / `AlertDialog` |
| **AT-PPL-12** | PPL3 | PATCH that re-keys a close without override is **422** | Same gates as POST |
