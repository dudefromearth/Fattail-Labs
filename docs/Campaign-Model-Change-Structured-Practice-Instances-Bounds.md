...
**THE TWO ROLES (Coach lock — this was the original point, restored 2026-08-08):** Every bound carries a **role**: **`boundary`** or **`goal`**. Coach's founding statement of the blood-panel model: *"these attributes can be **goals to measure against** as well as limits."* Same attributes, same all-bands grammar, same dimensions and n-floors — different witness semantics:
...
| Panel render | Corridor bars, marker in/out of range | Marker approaching target band, trend arrow across draws — "tracking toward / tracking away," never "failing" |
...
- New `member_practice_campaign_bounds` (Family B): `campaign_id` FK · **`role ENUM('boundary','goal')`** · `attribute` (enum: process clauses + six stats) · dimension fields (unit, basis, window) · `range_low`/`range_high` (either nullable → one-sided allowed) · `is_critical BOOL` (at most one true per campaign; **boundary-role rows only**) · `n_floor` · timestamps. Bounds are **charter fields** → amendment rows on post-signature change
...
12. Export/import round-trips bounds (including role), ledger flag, and stamp provenance; colliding import names suffix with report note.
13. **Goal-role rows never produce variance** — no fill and no panel reading generates a variance record from a goal; goal progress renders with trend across draws and respects the n-floor ("gathering" below validity).
14. **Critical designation rejects goal-role rows** (4xx, fail loud); only a boundary can be the Invalidation clause.
