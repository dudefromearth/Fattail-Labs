...
1. **Heat restraint, as code.** While the member has a live open position (source of
   record: open Trade Log position for that identity/date — never model judgment), the
   Coach is quiet and non-analytic: receives, acknowledges briefly, answers if asked; no
   unprompted questions, no analysis, no emotional processing. Hard reject at the same
   layer as existing render-time guardrails. Thin spec required: live-position signal.
