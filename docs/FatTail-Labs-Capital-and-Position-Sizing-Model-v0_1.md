...
**Spec (formal for review):** [`Specs/FatTail-Labs-Capital-and-Position-Sizing-Spec-v0.1.md`](../Specs/FatTail-Labs-Capital-and-Position-Sizing-Spec-v0.1.md)  
**Status:** DRAFT v0.1 — for bench review and Coach ratification  
**Date:** 2026-08-09  
**Author:** Advisor layer (Claude), from Coach's dictated model of 2026-08-09  
**Relationship to other specs:** Standalone doctrine document. Sits *beside* — not inside — `FatTail-Labs-Member-Campaign-Spec-v1_3.md`. Most of this model lives **above** the campaign layer, at the account and total-capital level, which is why it is its own document rather than a campaign-spec section. The campaign spec's §7 process clauses (position-size band, fear-sizing floor) remain in force; this document supplies the capital hierarchy those clauses size against.
...
### Ring 2 — Campaign allocation (declared intent)

- Each campaign declares the slice of total net capital it intends to run. Pure intent. Nothing stops the declaration.
- Consistent with the window model's **account freedom**: a campaign's allocation is a slice of the *whole pool*, not of any particular account it happens to trade from. Campaigns carry no account binding; allocations carry none either.
- Allocations make campaigns **compete for capital honestly**. When the sum of declared allocations exceeds actual total net capital, that is a visible, structural fact — witnessed, not hidden, and not blocked (see §4.1).
...
1. **The capital overview surface.** §4.1 and §4.2 require a surface above the campaign layer. Where does it live — a section of the Journey app, a new page, a card on the practice home? Needs Echo + Tango, then Coach.
2. **Tolerated master drawdown — where declared?** The sovereign witness needs the trader's declared tolerance on total net capital. It is not a campaign bound (it's campaign-blind), so it belongs at the identity/practice level. Where the trader declares and amends it is open.
...
## 9. Deferral note — Campaign Journey radar (carried from this session)

Per Coach's direction of 2026-08-09: the **Campaign Journey radar chart with lifetime slider** (Campaign Spec v1.3 §6 / D5) consumes disproportionate resources for its benefit and is **DEFERRED — parked, not deleted**, pending value re-evaluation. The design (axes-as-bounds, band-alignment, as-of-T evaluation, Minimum Shape rule) remains on the shelf exactly like the convexity gauge, and nothing in this document depends on it. The v1.3 spec should be annotated accordingly in its next touch; this note is the flag until then.
...
| Decision log | **Lima** | Model ratification entry; radar deferral entry; live-sync OD entry |
...
| Version | Date | Change |
|---|---|---|
| v0.1 | 2026-08-09 | Initial draft from Coach's dictated model: two ceilings, three rings, campaign-blind master drawdown, three witnesses, buying-power gate with three source postures, live-sync queued, radar deferral note. |
