...
**Date:** 2026-09-01
**Plan revision:** **v1.1** — merged review applied. **No new law. GP1–GP24 and OD-GP1–7 untouched.**
**Supersedes:** v1.0 (Juliet, 2026-09-01) — structure and content preserved; ten findings applied
...
**Not changed:** DAG shape after P2 · isolation §4 · seating §12 · every locked law · B4's
report-don't-invent default · AT-GP14 withdrawn.
...
| `--symbol` unused; feed idles without interest | GP21 — **and why P1b exists**. |
...
| Gate | Rule | Unblocks |
|------|------|----------|
| **W0-0** | Coach stamps **spec v0.2.2 BUILD AUTHORITY** and **this plan v1.1**. OD-GP3 ticked. **AT-GP22 ownership stated.** | W0-1 |
...
| **P1b-G** | `plane_interest.py` heartbeating. **`mb:ladder:*` key AND an `mb:pub` message observed with NO member watching.** Topic string **unmodified** under `list_interest_topics("mb:ladder:")`. `bus ≠ not_configured` on that host | P2 live AT-GP1 |
...
```text
W0-0 Coach GO SPEC + plan v1.1 + OD-GP3 + AT-GP22 ownership + DL-539 OKs
...
|------|-------|-----------|
| `W0-0-coach-go.md` | Coach | `GP-W0.md` W0-0 STAMP: spec v0.2.2 BUILD AUTHORITY, plan v1.1 Accept, OD-GP3 ticked, **AT-GP22 ownership stated**, DL-539 OKs started |
...
| Seed | Agent | Done when |
|------|-------|-----------|
| `P0-1-lima-arch30.md` | Lima | Arch 30 honesty (spec §10 list). DL. Canonical hyphenated spec. **Do not invent** OPF Reference or L4-A v0.4 unless B4 ticked. Report missing cites. **Record the GP18a erratum** (§1) so a builder does not read `generation.py` as out of scope |
...
| Seed | Agent | Done when |
|------|-------|-----------|
| `P1b-1-alpha-interest.md` | Alpha | `server/opf/plane_interest.py`. Heartbeat `< 45 s` grace (default 15 s) for enumerated listed pairs and `LABS_OPF_PLANE_WINGS_TOPICS`. **Same `mb:interest:{topic}` key** — attribution in a sidecar or `held_by`, **never a decorated topic**. Stops when the plane is disabled |
...
| Seed | Agent | Done when |
|------|-------|-----------|
| `P4-1-alpha-writer.md` | Alpha | `listed_writer.py`. GP16–GP20. `set_json` same path. `book: listed`, `source: listed_writer`. Cadence OD-GP4. **Calls `bus_ladder_key(book="listed")` — parser already proven at P2-0** |
...
| AT-GP23 | **P1b** / P4 full | **P1b-G** / P4-G | plane interest; feed does not idle |
...
| **v1.1** | 2026-09-01 | **Merged review applied — ten changes, no new law.** **P1 split P1a (Foxtrot infra) / P1b (Alpha `plane_interest.py`)** — P1's exit was circularly dependent on code Foxtrot was being asked to ship. **`keys.py` → P2-0, gated alone** on AT-GP22 before the hydrator. **W0-1 substitution** for the missing Reference. **AT-GP19 gated** at P2-G. **`store_read.py` assigned** to P2-1 with AT-GP2 as exit. **`_env` helper named on P2-2.** P1-2 restated as observable facts. **O6 promoted to §10.** Capability ≠ lift stated at §0. **GP18a spec erratum recorded at §1.** OD-GP3 recommends StudioTwo with `PONG` evidence |

**Next for Coach:** read this plan; tick **OD-GP3**, **AT-GP22 ownership**, and **DL-539 OKs** on
`agents/go/GP-W0.md`; stamp W0-0 or return.