# Options Lab Heatmap — LIM Full Agent Bench Plan v1.2

**Date:** 2026-09-02  
**Plan revision:** **v1.2**  
**Canonical filename:** `docs/Options-Lab-Heatmap-LIM-Full-Agent-Bench-Plan-v1.2.md`  
**Supersedes as working plan:** [`docs/Options-Lab-Heatmap-LIM-Full-Agent-Bench-Plan-v1.1.md`](./Options-Lab-Heatmap-LIM-Full-Agent-Bench-Plan-v1.1.md) **v1.1** (keep on disk; do not execute against it)  
...
| Doc | Path | Status |
|-----|------|--------|
| **Heatmap LIM Template Spec v0.4.3** | [`Specs/FatTail Labs — Heatmap LIM Template — Specification v0.4.3.md`](../Specs/FatTail%20Labs%20%E2%80%94%20Heatmap%20LIM%20Template%20%E2%80%94%20Specification%20v0.4.3.md) | **DRAFT for build planning.** Errata E1–E17. **Not BUILD AUTHORITY until LIM0-0.** Whole-file sha1 at plan land: `01f638f590492520236b3607edde487b949d6016` |
| Heatmap LIM Template Spec v0.4.2 | same directory `…Specification v0.4.2.md` | **SUPERSEDED.** E8–E14. Keep on disk. |
| Heatmap LIM Template Spec v0.4.1 | same directory `…Specification v0.4.1.md` | SUPERSEDED. sha1 `e5aa6fdc7305add72e02dc57b62f87665d348820`. Keep. |
...
**Spec status:** v0.4.3 **DRAFT**. **LIM0-0 GO** is the stamp. Do not fire LIM1 until `OLLIM-W0.md` is GO.
...
## 0. Delta from plan v1.1 (why v1.2 exists)

Hotel's eight goldens caught three **authoring** defects in v0.4.2, before any code. Spec **v0.4.3** is E15–E17. **No new geometry.**

| Errata | Plan consequence |
|--------|------------------|
| **E15** | `dPct = dist/spot × 100`. Proximity was pegged at 0. **AT-LIM29** requires a strictly interior value. Ninth golden. |
| **E16** | `steepness` uses `(hi − lo)`, not `ctx.strikeStep`. **AT-LIM30**. |
| **E17** | `spotBelowNearestCrossing = spot < lo`. Inside is false. **AT-LIM31**. |
| **OD-LIM10** | Comfort readout shows `magF` beside the proximity chip (Juliet default if silent). Compact unchanged. **AT-LIM32**. |

v1.1 E8–E14 remain in force:
...
**L1–L18 PROVISIONAL until LIM0-0.** Spec §15 ODs and Juliet recs are disposed on the GO token.
...
| **L18** | Comfort readout includes **`magF`** beside the proximity chip (OD-LIM10 Juliet default). No new geometry. Compact unchanged. | OD-LIM10 · AT-LIM32 | PROVISIONAL |
...
| **OD-LIM9** | Live Heatmap Templates parent | **India names the canonical filename** — `Specs/FatTail-Labs-Options-Lab-Heatmap-Templates-Spec-v0_2.md` (header: current revision **v0.2.3**) **or** `Specs/FatTail-Labs-Options-Lab-Heatmap-Templates-Spec-v0_3.md` (amendment, supersedes v0.2 only in the clauses named in its §2) — **before** opening the merged draft. There is no `v0_2_1` file. LIM0-1 named **`…-v0_2.md` rev v0.2.3**. |
| **OD-LIM10** | Comfort magF readout | **Juliet default if silent:** `magF` in the Comfort readout beside the proximity chip. No new geometry, no cell name, Compact unchanged. Echo + Tango dispose. Hotel F2 vs F4: same y=40, magF 80 vs 0. |
...
| **LIM0-9** | Lima | DL draft: GO · OD-LIM1…10 · sha1 `01f638f5…` procedure · Arch 29 outline |
...
| **AT-LIM29** | Nearest crossing between floor and ceiling (e.g. 50 pts / spot 5000 = 1.0 %) → `0 < crossingProximity < 1` |
| **AT-LIM30** | Crossing spans a skipped zero-net strike → steepness uses `(hi − lo)` |
| **AT-LIM31** | Spot inside → `spotBelowNearestCrossing` false **and** distance 0 |
| **AT-LIM32** | Two fixtures, equal `nearSpotMix`, magF 0 vs magF > 50 → both magF values reach Comfort readout |
...
Plan revision **v1.2**. Spec **v0.4.3 DRAFT**.  
...
| **v1.1** | Re-decomposition against LIM Spec **v0.4.2**. E8–E14 folded. OD-LIM8 · OD-LIM9. AT-LIM17b, 24–28. |
| **v1.2** | Spec **v0.4.3**. E15–E17 (proximity percent, steepness hi−lo, spotBelow = spot < lo). AT-LIM29–32. OD-LIM10 magF Comfort readout. Ninth golden. Spec sha1 `01f638f590492520236b3607edde487b949d6016`. |
