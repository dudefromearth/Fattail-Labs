# Options Lab Heatmap — Term Mass Full Agent Bench Plan v1.1

**Date:** 2026-09-18  
**Plan revision:** **v1.1**  
**Supersedes as working plan:** [`docs/Options-Lab-Heatmap-Term-Mass-Full-Agent-Bench-Plan-v1.0.md`](./Options-Lab-Heatmap-Term-Mass-Full-Agent-Bench-Plan-v1.0.md) **v1.0** (keep on disk; do not execute against it)  
**Canonical filename:** `docs/Options-Lab-Heatmap-Term-Mass-Full-Agent-Bench-Plan-v1.1.md`  
...
**Member name (Coach 2026-09-18):** **Term Mass** — that is the Template list string. Internal id stays `gex-cal`. Working spec filename is `…-GEX-Calendar-Spec-v0_1_1.md` (v0_1 is the baseline freeze). Law IDs stay **GC\***. Do not coin a second id.

**Primary law:**

| Doc | Path | Status |
|-----|------|--------|
| **GEX Calendar Spec v0.1.1** | [`Specs/FatTail-Labs-Options-Lab-Heatmap-GEX-Calendar-Spec-v0_1_1.md`](../Specs/FatTail-Labs-Options-Lab-Heatmap-GEX-Calendar-Spec-v0_1_1.md) | **BUILD AUTHORITY** upon `GC0-W0`. sha1 `37c4427c2ffc3071dc27a8592c16cf6307acd289`. v0_1 baseline freeze. OD-GC1…GC5 ACCEPT. |
| Heatmap Templates Spec **v0.2.4** | [`Specs/FatTail-Labs-Options-Lab-Heatmap-Templates-Spec-v0_2.md`](../Specs/FatTail-Labs-Options-Lab-Heatmap-Templates-Spec-v0_2.md) | Live parent. **HM21 = inspector tab-session** (DL-575). Frozen `gex` §5.5 `gex_v1`. **There is no v0.2.1 file.** |
| Heatmap Templates Spec **v0.3** (amendment) | [`Specs/FatTail-Labs-Options-Lab-Heatmap-Templates-Spec-v0_3.md`](../Specs/FatTail-Labs-Options-Lab-Heatmap-Templates-Spec-v0_3.md) | **DRAFT.** Auxiliary **read plane** (SVP). **Not used by gex-cal.** Rebase note: must not steal live **HM21**. |
| LIM Spec v0.4.7 | `Specs/FatTail Labs — Heatmap LIM Template — Specification v0.4.7.md` (exact disk filename; spaces + em dash) | Sibling. Never fused. Frozen `gex` SHA1 stays. |
...
**Spec status:** v0.1.1 **BUILD AUTHORITY** upon `GC0-W0`. Do not fire GC1 until that token is stamped. JR8 option **(a)** is this GO (AGENTS.md current-state row).
...
| **JR6** | **WITHDRAWN (plan v1.1).** Spec forked to `…-v0_1_1.md`. Hash is whole-file, in DL, not in the Spec. |
| **JR7** | Reviews and gates → **GROK ADVISOR via Coach**. GBH does not self-review the spec. |
| **JR8** | **Isolation (DL-539 / DL-747).** **GC0-0 option (a):** this GO authorizes adding Term Mass / GBH to the AGENTS.md current-state table at GC0. |
...
Plan revision **v1.1**. Spec **v0.1.1** sha1 `37c4427c2ffc3071dc27a8592c16cf6307acd289`.  

**Next:** GC0 seats on disk → GC0-G → stamp `GC0-W0.md` → GC1.

### Changelog

| Id | Note |
|----|------|
| **v1.1** | Advisor F2. Points at spec **v0_1_1** sha1 `37c4427c…`. JR6 filename-stays withdrawn. Templates v0.3 auxiliary-plane row (not used by gex-cal). LIM path = exact disk filename. OD-GC1…GC5 ACCEPT. JR8 option (a). |
| **v1.0** | First Juliet decomposition of GEX Calendar Spec v0.1. Coach names picker **Term Mass**. Keep on disk. |
