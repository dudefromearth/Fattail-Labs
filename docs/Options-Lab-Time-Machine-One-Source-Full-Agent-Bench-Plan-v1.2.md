# Options Lab Time Machine One Source — Full Agent Bench Plan v1.2

**Date:** 2026-08-28  
**Plan revision:** **v1.2**  
**Canonical filename:** `docs/Options-Lab-Time-Machine-One-Source-Full-Agent-Bench-Plan-v1.2.md`  
**Supersedes:** plan v1.1 (missed disposing `seedTodayFromSession`; OS-1 allowed a fixture) and plan v1.0  
...
**What v1.2 adds over plan v1.1** (no spec change — review leftover):

- **W2 disposes `seedTodayFromSession` by name.** It is not an as-built footnote. W1 lifts `TODAY_LIVE`, so a path that has been failing safely starts working unless W2 removes it or folds it into `loadTmDay`. It must not sit. W2-G fail-closed names it — quieter than `engageTodayFromCache`, and more dangerous for that reason.
- **OS-1 is a live late-tab walk.** A fixture may characterize the code path. It does not close OS-1. The member outcome is the reason this GO exists (same lesson as the layout wave: tests passed, live walk found the clip).

**What v1.1 added over plan v1.0** (no new product — leftover pass matching the spec):
...
**No product code until W0-0 stamps this plan v1.2 and promotes spec v0.2.1 to BUILD AUTHORITY, and W0-G PASS.**
...
| **NX-SEED** | Leaving `seedTodayFromSession` alive after W1. A dead path that starts working is a silent today-walk. |
...
| **FP12** | No two-slot film. Occupancy is **one downloaded day** or none. A packet that keeps `captureToday` as the today-replay source **fails W2-G**. A packet that leaves `seedTodayFromSession` sitting after W1 **fails W2-G**. |
...
| Gate | Rule | Unblocks |
|------|------|----------|
| **W0-0** | Coach stamps **this plan v1.2** on `TMOS-W0.md` **and** promotes spec **v0.2.1 to BUILD AUTHORITY** (or names a fold into TM v0.8). Until then the spec is DRAFT and v0.7.4 is still law. | W0-1 |
| **W0-2 India** | One source; snapshot; full download; `TODAY_LIVE` lift; `day_changed` in-flight only; Average protected; `seedTodayFromSession` named for W2 disposal; no Labs film; no refresh control; parents *confirm* | W0-G |
...
| **W2-G** | One `loadTmDay` for every date. Raise today downloads StudioOne today. `captureToday` is not required for the range. **`seedTodayFromSession` is gone or is `loadTmDay` — it does not sit.** **Average still works** (OS-9). Snapshot: last sample does not grow. Switch discards first. Reset drops hold. **No refresh control.** Fail-closed: live-socket film as replay; `seedTodayFromSession` leftover; second hold; tail-append; Average dead; 1-min; Record | W3 |
...
| **W-G** | Fail-closed: TM glow, Record, Labs film, 1-min, Basic chrome, TPO, 1×, Spaces, Factory, Instant Replay name, `TODAY_LIVE` refusal, login-bounded today, megabyte slider, second hold, tail-append, refresh control, Average broken, `seedTodayFromSession` leftover, rehearsal in Trade Log. **OS-1 live late-tab walk required.** **C11 unusable without Coach = not ship** | ship |
...
```text
W0-0 Coach stamps plan v1.2 + spec v0.2.1 BUILD AUTHORITY
  → W0-1 Lima sha1 + DL
  → W0-2 India ∥ W0-3 Echo ∥ W0-4 Hotel
  → W0-G
       → W1 SO-AR today retrieve → W1-G
            → W2 one load path / snapshot / dispose seedTodayFromSession / Average stays → W2-G
...
|------|-------|-----------|
| `W0-0-coach-plan-stamp.md` | Coach | `TMOS-W0.md` W0-0 STAMP: plan **v1.2** Accept; spec **v0.2.1 BUILD AUTHORITY** (or named fold) |
...
| `W0-2-india-parents.md` | India | As-built quote of `TODAY_LIVE`, `captureToday` **into the hold** vs Average ring, `engageTodayFromCache`, **`seedTodayFromSession`**, two-slot occupancy; TMI-82…90 vs v0.7.4; `day_changed` in-flight only; §12 of v0.2.1 matches TMI-88; W2 must dispose the seed; no Labs film |
...
| Seed attempt | `seedTodayFromSession` (`web/lib/options-lab/archiveLoad.ts`) writes StudioOne gens into the today slot. **As-built it cannot land** because retrieve of today is refused (`TODAY_LIVE`). **W1 lifts that refusal.** After W1 the seed starts working unless W2 **disposes it by name**: remove it, or fold it into `loadTmDay` so there is one path. It must not sit. A silent fill of the today slot is a quieter bug than `engageTodayFromCache`. |
...
| Seed | Agent | Done when |
|------|-------|-----------|
| `W2-1-charlie-one-path.md` | Charlie | `loadTmDay` uses `fillArchiveSlot` for **every** date including today. TMI-64: no download until playhead. Raise today → WAITING → coarse → infill to **full**. **Snapshot:** hold does not append; completed hold does not re-check hash. Switch discards first. Reset drops hold. **`captureToday` writes into the TM hold go.** **`seedTodayFromSession` is disposed by name** — deleted, or identical to `loadTmDay`, not a third path that fills the hold on mount. Live-generation capture that feeds **Width Fit Average stays** (OS-9). Occupancy is one day or none. **No refresh control** — newer range is Reset then raise. |
| `W2-G-delta.md` | Delta | Fail-closed: `engageTodayFromCache` as the today walk; **`seedTodayFromSession` leftover** (call site, export, or mount-time fill of the hold); `captureToday` required for OS-1; tail-append; `day_changed` after complete; two holds; Average broken; refresh control; 1-min; Record |

**Proof OS-1 (live walk required):** a **real tab opened after 13:00 ET** (America/New_York), empty local hold, raise today; left edge is StudioOne’s first print of that session, not tab-open. A fixture may characterize the code path in W2; **it does not close OS-1.** W5 and W-G require the live late-tab walk. Same lesson as the layout wave: two tests passed and the live walk found the transport clipped.
...
| Seed | Agent | Done when |
|------|-------|-----------|
| `W5-1-kilo-ats.md` | Kilo | **AT-TM-OS-1…9** with evidence. **OS-1 is a live late-tab walk** (after 13:00 ET, empty hold) — a fixture does not close it. **AT-TM-C11:** one fully infilled **dense** session (real, infill complete, count matches index). Record resident bytes. **No ceiling named.** If the tab is unusable, say so — do not pass. HOLD C6. Never waive. |
| `W5-G-delta.md` | Delta | Ternary on OS-1…9 + C11 recorded. OS-1 without a live late-tab walk is not PASS. HOLD C6 is not a fail. Unusable C11 → not PASS without Coach. |
...
| ID | Criterion |
|----|-----------|
| **OS-1** | **Live late-tab walk.** Tab opened after 13:00 ET, empty local hold, raise today; left edge is StudioOne’s first print of that session. A fixture does not close this row. |
...
Fail-closed: live-socket film as replay source; `TODAY_LIVE` refusal; login-bounded today; megabyte slider; Instant Replay product name; second hold; tail-append; refresh control; Average broken; `seedTodayFromSession` leftover; `day_changed` on a completed hold; client Massive; Labs film module; 1-minute fetch; TM glow; rehearsal in Trade Log; OS-1 closed on a fixture only.
...
| Version | Date | Notes |
|---------|------|-------|
| **v1.2** | 2026-08-28 | **Stamp target.** Law = spec **v0.2.1**. W2 disposes `seedTodayFromSession` by name (W1 would otherwise turn a dead path live). OS-1 requires a **live late-tab walk**, not a fixture. Supersedes v1.1. |
| **v1.1** | 2026-08-28 | Spec v0.2.1 leftover pass in the plan. SUPERSEDED: `seedTodayFromSession` was only an as-built note; OS-1 allowed a fixture. Do not stamp v1.1. |
| **v1.0** | 2026-08-28 | Written against spec v0.2. SUPERSEDED: that spec’s §12 still offered tail-append. Do not stamp v1.0. |

**One-line law:**  
**One surface, one scrubber, one source — StudioOne for every date including today; the hold is a snapshot; newer range is Reset then raise; a late login still gets what the archive holds and OS-1 is a live walk; full download, measured not argued, unusable is Coach; Instant Replay is not a name; Average is not replay; `seedTodayFromSession` does not sit; Basic, TPO, and 1× are not this GO.**
