...
> **SUPERSEDED** by [`docs/Options-Lab-XSP-SPY-Scale-Full-Agent-Bench-Plan-v1.1.md`](./Options-Lab-XSP-SPY-Scale-Full-Agent-Bench-Plan-v1.1.md). Do **not** stamp `XS0-0` against this file. Claude review 2026-09-13: B1–B4 blocking.
...
**Proposed canonical land path (Coach names it):** `docs/Options-Lab-XSP-SPY-Scale-Full-Agent-Bench-Plan-v1.0.md`  
...
3. Non-regression for **SPX / NDX / RUT / VIX / QQQ / IWM**: Create recipes that are 20 today stay 20 (NDX butterfly stays 20, not 50 — JR2); `defaultWidth` NDX/NQ stays 50 (WIDTH-1 Keep); heatmap columns for those symbols stay `HEATMAP_FLY_WIDTHS` 10…50 (not profile step-multiples).
4. Heatmap Advanced Fly **and** Width Fit columns on **XSP and SPY** consume the **overlay** `[1..7]` (OD-XS1 rec), never kind-default MSC_SPX. Heatmap **SPX-class stays 10…50** (DL-435 remainder).
5. Butterfly / `labCreateOpenDefault` stop returning 20 for XSP/SPY via a **butterfly-only** resolver (not via `productWingHint`). XSP Lab recipes that assign `wingWidth: w` become 1 (**L11b**). **iron_condor stays `Math.max(w, w*2)` → 2.** SPY non-butterfly recipes stay at today’s hint (5) unless Coach expands.
...
| **NX5** | Move SPX heatmap off 10…50, or NDX/RUT/VIX/QQQ/IWM profiles, unless Coach expands |
...
|---------|----------------|
| **Profile is scale SoR** | `market_symbol_universe.app_profile_json.fly_widths` + `fly_width_mode` (server `resolve_symbol_profile`, client `coerceSymbolProfile`). Kind is a **settlement** label, not a scale discriminator (**DL-699**) |
...
**Design invariant:** *XSP and SPY fly scale is the profile list `[1..7]` (or the minimum OPF-listed symmetric wing). SPX-class fly scale on the heatmap remains DL-435 `[10…50]`. Create never presents an unlisted arithmetic width as a finished butterfly.*
...
Universe profile (XSP/SPY: fixed_points 1–7)
  → one column-width resolver (heatmap) + one recipe-width resolver (Create)
       heatmap: XSP/SPY → profile 1–7 ; SPX-class → HEATMAP_FLY_WIDTHS
...
| SPX/RUT/NDX/VIX/QQQ/IWM profiles | migration 119 kind defaults as landed |
...
| Client kind default vs helper | `useOptionsLab` always coerces (`optionsLabContext.tsx` L100–115). XSP index → MSC_SPX `[20…50]`; SPY etf → step-multiples `[1..8]`. Empty-`fly_widths` fallback is **dead** on the panel | **XS2** overlay gate (mode `fixed_points` **and** min < 10 / max ≤ 7) |
...
|----------|-------|--------|
| `productWingHint` | XSP → **20**; SPY → **5** (fallthrough); NDX → 50; RUT/SPX → 20 | **L11b:** XSP → **1**; SPY stays **5**; others unchanged. Used as `w` by vertical / bwb / condor / iron_fly / calendar / diagonal / straddle / strangle / single. **iron_condor** uses `Math.max(w, w*2)` → **2** on XSP |
...
| Runner heatmap / width-fit | `heatmapFlyWidths()` | helper with `{ symbol: ctx.symbol }` only. `ChainContext.symbol` is `web/lib/options-lab/templates/types.ts` L30 — **not** a runner `types.ts`. **L12:** runner uses the XSP/SPY constant; panel follows overlay profile |
...
| `kindDefaultProfile` XSP offline | MSC_SPX 20…50 always present on the panel via coerce | do not change kind defaults; helper **rejects** MSC_SPX / step-multiples for XSP/SPY |

**Related, default out unless OD-XS7 Accept:**

| Location | Note |
|----------|------|
| `OpfRiskAnalyzer.tsx` L1520 `XSP → 6000` | Empty-shell axis only (before first mark). Wrong scale. **SPY has no special case** (falls through to 100). OD-XS7: XSP → ~spot-class (e.g. 600 or last mark), not 6000 |
...
| **L1** | Heatmap XSP/SPY columns = **overlay-gated** `fly_widths` `[1..7]` (`fixed_points` and min < 10 / max ≤ 7). Kind-default MSC_SPX or step-multiples 1…8 must **not** paint. Heatmap SPX-class stays `HEATMAP_FLY_WIDTHS` 10…50 | OD-XS1 rec (a) | PROVISIONAL |
...
| **L9** | Other symbols do not move (Create 20 where today 20; heatmap 10…50 for NDX/RUT/VIX/QQQ/IWM/AAPL/SPX) | Coach intent | PROVISIONAL |
| **L10** | `AnalyzerPositionsList.tsx` is frozen | Standing constraint | LOCKED (standing) |
| **L11b** | `productWingHint("XSP")` → 1. Templates that set `wingWidth: w` seed 1. **`iron_condor` keeps `Math.max(w, w*2)` → 2** — do not change the multiplier. SPY **butterfly / labCreateOpenDefault** use 1; SPY non-butterfly stay at today’s hint (**5**). Butterfly **never** calls `productWingHint` | Juliet · Issue 1/6 | Plan lock |
| **L12** | Runner may call `heatmapColumnWidths({ symbol })` and use `XSP_SPY_FLY_WIDTHS` when overlay is absent. Panel follows overlay-gated profile. **Do not** add `profile` to `ChainContext` this program (TRSB sibling) | Juliet | Plan lock |
...
| **OD-XS7** | `defaultDiagonalWidth(XSP)=15` and `axisSpot` XSP=6000 | **In scope**, small XS1 seeds, **XSP only**. SPY diagonal already 5; SPY axisSpot already falls through to 100 — out unless Coach expands | **XSP only** |
...
| **JR5** | Hotel owns listed-vs-arithmetic; Sheldon is **not** seated (probe already on disk) |
| **JR6** | L11b: XSP `productWingHint` → 1 (all width-bearing Lab templates). SPY hint stays 5. Do not “fix” SPY vertical 5 → 1 this packet |
| **JR7** | Overlay gate: XSP/SPY profile widths accepted only when `fly_width_mode === "fixed_points"` **and** `min(widths) < 10` **and** `max(widths) ≤ 7`. Kind-default MSC_SPX or step-multiples 1…8 → `XSP_SPY_FLY_WIDTHS` |
...
| **Hotel** | Listed vs arithmetic (OD-XS2); fillability vs 20-wide XSP; no invented strikes |
...
flowchart TB
  ...
  subgraph resolvers [This program]
    HC["heatmapColumnWidths overlay-gated"]
...
/** DL-699 overlay list — XSP/SPY only. Used when the overlay is absent or is a kind-default. */
export const XSP_SPY_FLY_WIDTHS = [1, 2, 3, 4, 5, 6, 7] as const;

function isXspSpyFixedOverlay(
  profile?: { fly_widths?: number[]; fly_width_mode?: string } | null,
): boolean {
  if (!profile || profile.fly_width_mode !== "fixed_points") return false;
  const w = (profile.fly_widths || []).filter(
    (n) => Number.isFinite(n) && n > 0,
  );
  if (!w.length) return false;
  // Overlay 1–7. Reject kind-default MSC_SPX (min 20) and mis-tagged 10…50.
  return Math.min(...w) < 10 && Math.max(...w) <= 7;
}
...
export function heatmapColumnWidths(opts: {
  ...
  if (s === "XSP" || s === "SPY") {
    if (isXspSpyFixedOverlay(opts.profile)) {
      return opts.profile!.fly_widths!.filter((n) => n > 0);
    }
    return [...XSP_SPY_FLY_WIDTHS];
  }
  return [...HEATMAP_FLY_WIDTHS];
}
```

`useOptionsLab()` **always** returns a coerced profile (`web/lib/optionsLabContext.tsx` L100–115). `coerceSymbolProfile` fills `fly_widths` from `kindDefaultProfile`: XSP `kind=index` → MSC_SPX `[20…50]`; SPY `kind=etf` → step-multiples `[1..8]`. An empty-list fallback **never runs** on the panel. The overlay gate is the actual fallback.

**Required unit ATs for the helper (not optional):**

- `{ symbol: "XSP", profile: coerceSymbolProfile(null, "XSP", "index") }` → `[1..7]`, **not** `[20…50]`
- `{ symbol: "SPY", profile: coerceSymbolProfile(null, "SPY", "etf") }` → `[1..7]`, **not** `[1..8]`
- `{ symbol: "XSP", profile: { fly_width_mode: "fixed_points", fly_widths: [1,2,3,4,5,6,7] } }` → `[1..7]`
- `{ symbol: "NDX" | "RUT" | "VIX" | "QQQ" | "IWM" | "AAPL" | "SPX" }` (profile omitted or kind-default) → `[10…50]`
...
**Runner (L12 — intentional split):** `ChainContext.symbol` lives at `web/lib/options-lab/templates/types.ts` L30 (there is **no** `web/lib/runner/templates/types.ts`). Pass symbol only:

```ts
fixedPoints: heatmapColumnWidths({ symbol: ctx.symbol }),
```

Do **not** add `profile` to `ChainContext` this program (TRSB sibling). Runner XSP/SPY therefore uses `XSP_SPY_FLY_WIDTHS` even if a future overlay list differs. Panel follows the overlay gate. If the DB overlay later changes off `[1..7]`, that is a later packet.

**`widthList` in `symFly.ts` / `vertical.ts` / `bwFly.ts`:** keep honoring `params.fixedPoints`. Change the fallthrough from `heatmapFlyWidths()` to `heatmapColumnWidths({ symbol: ctx.symbol })`.
...
2. **`productWingHint` (L11b — named, not a drive-by):** `XSP` → **1** (was 20). **SPY stays 5** (already the fallthrough). NDX/NQ* stay 50; RUT/SPX stay 20. Templates that assign `wingWidth: w` (vertical, bwb, condor, iron_fly, calendar, diagonal, straddle, strangle, single) therefore seed **1** on XSP. **`iron_condor` keeps the as-built formula** `Math.max(w, w * 2)` (`builderCreateDefault.ts` L212) — with `w = 1` that is **2**, not 1. Do **not** change the multiplier this packet. ATs per template (AT-XS2b). Do not change SPY non-butterfly recipes this packet (JR6).

- Blurb: stop saying “20-wide butterfly” as universal copy (Tango: “ATM call butterfly — profile minimum listed wing”).

`PositionBuilder.handleTemplate` (L1310–1338) already snaps `lab.wingWidth`. Once butterfly recipe is 1, XSP Lab strategy-switch to butterfly seeds 1 then `snapWidthToListed`. XSP switch to vertical follows the new hint (1). **Delta FAIL** if this packet also edits dialog chrome, Tos padlock, or `defaultWidth` (WIDTH-1 Keep).
...
sequenceDiagram
  ...
  H->>R: symbol=XSP, overlay-gated profile
  R-->>H: [1,2,3,4,5,6,7]
...
| `productWingHint("XSP")` | 20 | 1 |
| `productWingHint("SPY")` | **5** | **5** (unchanged) |
| `labDefaultForStrategy("vertical", "XSP").wingWidth` | 20 | **1** (L11b) |
| `labDefaultForStrategy("vertical", "SPY").wingWidth` | 5 | **5** (JR6) |
| `labDefaultForStrategy("iron_fly", "XSP").wingWidth` | 20 | **1** (L11b) |
| `labDefaultForStrategy("iron_condor", "XSP").wingWidth` | `Math.max(20, 40)` = 40 | **2** (`Math.max(1, 2)` — **keep the 2× formula**) |
| `labDefaultForStrategy("straddle" \| "strangle" \| "single", "XSP").wingWidth` | 20 | **1** (L11b) |
...
| `heatmapColumnWidths({ symbol: "XSP", overlay 1–7 })` | n/a | `[1..7]` |
...
| Seed | Agent | Intent |
|------|-------|--------|
| **XS0-1** | India | AZ-DEF-4 stands; DL-435 scoped reverse needs new DL **in PR 3**; L5 changelog-row-not-bump; DL-539 tree list; L3 Width picker locked; `AnalyzerPositionsList` freeze |
| **XS0-2** | Hotel | Listed vs arithmetic golden; probe 20+ bid-null; OD-XS2 rec (a); HM8 invalid cells on sparse SPY heatmap 1-wide columns |
...
| **XS0-5** | Charlie | Feasibility: `butterflyWingWidth` vs `productWingHint`; overlay gate; ingestKey+symbol; `FLY_MAX_WIDTHS` cap; do not call `flyWidthsFromProfile` globally |
...
| **XS0-0** | Coach | Stamp `agents/go/XS0-W0.md` **only after** seeds exist (S10). OD-XS0, XS1, XS2, XS4–XS8. JR1–7. Plan sha1 → DL. AGENTS.md line **in the GO commit / PR 1** |

### Phase XS1 — Create / recipe scale

| Seed | Agent | Intent |
|------|-------|--------|
| **XS1-0** | Charlie | `butterflyWingWidth` (XSP\|SPY → 1 else 20); butterfly + `labCreateOpenDefault` use it **only**; `productWingHint("XSP")` → 1, SPY hint stays 5; `handleTemplate` Lab branch; tests split |
| **XS1-1** | Charlie · Hotel | OD-XS7 **XSP only**: `defaultDiagonalWidth` 15 → 1; `axisSpot` 6000 → ~spot-class. Do not touch SPY diagonal (already 5) |
| **XS1-2** | Kilo | `builderCreateDefault.test.ts`: XSP/SPY butterfly 1; **keep** 20 for SPX/NDX/RUT/QQQ/IWM/AAPL; XSP vertical/iron_fly/straddle/strangle/single 1; XSP iron_condor **2**; SPY vertical 5; grep `defaultWidth` NDX/NQ* → 50 untouched |
| **XS1-3** | Tango | Butterfly blurb no longer claims universal 20-wide |
| **XS1-G** | Delta · Kilo · Hotel | XSP butterfly 1; SPY butterfly 1; SPX/NDX/QQQ 20; XSP vertical/straddle 1; XSP iron_condor **2**; **diff does not include** `defaultWidth`, Tos padlock, dialog chrome |

### Phase XS2 — Heatmap columns

| Seed | Agent | Intent |
|------|-------|--------|
| **XS2-0** | Charlie | New `heatmapColumnWidths.ts` with overlay gate; wire panel, runner (symbol only), widthList fallthroughs, widthFitTemplate fallback; ingestKey includes symbol; **`lastIngestRef` skip when panel.symbol changed and bus.hash has not** — do not touch `useOptionChainBus.ts` |
...
| **XS2-2** | Kilo | AT-XS7/8/8b; coerced-index XSP still `[1..7]`; `advancedFly.structure.test.ts` still 10…50 for `heatmapFlyWidths()`; LIM fixtures still use the constant |
| **XS2-3** | Echo | Visual 7-col XSP vs 9-col SPX; footer Width Fit \(n\) |
| **XS2-G** | Delta · Echo · Kilo · Lima | XSP heatmap 1–7; SPX/NDX/RUT/VIX/QQQ/IWM 10–50; DL-435 reverse **in the diff**; no LIM/WF math diff |
...
| Seed | Agent | Intent |
|------|-------|--------|
| **XS4-0** | Kilo | AT-XS1…16 + 8b/2b/12b on disk with command evidence |
...
| **AT-XS2b** | XSP `labDefaultForStrategy` `wingWidth === 1` for **vertical, bwb, condor, iron_fly, calendar, diagonal, straddle, strangle, single**. XSP **iron_condor === 2** (`Math.max(w, w*2)` with `w=1` — **do not change the multiplier**). SPY **vertical** stays **5**. NDX vertical stays **50** |
| **AT-XS3** | Grep: no `s === "SPX" \|\| s === "XSP"` wing 20. `productWingHint("SPY")` still 5 |
...
| **AT-XS7** | `heatmapColumnWidths({ symbol: "XSP", profile: { fly_width_mode: "fixed_points", fly_widths: [1..7] } })` → `[1..7]` |
| **AT-XS7b** | Coerced kind-default: `heatmapColumnWidths({ symbol: "XSP", profile: coerceSymbolProfile(null, "XSP", "index") })` → `[1..7]`, **not** `[20…50]`. SPY coerced etf → `[1..7]`, **not** `[1..8]` |
| **AT-XS8** | `heatmapColumnWidths({ symbol: "SPX" })` → `[10,15,…,50]`; `heatmapFlyWidths()` still `[10…50]` |
| **AT-XS8b** | `heatmapColumnWidths({ symbol })` for **NDX, RUT, VIX, QQQ, IWM, AAPL** → `[10…50]` (not step-multiples 1…8) |
...
| Signal | How |
|--------|-----|
| Characterization | AT-XS1…16 + 2b/5b/7b/8b/12b command output in `gate-reports/` |
...
4. Heatmap Advanced Fly **and** Width Fit on XSP/SPY → columns **1–7** (if OD-XS1 a). Shot.  
5. Heatmap SPX **and** NDX/RUT/VIX/QQQ/IWM/AAPL → columns **10…50** (unit AT-XS8b; Playwright SPX).  
6. `labDefaultForStrategy("butterfly")` / `labCreateOpenDefault` XSP/SPY **1**, SPX **20**. XSP vertical/iron_fly/straddle/strangle/single **1**. XSP iron_condor **2**. SPY vertical **5**.  
...
| Kind-default MSC_SPX painted as XSP heatmap | **High** | Overlay gate · AT-XS7b · JR7 |
...
| Empty-shell axisSpot 6000 on XSP | **Low** | OD-XS7 XSP only |
...
| `p-template-runner-stream-book` | Runner heatmap / width-fit wrap | XS2 touches `web/lib/runner/templates/{heatmap,width-fit}.ts` **registration only** — same helper. Do not reopen TRSB |
...
- [ ] Plan revision **v1.0** and whole-file sha1  
...
- [ ] OD-XS0, XS1, XS2, XS4–XS8 Accept or Override (table on the token). **No OD-XS3** — Width picker is L3  
- [ ] JR1–7  
- [ ] **L11b** (XSP `wingWidth: w` templates → 1; **iron_condor stays 2**; SPY butterfly 1; SPY hint stays 5)  
- [ ] **L12** (runner uses constant; panel follows overlay)  
...
- `web/lib/options-lab/builderCreateDefault.ts` (`butterflyWingWidth`, `productWingHint("XSP")` only)  
...
- `web/lib/options-lab/templates/heatmapColumnWidths.test.ts` (**new**)  
...
1. **OD-XS1** — Heatmap XSP/SPY columns: overlay 1–7 vs keep DL-435 10…50 for heatmap only.  
...
7. **OD-XS7** — Include XSP `axisSpot` 6000 and XSP `defaultDiagonalWidth` 15 in XS1? (SPY diagonal already 5 — out.)  
8. **OD-XS8** — Vertical / bw-fly share the resolver?

No other product questions are silently decided. L11b (XSP all Lab width-bearing templates → 1) is **plan lock**; Coach Override on the token if he wants butterfly-only.
...
2. **Two resolvers, not `flyWidthsFromProfile` globally.** Heatmap SPX-class must remain DL-435 `[10…50]`; XSP/SPY consume **overlay-gated** `[1..7]`, never kind-default MSC_SPX.  
3. **Do not mutate `HEATMAP_FLY_WIDTHS`.** LIM/AF fixtures stay valid.  
...
11. **Butterfly uses `butterflyWingWidth` (XSP\|SPY → 1 else 20), never `productWingHint`.** NDX/QQQ/IWM/AAPL/RUT/SPX butterfly stay 20 (JR2).  
12. **L11b:** XSP templates with `wingWidth: w` → 1. **iron_condor stays 2× hint.** SPY non-butterfly stay at hint 5.  
...
16. **L12:** runner uses the XSP/SPY constant; panel follows overlay. No `ChainContext.profile` this program.  
...
- **Files/components affected:** `agents/p-options-lab-xsp-spy-scale/ORCHESTRATOR.md`, `agents/p-options-lab-xsp-spy-scale/seeds/*`, `agents/p-options-lab-xsp-spy-scale/characterization-list.md`, `agents/go/XS0-W0.md`, `AGENTS.md` (active-program line), `Architecture/00-decision-log.md` (GO + reassignment DL only), `docs/Options-Lab-XSP-SPY-Scale-Full-Agent-Bench-Plan-v1.0.md` (when Coach names the land path)
- **Dependencies:** None (docs/governance only; blocked on Coach XS0-0 before code PRs)
- **Description:** Materialize the bench **and** introduce the third tree (DL-539) **before** any product edit. Seed files must exist before the stamp. No product code.
...
- **Description:** `butterflyWingWidth` (XSP|SPY → 1 else 20). `productWingHint("XSP")` → 1; SPY hint stays 5. XSP templates with `wingWidth: w` → 1; **iron_condor stays `Math.max(w, w*2)` = 2**. SPX/NDX/QQQ/IWM/AAPL/RUT butterfly stay 20. Sequence behind Dialog board if that GO is open. Delta FAIL on chrome / padlock / `defaultWidth`.

### PR 3: Heatmap column resolver + scoped DL-435 reverse (XS2)

- **Files/components affected:** `web/lib/options-lab/templates/heatmapColumnWidths.ts`, `web/lib/options-lab/templates/heatmapColumnWidths.test.ts`, `web/lib/options-lab/templates/symFly.ts` (`widthList` fallthrough), `web/lib/options-lab/templates/vertical.ts`, `web/lib/options-lab/templates/bwFly.ts`, `web/lib/options-lab/templates/widthFitTemplate.ts`, `web/lib/options-lab/templates/flySurfacePipeline.ts`, `web/components/options-lab/HeatmapChainPanel.tsx`, `web/lib/runner/templates/heatmap.ts`, `web/lib/runner/templates/width-fit.ts`, `Architecture/00-decision-log.md` (scoped DL-435 reverse)
- **Dependencies:** PR 1, XS0-0, OD-XS1 (a). **Sibling of PR 2** — does not wait on PR 2. **Descoped entirely if OD-XS1 (b)**
- **Description:** Overlay-gated columns: XSP/SPY `[1..7]` (reject kind-default MSC_SPX / 1…8). SPX/NDX/RUT/VIX/QQQ/IWM/AAPL `[10…50]`. `heatmapFlyWidths()` identity preserved. Cap ≥ 9. IngestKey includes symbol. `lastIngestRef` skip when panel.symbol changed and bus.hash has not; hide stale paint. **No** `useOptionChainBus.ts`. **The DL-435 reverse is in this PR**, not PR 6.
...
- **Dependencies:** PR 2, PR 4, and **PR 3 if OD-XS1 (a)**. If OD-XS1 (b), do **not** require heatmap 1–7 shots or AT-XS7/9/10
- **Description:** Land the untracked e2e with `OUT` on **this** board’s `gate-reports/xs4/` (not WF). Playwright shots. Inline pytest overlay + live SELECT that migrate 152 is on the MacBook DB. tsc. Grep leftovers including NDX/RUT/VIX/QQQ/IWM. Gate XS4-G.
...
**End of Options Lab XSP / SPY Scale Refactor Full Agent Bench Plan v1.0 (Draft)**
