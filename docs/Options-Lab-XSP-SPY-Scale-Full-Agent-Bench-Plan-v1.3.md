...
**Date:** 2026-09-13  
**Status:** **Execution authority** — Coach: *“let’s execute plan 1.3”* (2026-09-13). PR 1 materializes `agents/go/XS0-W0.md` as the Delta stamp (**DL-328**: this sentence is not the stamp file; the token is). Silent-at-GO OD defaults apply.  
**Plan revision:** **v1.3** (v1.0, v1.1, v1.2 SUPERSEDED)  
**Author:** Juliet (orchestration) / design-doc-writer  
**Authority:** Coach (GO / ship)  
**Canonical land path:** `docs/Options-Lab-XSP-SPY-Scale-Full-Agent-Bench-Plan-v1.3.md`  
**Parent:** v1.2 (Amendment A). **v1.3 freeze:** durable boundary NX18 / FI-049 / DL-328; GO checklist NX18 tick; Runner **is** Template Runner (`web/lib/runner/`), not a coined name. No product-behaviour change vs v1.2.  
**This packet is the XSP/SPY template bugfix only.** Do not deploy MiniTwo. Do not bump a Spec version. Do not edit `AnalyzerPositionsList.tsx`. **NX18:** a passing XS gate is not IKI Labs progress.

### v1.3 vs v1.2 (why this file exists)

v1.2 was mutated in place after issue (NX18, GO tick). That was a process miss — later folds belong in a new revision. v1.3 is the frozen execution snapshot of v1.2 **plus**:

| Add | Where |
|-----|--------|
| **NX18 / FI-049** | IKI Labs / `observer-light` / Factory catalog are **out**. Standing review context only |
| **DL-328** | Chat is not the GO token. PR 1 writes `agents/go/XS0-W0.md` |
| **“A passing XS gate is not progress on the bigger goal”** | GO checklist tick · NX18 · FI-049 · Observer note v0.6 |
| **Runner = Template Runner** | Not a name clash with `web/lib/runner/`. Do not re-raise |

### Amendment A (Claude 2026-09-13) — required for v1.2

Intent unchanged. No new phase, no new file, no new symbol in scope, no product behaviour change on any symbol **today**.

| ID | Finding | v1.2 disposition |
|----|---------|------------------|
| **A1** | Provenance lives only in the helper; `profileLine` (L1221–1225) cannot tell universe from kind-default when both paint `1–7`. Empty `flyWidths` renders `undefined–undefined` | `profileLine` **only**: append `· universe` / `· kind default` / `· unknown`; empty list → `widths — (no column list)`. AT-XS17/18/19. Echo + Tango confirm wording |
| **A2** | Overlay path still branches `XSP \|\| SPY` — fifth per-symbol table | Overlay path has **no symbol literal**. Universe `fixed_points` wins for every symbol. Offline map `OFFLINE_FALLBACK_WIDTHS` is XSP/SPY only. AT-XS7g / AT-XS20 / AT-XS21. **OD-XS11** on whether offline should fail loud |
| **Editorial** | §6 file table still said `productWingHint("XSP") → 1` and nine-template ATs — contradicts OD-XS9 (a) | §6 rows rewritten. Largest scope-explosion risk in v1.1 was a stale table |

### Advisor fold (Claude review of v1.0 → v1.1) — still in force

IDs **v1.1-A*** below are the v1.0-review advisories, **not** Amendment A.

| ID | Finding | Disposition (still in force) |
|----|---------|------------------|
| **B1** | Overlay gate `max ≤ 7` makes the constant authoritative and the database advisory | Gate on **`profile.source === "market_symbol_universe"`** and **`fly_width_mode === "fixed_points"`**. Empty overlay **fails loud**. Constant is kind-default / offline fallback only |
| **B2** | AT-XS7 is byte-identical to the fallback (LIM E15 class) | **AT-XS7c** interior overlay `[1,2,3]`. **AT-XS7d** `[1..8]` not clamped |
| **B3** | QQQ and IWM labelled non-regression | **Deferred defects** · **XS-ETF** |
| **B4** | L12 licensed runner ≠ panel | **OD-XS10** · `ChainContext.columnWidths` |
| **v1.1-A1** | L11b applies hint=1 to nine templates | **OD-XS9** butterfly-only |
| **v1.1-A2** | Dead `min < 10` clause | Moot (B1) |
| **v1.1-A3** | `axisSpot` both arms stale | OD-XS7 XSP only; SPX deferred |
...
| **NX18** | IKI Labs suite, `observer-light`, Factory catalog, Runner-as-gift, stream widening past `chain`. Coach 2026-09-13: this packet **only** fixes a glaring problem on **current** templates. It does **not** address the bigger goal and is **not intended to**. **A passing XS gate is not progress on the bigger goal.** Chat does not count (**DL-328**). Context: `docs/Observer-Post-Trial-Gift-Surface-Note-2026-09-13.md` · **FI-049**. **Runner** in that note **is** Template Runner (`web/lib/runner/`) — not a new name and not a collision |
...
| `productWingHint("XSP")` = 20 | `builderCreateDefault.ts` L91–96: `if (s === "SPX" \|\| s === "XSP") return 20`. **SPY is not in that branch** — SPY hint is already **5** | **OD-XS9 silent: leave at 20.** Do not change this packet |
...
| `builderCreateDefault.test.ts` | asserts 20 for SPX, XSP, SPY, QQQ, IWM, AAPL, NDX, RUT | XSP/SPY **butterfly** → 1; **keep** 20 for SPX, RUT, NDX, QQQ, IWM, AAPL. **No non-butterfly XSP assertions unless OD-XS9 (b)** |
...
| **OD-XS11** | Offline / kind-default fallback: substitute `[1..7]` for XSP/SPY, or fail loud? | **(a)** Keep `OFFLINE_FALLBACK_WIDTHS` as written — byte-identical to v1.1, zero risk, one small symbol map remains. **(b)** Return `[]` for every non-universe profile and paint nothing until the row arrives (LIM34; at 120 symbols this is likely where it lands) | **(a)** — preserve v1.1 behaviour exactly |
...
| **Tango** | Off-hours SPY honesty copy; no “best width / optimizer”; learner capacity; **A1 `profileLine` wording** (XS2-4) |
...
**A2 is behaviour-neutral today only if no third symbol carries `fixed_points`.** Confirm, do not assume (AT-XS7g):

```sql
SELECT symbol, JSON_EXTRACT(app_profile_json, '$.fly_width_mode')
FROM market_symbol_universe
WHERE JSON_EXTRACT(app_profile_json, '$.fly_width_mode') = 'fixed_points';
```

Expect exactly `XSP` and `SPY`. Any third row → **stop and report**; A2 would change that symbol’s heatmap and that is a product decision, not a refactor.

SPX-class rows are `msc_spx`, QQQ/IWM are `step_multiples` — they never hit branch 1. DL-435 remainder and the QQQ/IWM deferred-defect freeze stay intact. The scoped DL-435 reverse in PR 3 is still required and still scoped to XSP/SPY (they are the only `fixed_points` rows).
...
- **AT-XS7g:** live SELECT `fixed_points` = exactly XSP, SPY
- **AT-XS20:** TESTSYM universe `fixed_points [2,4,6]` → `[2,4,6]` (no symbol list on overlay path)
- **AT-XS21:** TESTSYM kind-default equity → `[10…50]`
...
**Chrome — `profileLine` only (Amendment A1).** As-built L1221–1225 concatenates `widths ${flyWidths[0]}–${flyWidths[flyWidths.length - 1]}` with no source token. A universe overlay and a kind-default fallback that both paint `[1..7]` are identical on screen (LIM E15 class). Empty overlay (`[]`) currently renders `undefined–undefined` — not fail-loud.

Replace **that prop only**. No new element, no new prop, no layout change:

```ts
const widthSource =
  profile.source === "market_symbol_universe" ? "universe"
  : profile.source === "client_kind_default" ? "kind default"
  : "unknown";
...
| State | Renders |
|-------|---------|
| XSP, universe overlay `[1..7]` | `Profile · step 1 · widths 1–7 · universe · index` |
| XSP, offline / kind default | `Profile · step 1 · widths 1–7 · kind default · index` |
| XSP, universe overlay empty | `Profile · step 1 · widths — (no column list) · universe · index` |
| SPX | `Profile · step 5 · widths 10–50 · universe\|kind default · index` |

Echo and Tango confirm the wording (one line each on the token). The strings above are a default, not a copy decision.

**Sibling, not A1:** L1353–1354 (width-range caption) uses the same `flyWidths[0]` / `length - 1` pattern. A1 does **not** touch it. Echo XS2-3 records whether that caption needs the same empty guard — do not expand A1 silently.
...
if (widths && widths.length) return paintWith(widths);
if (Array.isArray(widths) && widths.length === 0) return emptyTiles(ctx); // fail loud
// list missing: caller never resolved. Do not invent a second SoR.
return emptyTiles(ctx);
```
...
| Path | Action |
|------|--------|
| `web/lib/options-lab/templates/heatmapColumnWidths.ts` | **New** helper. Overlay path has **no symbol literal** (A2). `OFFLINE_FALLBACK_WIDTHS` XSP/SPY only. `XSP_SPY_FLY_WIDTHS` exported deprecated |
| `web/lib/options-lab/templates/heatmapColumnWidths.test.ts` | AT-XS7b/7c/7d/7e + **AT-XS20/21**. No `XSP \|\| SPY` on the overlay path |
...
| `web/lib/options-lab/templates/types.ts` | optional `columnWidths?` if OD-XS10 (a) |
| `web/components/options-lab/HeatmapChainPanel.tsx` | `flyWidths` from helper + deps; **`profileLine` source token + empty-list phrase (A1)**; `lastIngestRef`; set `ctx.columnWidths` |
| `web/lib/runner/templates/heatmap.ts` | consume `ctx.columnWidths` (OD-XS10 a). **Not** `heatmapColumnWidths({ symbol })` as a second SoR |
| `web/lib/runner/templates/width-fit.ts` | same |
| `web/lib/options-lab/builderCreateDefault.ts` | `butterflyWingWidth` only. **`productWingHint` unchanged unless Coach Accepts OD-XS9 (b)** |
| `web/lib/options-lab/builderCreateDefault.test.ts` | XSP/SPY butterfly 1; keep the 20 loop for SPX/NDX/RUT/QQQ/IWM/AAPL. **No non-butterfly XSP assertions unless OD-XS9 (b)** |
...
| Seed | Agent | Intent |
|------|-------|--------|
| **XS2-0** | Charlie | New `heatmapColumnWidths.ts`: provenance gate, **no symbol literal on overlay path** (A2); `OFFLINE_FALLBACK_WIDTHS`; wire panel; **`profileLine` A1**; **OD-XS10 (a):** `ChainContext.columnWidths`; **`lastIngestRef` skip**. AT-XS7g SELECT **before** the helper lands. Do not touch `useOptionChainBus.ts` |
...
| **XS2-2** | Kilo | AT-XS7 / **7b / 7c / 7d / 7e / 7g / 7f** / 8 / 8b / **17 / 18 / 19 / 20 / 21**; coerced-index XSP still `[1..7]`; **7c and 20 fail if profile or the symbol list is the overlay discriminator**; `advancedFly.structure.test.ts` still 10…50 for `heatmapFlyWidths()` |
| **XS2-3** | Echo | Visual 7-col XSP vs 9-col SPX; footer Width Fit \(n\); **confirm `profileLine` wording** (A1). Record L1353–1354 empty-guard as a sibling, do not expand A1 |
| **XS2-4** | Tango | One-line confirm: `universe` / `kind default` / `widths — (no column list)` are observation, not “best/optimizer” |
| **XS2-G** | Delta · Echo · Kilo · Lima · Tango | Overlay path has **no** `XSP \|\| SPY`; AT-XS7c + AT-XS20 PASS; AT-XS7g is exactly two rows; chrome source token visible; no `undefined` in `profileLine`; runner cols **===** panel cols (AT-XS7f); DL-435 reverse **in the diff** |
...
| Seed | Agent | Intent |
|------|-------|--------|
| **XS4-0** | Kilo | AT-XS1…21 + 8b/2b/**7c/7d/7e/7f/7g**/12b on disk. **XS2-G cannot PASS without AT-XS7c, AT-XS17, AT-XS20** |
...
| **AT-XS17** | XSP with `source: "market_symbol_universe"` → `profileLine` contains `· universe`. Same XSP with `source: "client_kind_default"` → contains `· kind default`. The two strings differ even though both render `widths 1–7` |
| **AT-XS18** | `flyWidths = []` → chrome contains **no** `undefined`; renders the no-column-list phrase |
| **AT-XS19** | SPX chrome unchanged apart from the appended source token |
| **AT-XS7g** | Live SELECT `fly_width_mode = 'fixed_points'` returns **exactly** `XSP` and `SPY` on this MacBook’s `labs` DB. Filed in `gate-reports/xs2/fixed-points-rows.txt`. Any third row → **stop** |
| **AT-XS20** | `heatmapColumnWidths({ symbol: "TESTSYM", profile: { source: "market_symbol_universe", fly_width_mode: "fixed_points", fly_widths: [2,4,6] } })` → `[2,4,6]`. Fails if the `XSP \|\| SPY` overlay branch survives |
| **AT-XS21** | `heatmapColumnWidths({ symbol: "TESTSYM", profile: coerceSymbolProfile(null, "TESTSYM", "equity") })` → `[10…50]` |
...
- **IKI Labs orbit / Factory catalog / observer-light** — **NX18**. Not a phase of this program |
...
17. `profileLine` shows `universe` vs `kind default` (AT-XS17); empty list never prints `undefined` (AT-XS18).  
18. Overlay path has **no** `XSP \|\| SPY` (AT-XS20). AT-XS7g is exactly XSP and SPY.
...
| Overlay path still lists `XSP \|\| SPY` (A2) | **High** | AT-XS20 TESTSYM · no symbol literal on branch 1 |
| Third `fixed_points` row in DB; A2 changes that symbol | **High** | AT-XS7g SELECT before helper lands; stop and report |
| Empty `flyWidths` prints `undefined–undefined` | **Med** | A1 `profileLine` guard · AT-XS18 |
| Universe vs kind-default both look like `widths 1–7` | **Med** | A1 source token · AT-XS17 |
...
- [ ] Plan revision **v1.3** and whole-file sha1 — **not v1.0, v1.1, or v1.2**  
...
- [ ] OD-XS0, XS1, XS2, XS4–**XS11** Accept or Override (table on the token). **No OD-XS3** — Width picker is L3  
- [ ] JR1–**8**  
- [ ] **OD-XS9** (silent: butterfly-only; `productWingHint("XSP")` stays 20)  
- [ ] **OD-XS10** (silent: `ChainContext.columnWidths`; no runner/panel split)  
- [ ] **OD-XS11** (silent: keep `OFFLINE_FALLBACK_WIDTHS`)  
- [ ] Echo + Tango confirm `profileLine` wording (A1)  
- [ ] **NX18 / FI-049:** this GO does **not** entitle IKI Labs, `observer-light`, or Factory catalog. **A passing XS gate is not progress on the bigger goal.** Runner = existing Template Runner, not a coined name  
...
- `web/components/options-lab/HeatmapChainPanel.tsx` (`flyWidths` useMemo + **`profileLine` A1** + ingestKey + `lastIngestRef` + `columnWidths`). **Not** `useOptionChainBus.ts`. **Not** L1353–1354 caption (Echo notes only)  
...
11. **OD-XS11** — Offline fallback `[1..7]` for XSP/SPY vs fail loud until the universe row arrives.

B1–B4 and Amendment A1/A2 are **not** open. They are disposed in this revision. OD-XS9 silent (butterfly-only) is still Coach’s to Override.
...
19. **Overlay path has no symbol list (A2).** Universe `fixed_points` is SoR for every symbol. `OFFLINE_FALLBACK_WIDTHS` is XSP/SPY only, and only when the universe has not spoken. AT-XS20 proves TESTSYM overlay. AT-XS7g proves no third `fixed_points` row.  
20. **`profileLine` shows provenance (A1).** Universe vs kind-default must be visible even when both paint `1–7`. Empty list must not render `undefined`.
...
- **Files/components affected:** `agents/p-options-lab-xsp-spy-scale/ORCHESTRATOR.md`, `agents/p-options-lab-xsp-spy-scale/seeds/*`, `agents/p-options-lab-xsp-spy-scale/characterization-list.md`, `agents/go/XS0-W0.md`, `AGENTS.md` (active-program line), `Architecture/00-decision-log.md` (GO + reassignment DL only), `docs/Options-Lab-XSP-SPY-Scale-Full-Agent-Bench-Plan-v1.3.md`
- **Dependencies:** None (docs/governance only; blocked on Coach XS0-0 before code PRs)
- **Description:** Materialize the bench **and** introduce the third tree (DL-539) **before** any product edit. Seed files must exist; this PR writes `agents/go/XS0-W0.md` as Coach GO (silent-at-GO OD defaults). No product code. Land this **v1.3** plan.
...
- **Description:** Provenance-gated columns. Overlay path has **no** `XSP \|\| SPY` (AT-XS20). AT-XS7g SELECT exactly two `fixed_points` rows. `profileLine` source token + empty-list phrase (AT-XS17/18/19). Runner consumes `ctx.columnWidths`. Cap ≥ 9. `lastIngestRef` skip. **The DL-435 reverse is in this PR**, not PR 6.
...
**End of Options Lab XSP / SPY Scale Refactor Full Agent Bench Plan v1.3 (execution authority)**
