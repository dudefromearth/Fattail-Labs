...
**Status:** **FOLDED** into Guiding Doctrine Spec v1.0 **§17** and Arch **32** §6c (**DL-385** grammar · **DL-386** market memory, 2026-08-16).  
**Source:** Coach PDF `docs/VP Structural Analysis — Reference for the Timing & Entry Trigger Grammar.pdf` (updated 2026-08-16 13:39 with §7 peer-reviewed long-memory grounding). Full Coach text: [`VP-Structural-Analysis-Trigger-Grammar-Reference_1.md`](./VP-Structural-Analysis-Trigger-Grammar-Reference_1.md).  
**Authority is the Spec.** This file is the searchable extract.

**From:** Coach 2026-08-16  
**For:** Strategy Lab Designer (Timing & Entry tab) and backtester resolution loop  
**Companion:** Coach’s morning structural analysis (ES1!, 5-minute, Aug 12–14 2026) — the artifact sent to members every morning.  
**Binds:** Config Resolution Standard (§16) · VP as 0DTE **entry events** (not only 3–5 DTE placement) · Arch 32 VP lens

---

## 1. What the image is

This is not a chart decoration. It is the **human version of a strategy trigger**: every morning Coach publishes the structural levels and where they meet the session boundaries, and members trade the **interactions**. **The cyan arrows are the strategy.** The Designer’s job is to make the Timing & Entry tab able to write what the arrows say; the backtester’s job is to fire the same events on replay.

## 2. Reading the chart

**Horizontal lines — VP structural levels, color-classed by node topology.** Colors describe *where the level sits in the volume structure* (Coach’s system):

| Color on chart | Class |
|----------------|-------|
| **Green** | **HVN bottom** — the lower edge of a high-volume node |
| **Red** | **HVN top** — the upper edge of a high-volume node |
| **Yellow** | **LVN** — structure *between* nodes (low-volume gaps) |
| **Blue** | **Intranode structure** — levels *inside* a node |
| Retracement ladder | Fib over the prior swing (0 / 0.25 / 0.33 / 0.5 / 0.68 / 0.75 / 1 / 1.25 / 1.5) |

A red and a green line **bracket an HVN** (top and bottom); blue lines are internal shelves; yellow is the thin ground between nodes. Travel between them is fast through yellow and stalls at red/green.

**Vertical blue dashed lines — session boundaries:** overnight, premarket, open (9:30), mid-morning, midday, early afternoon, late afternoon, close. Structure interacts *differently* at each boundary; the boundary pair defines the window.

**Cyan arrows — the events.** Each arrow: price **interacts** with a **level** inside a **session window**, then **travels** to the next level.
...
| Field | Values (surface-relative, **never absolute**) |
|-------|-----------------------------------------------|
| **level_class** | HVN top · HVN bottom · LVN (between nodes) · intranode structure · retracement level |
...
A strategy can require **all three** of: regime admits · window open · **structural event fires** → selector places the fly.

## 4. Engine

- **Designer:** Timing & Entry writes this grammar. **VP levels are entry events for 0DTE**, not only a 3–5 DTE placement lens.  
- **Backtester:** surface read includes **today’s profile with classified levels**; selector runs only when the trigger has fired.  
- **Capture / replay:** VP derived on replay from the tape (frozen-geometry). Source labeled: **SPY tape now; ES when wired** (this chart is ES — where the structure actually forms).

## 5. Provenance for the classifier

The classifier (HVN top vs LVN between these two nodes) is part of the strategy’s provenance. Version and name it on every resolved trade. Coach’s morning-analysis classification is the **reference implementation**. Level classes are the vocabulary; **concrete prices are never stored in a config**.

## 6. Acceptance (Timing & Entry tab)

1. A member can express every arrow on the chart as a config trigger — no arrow requires a hard price.  
2. The same trigger, on replay of the gold day, fires at the moments Coach’s arrows mark.  
3. The heatmap preview shows where the selector would place the fly **when** the trigger fires — one selector, two callers.  
...
**Lineage seat:** this section is Yankee's lane (Mandelbrot channel) — the first packet in that lane. Yankee gates the framing: fat tails and long memory as documented properties, never as a slogan; math matched to the audience.
