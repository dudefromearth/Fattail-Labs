...
def test_display_rebin_coarser_conserves_volume():
    ...
    err, out = display_rebin(native, native_row=0.25, requested_row=1.0)
    assert err is None
    assert len(out) == 1
    assert out[0]["price"] == 7700.00
    assert out[0]["volume"] == 100


def test_display_rebin_finer_is_loud_422():
    from market_data.vp_engine.display_rebin import display_rebin

    err, out = display_rebin(
        [{"price": 7700.00, "volume": 1}], native_row=0.25, requested_row=0.10
    )
    assert err is not None
    assert err["error"] == "row_below_substrate"
    assert out == []


def test_display_rebin_non_multiple_is_loud_422():
    from market_data.vp_engine.display_rebin import display_rebin

    err, _ = display_rebin(
        [{"price": 7700.00, "volume": 1}], native_row=0.25, requested_row=0.30
    )
    assert err is not None
    assert err["error"] == "row_not_multiple_of_substrate"
