...
def test_scheduled_halt_is_never_a_gap(tmp_path: Path):
    ...
    during = datetime(2026, 9, 17, 16, 30, tzinfo=CT)
    outside = datetime(2026, 9, 17, 12, 0, tzinfo=CT)
    assert cme_equity_index_halt_now(during)
    assert not cme_equity_index_halt_now(outside)
    assert halt_is_scheduled(
        {"results": [{"type": "maintenance_halt"}]}, now=during, product="ES"
    )
    assert not halt_is_scheduled(
        {"results": [{"type": "maintenance_halt"}]}, now=outside, product="ES"
    )
