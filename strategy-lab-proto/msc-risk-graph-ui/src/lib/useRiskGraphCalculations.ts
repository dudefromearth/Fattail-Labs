...
/** Structural max-loss in total dollars for theo curve floor. */
function structuralMaxLossDollars(strategy: Strategy, debitAbs: number): number {
  if (strategy.costBasisType === 'credit') {
    let width = strategy.width || 0;
    if (strategy.legs && strategy.legs.length >= 2) {
      const strikes = [...new Set(strategy.legs.map(l => l.strike))].sort((a, b) => a - b);
      if (strikes.length >= 2) {
        let minGap = Infinity;
        for (let i = 1; i < strikes.length; i++) {
          minGap = Math.min(minGap, strikes[i] - strikes[i - 1]);
        }
        if (minGap > 0 && minGap < Infinity) width = minGap;
      }
    } else if (strategy.strategy === 'butterfly' && strategy.width > 0) {
      // Legacy half-width field on butterflies
      width = strategy.width;
    }
    return Math.max(0, width - debitAbs) * 100;
  }
  return Math.max(0, debitAbs) * 100;
}
  ...
  const surfaceCompute = useMemo(() => {
    ...
    try {
      ...
      const activeStrategies = visibleStrategies.filter(strat => {
        // Strategy Lab shape-studio ids: always price as live (T floored below).
        // 0DTE after the cash close would otherwise vanish into expired-ghost only.
        if (typeof strat.id === 'string' && strat.id.startsWith('labs-')) return true;

        const realDays = realTimeDaysMap.get(strat.id) ?? 0;
        // Same-session after-hours (realDays slightly negative) still draws with T floor.
        // Only fully settle multi-day past close.
        if (realDays - timeOffsetDays < -1) return false;
        // Double-check: if all legs have an expiration date well in the past, treat as expired
        ...
        if (!timeOffsetDays && strat.legs && strat.legs.length > 0) {
          const allLegsExpired = strat.legs.every(l => {
            ...
            // >1 calendar day past settlement
            return expClose.getTime() < now.getTime() - 24 * 60 * 60 * 1000;
          });
      ...
      //
      // Labs design-studio (id labs-*): NEVER mid-calibrate. Card debit is already
      // package BS mid (same model as the theo mark). Anchoring cardMid − |bs|
      // double-counts and can freeze the residual shape when debit reprices
      // without the mark curve (R2R moves, orange curve looks glued).
      ...
      for (const strat of activeStrategies) {
        const isLabs = typeof strat.id === 'string' && strat.id.startsWith('labs-');
        ...
        // Keep SIGNED package mark (short credit packages are negative).
        // Older code forced abs(debit) when bsValue < 0, then anchored
        // marketNet(signed) − absDebit → huge false offset that flattened theo.
        const storedDebit = strat.debit != null && Number.isFinite(strat.debit) ? Math.abs(strat.debit) : null;
        const absBs = Math.abs(bsValue);
        const safeAbs =
          absBs > 0.01 ? absBs : (storedDebit != null && storedDebit > 0 ? storedDebit : 0.01);
        strategyBsValues[strat.id] = safeAbs;
        strategyTheoValues[strat.id] = safeAbs;

        if (isLabs) {
          // No anchor — P&L = mark(S) − entry with entry = ±card debit.
          continue;
        }

        // Anchor offset from per-leg mids (same chain data as server).
        // Use SIGNED package BS so short packages don't get mid − (+debit).
        if (getMid && strat.legs && strat.legs.length > 0) {
          ...
          if (allMids) {
            anchorOffsets[strat.id] = marketNet - bsValue;
          }
        }
        // Fallback: card shows abs mid; convert to signed via costBasisType
        if (anchorOffsets[strat.id] == null) {
          const cardMid = naturalPricesOverride?.[strat.id];
          if (cardMid != null && cardMid > 0 && Number.isFinite(bsValue)) {
            const signedCard =
              strat.costBasisType === 'credit' ? -Math.abs(cardMid) : Math.abs(cardMid);
            anchorOffsets[strat.id] = signedCard - bsValue;
          }
        }
      }
      ...
      for (const price of pricePoints) {
        ...
        for (const strat of activeStrategies) {
          ...
          // Floor at structural max loss — credit defined-risk = (width − credit)×100,
          // not credit×100 (that wrongly clips short IC/fly wings at −credit).
          const effectiveDebitAbs = Math.abs(getEffectiveDebit(effectiveStrat));
          stratTheoPnL = Math.max(-structuralMaxLossDollars(effectiveStrat, effectiveDebitAbs), stratTheoPnL);
      ...
      for (let si = 1; si < NT_SLICES; si++) {
        ...
        for (const price of pricePoints) {
          ...
          for (const strat of activeStrategies) {
            ...
            const anchor = anchorOffsets[strat.id];
            if (anchor != null) stratTheoPnL += anchor * 100;
            const dAbs = Math.abs(getEffectiveDebit(effectiveStrat));
            stratTheoPnL = Math.max(-structuralMaxLossDollars(effectiveStrat, dAbs), stratTheoPnL);
            theoPnL += stratTheoPnL;
          }
