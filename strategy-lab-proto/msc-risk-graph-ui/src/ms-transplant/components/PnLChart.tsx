...
/** Strike drag-to-reposition callback. info during drag, null on release (commit). */
  ...
  /** Labs: sticky viewState — do not autofit on every strike change after first fit */
  autoFitOnStrategyChange?: boolean;
...
const PnLChart = forwardRef<PnLChartHandle, PnLChartProps>(({
  ...
  // Strike drag-to-reposition state (MSC handle extension)
  ...
  // Redraw when P&L data or strikes change (handle drag re-render)
  useEffect(() => {
    scheduleDraw();
  }, [
    expirationData,
  ...
  // Mouse handlers for pan/zoom — MSC model (mouse events + onWheel).
  // Do not use pointer-capture / window listeners here: they broke pan/zoom in the Labs iframe.
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    // Strike tick hit (only near zero-line ticks — keep small so pan still works)
    const zeroY = toCanvasY(0, height);
    const sMap = strikeToStrategyIdRef.current;
    if (sMap && sMap.size > 0 && zeroY >= PADDING.top && zeroY <= height - PADDING.bottom) {
      for (const strike of strikesRef.current) {
        const key = Math.round(Number(strike) * 100) / 100;
        const stratId = sMap.get(strike) ?? sMap.get(key);
        if (!stratId) continue;
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (isXAxisDrag.current) {
      ...
      const newXRange = Math.max(xRange * zoomFactor, 1e-6);
      viewState.current = {
      ...
      const newYRange = Math.max(yRange * zoomFactor, 1e-6);
      viewState.current = {
        ...viewAtDragStart.current,
  ...
  const handleWheel = useCallback((e: React.WheelEvent) => {
    ...
    if (e.shiftKey) {
      ...
      viewState.current.yMin = dataY - (dataY - yMin) * zoomFactor;
      viewState.current.yMax = dataY + (yMax - dataY) * zoomFactor;
    } else {
      // Normal scroll: zoom X only
      viewState.current.xMin = dataX - (dataX - xMin) * zoomFactor;
      viewState.current.xMax = dataX + (xMax - dataX) * zoomFactor;
    }
  ...
  const handleMouseHover = useCallback((e: React.MouseEvent) => {
    ...
    // Y-axis zone = left area
      ...
      if (sMap && sMap.size > 0 && zeroY >= PADDING.top && zeroY <= height - PADDING.bottom) {
        for (const strike of strikesRef.current) {
          const key = Math.round(Number(strike) * 100) / 100;
          if (!sMap.has(strike) && !sMap.has(key)) continue;
          const sx = toCanvasX(strike, width);
          if (Math.abs(mouseX - sx) < 15 && Math.abs(mouseY - zeroY) < 15) {
            foundStrike = strike;
            break;
  ...
  // Labs: autoFitOnStrategyChange=false keeps sticky viewState across handle drop.
  const strategyHash = strikes.join(',');
  const prevStrategyHash = useRef<string | null>(null);
  /** First fit often runs before expiration/theo points exist → wrong viewport
   *  and ungrabbable handles (ticks mapped off-screen). Refit once curves land. */
  const hasCurvesFitRef = useRef(false);

  useEffect(() => {
    if (strategyHash !== prevStrategyHash.current) {
      const isFirst = prevStrategyHash.current === null;
      prevStrategyHash.current = strategyHash;
      // New structure: allow a curves-ready fit again for this hash
      if (!isFirst && autoFitOnStrategyChange) {
        hasCurvesFitRef.current = false;
      }
      if (isStrikeDragging.current) {
        draw();
      } else if (autoFitOnStrategyChange || isFirst) {
        autoFit();
  ...
  }, [strategyHash, autoFit, draw, autoFitOnStrategyChange]);

  // Critical for Labs (autoFitOnStrategyChange=false): first strike hash may fire
  // with empty curves. Force Re-render "fixed" handles only because remount+autofit
  // ran after data existed. Fit once when curves first become available.
  useEffect(() => {
    const hasCurves =
      expirationData.length > 1 || theoreticalData.length > 1;
    if (!hasCurves || strikes.length === 0 || hasCurvesFitRef.current) return;
    if (isStrikeDragging.current) return;
    hasCurvesFitRef.current = true;
    autoFit();
    const t = window.setTimeout(() => autoFit(), 80);
  ...
  return (
    ...
    >
      ...
      <canvas
        ...
        onMouseDown={(e) => {
          // MSC handlers for pan / axis-zoom / strike handles.
          // Alerts stay on right-click (context menu) — never steal left-drag.
          closeContextMenu();
          if (e.button === 0) {
            handleMouseDown(e);
            setCursorStyle('grabbing');
        ...
        onMouseMove={(e) => {
          handleMouseMove(e);
          if (!isDragging.current && !isStrikeDragging.current) handleMouseHover(e);
        }}
