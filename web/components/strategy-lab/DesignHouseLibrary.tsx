...
/**
 * FatTail house strategies — admin catalog.
 * Apply / Copy & rebuild write onto the selected Design bot.
 */
...
export default function DesignHouseLibrary({
  ...
  async function onAction(d: HouseDesign, mode: "apply" | "copy_rebuild") {
    ...
    try {
      ...
      onApplied(cfg, mode);
      const ok =
        mode === "apply" ? `Applied “${d.name}”.` : `Copy loaded: “${d.name}”.`;
      pushNotice?.("success", ok);
      setLocalOk(ok);
    } catch (e) {
      const m =
        e instanceof Error ? e.message : "House design action failed.";
      setErr(m);
      pushNotice?.("error", m);
    } finally {
      setBusy(null);
  ...
  return (
    <div data-testid="design-house-library">
      {err ? (
            ...
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <div className="flex shrink-0 flex-wrap gap-1">
                <button
                  ...
                  className="rounded-md bg-blue-600 px-2 py-0.5 text-[11px] font-semibold text-white hover:bg-blue-700 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="rounded-md border border-[var(--color-separator)] px-2 py-0.5 text-[11px] font-semibold hover:bg-[var(--color-fill)] disabled:opacity-50"
                >
